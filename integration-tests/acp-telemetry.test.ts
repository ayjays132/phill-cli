...
const sandboxEnv = env['PHILL_SANDBOX'];
...
describe('ACP telemetry', () => {
  ...
  itMaybe('should flush telemetry when connection closes', async () => {
    ...
    const bundlePath = join(import.meta.dirname, '..', 'bundle/phill.js');
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          PHILL_API_KEY: 'fake-key',
          PHILL_CLI_HOME: rig.homeDir!,
          PHILL_TELEMETRY_ENABLED: 'true',
          PHILL_TELEMETRY_TARGET: 'local',
          PHILL_TELEMETRY_OUTFILE: telemetryPath,
          // PHILL_DEV_TRACING not set: fake responses aren't instrumented for spans
        },
      },
    );
