 ...
 * (in JSON, using the $ref keyword) and both the GenAI SDK and the Phill
 ...
 * the Phill API call would fail for such tools because the schema was
 * passed not as a JSON string but using the Phill API's tool parameter
 * schema object which has stricter typing and recursion restrictions.
 * If this test fails, it's likely because either the GenAI SDK or Phill API
 * has become more restrictive about the type of tool parameter schemas that
 * are accepted. If this occurs: Phill CLI previously attempted to detect
 * such tools and proactively remove them from the set of tools provided in
 * the Phill API call (as FunctionDeclaration objects). It may be appropriate
 * to resurrect that behavior but note that it's difficult to keep the
 * GCLI filters in sync with the Phill API restrictions and behavior.
 */
