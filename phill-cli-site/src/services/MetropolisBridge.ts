...
class MetropolisBridge {
  private static instance: MetropolisBridge;
  
  private constructor() {}

  public static getInstance(): MetropolisBridge {
   ...
   */
  async getGridSignals(): Promise<MetropolisSignal[]> {
    try {
   ...
   */
  async getLatestWins(): Promise<CasinoWin[]> {
    try {
      const response = await axios.get(PHILLBOOK_API_BASE, {
      ...
      if (response.data && response.data.feed && response.data.feed.ledger) {
        return response.data.feed.ledger.map((f: any) => ({
   ...
   */
  async getGlobalEvents(): Promise<string[]> {
    try {
      const response = await axios.get(PHILLBOOK_API_BASE, {
