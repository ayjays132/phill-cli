...
try {
    ...
    console.warn('[SessionManager] @google/genai not found. Music generation will fail.');
}
export class SessionManager extends EventEmitter {
    static instance;
    session = null; // Lyria RealTime Session
    currentIntent = null;
    outputHandler = null;
    constructor() {
        super();
    ...
    async getCredentials() {
        ...
        const apiKey = process.env['PHILL_API_KEY'] ||
        ...
        if (envToken) {
            console.log('[SessionManager] Using GOOGLE_CLOUD_ACCESS_TOKEN from environment.');
        ...
        try {
            ...
            console.warn('[SessionManager] efficient auth library not found or failed, skipping ADC check.');
        }
        console.warn('[SessionManager] No valid credentials found. Session may be unauthenticated.');
        return {};
    }
    async connect() {
        if (this.session)
        ...
        console.log('[SessionManager] Connecting to Lyria RealTime (models/lyria-realtime-exp)...');
        try {
    ...
    async startGeneration(intent) {
        ...
        await this.session.setMusicGenerationConfig({
            musicGenerationConfig: {
                ...
                // map other fields
                density: typeof intent.density === 'number' ? intent.density : undefined,
            },
        });
    ...
    async steer(partialIntent) {
        if (!this.session || !this.currentIntent)
            return;
