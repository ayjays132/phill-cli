...
interface MinimalSpeaker {
  write(chunk: Buffer): void;
  end(): void;
}

export class OutputHandler {
  private speaker: MinimalSpeaker | null = null;
  ...
  private initSpeaker() {
    try {
      ...
      console.warn(
        "[OutputHandler] 'speaker' package not found. Streaming playback disabled.",
      );
    }
  }
  ...
  async finalize(): Promise<string | null> {
    ...
    if (
      this.intent.outputFormat === 'mp3' ||
      this.intent.outputFormat === 'ogg'
    ) {
      console.log(
        `[OutputHandler] Converting to ${this.intent.outputFormat}...`,
      );
