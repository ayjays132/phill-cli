...
- **[Before you begin](#before-you-begin):** Essential steps to take before
  becoming a Phill CLI contributor.
...
- **[Documentation contribution process](#documentation-contribution-process):**
  How to contribute documentation to Phill CLI.
...
To run the review tool, enter the following command from within Phill CLI:
...
with a value of `PHILL_API_KEY` and set that to a valid API key that you have
...
```bash
git clone https://github.com/ayjays132/phill-cli.git # Or your fork's URL
cd phill-cli
```
...
setting `PHILL_SANDBOX=true` in your `~/.env` and ensuring a sandboxing
provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See
[Sandboxing](#sandboxing) for details.

To build both the `phill` CLI utility and the sandbox container, run
...
To start the Phill CLI from the source code (after building), run the following
...
If you'd like to run the source build outside of the phill-cli folder, you can
utilize `npm link path/to/phill-cli/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias phill="node path/to/phill-cli/packages/cli"` to run with `phill`
...
the Phill CLI. They are not run as part of the default `npm run test` command.
...
- Consult
  [PHILL.md](https://github.com/ayjays132/phill-cli/blob/main/GEMINI.md)
...
- `packages/`: Contains the individual sub-packages of the project.
  - `a2a-server`: A2A server implementation for the Phill CLI. (Experimental)
  - `cli/`: The command-line interface.
  - `core/`: The core backend logic for the Phill CLI.
  - `test-utils` Utilities for creating and cleaning temporary file systems for
    testing.
  - `vscode-ide-companion/`: The Phill CLI Companion extension pairs with
    Phill CLI.
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/phill.js` within the
...
phill-cli due to automatic exclusion. Use `.phill/.env` files for phill-cli
...
1.  **Start the Phill CLI in development mode:**
...
On macOS, `phill` uses Seatbelt (`sandbox-exec`) under a `permissive-open`
...
`.phill/sandbox-macos-<profile>.sb` under your project settings directory
`.phill`.

#### Container-based sandboxing (all platforms)

For stronger container-based sandboxing on macOS or other platforms, you can set
`PHILL_SANDBOX=true|docker|podman|<command>` in your environment or `.env`
...
as you start/stop Phill CLI. Files created within the sandbox should be
...
for your projects by creating the files `.phill/sandbox.Dockerfile` and/or
`.phill/sandbox.bashrc` under your project settings directory (`.phill`) and
running `phill` with `BUILD_SANDBOX=1` to trigger building of your custom
sandbox.
...
can be specified as `PHILL_SANDBOX_PROXY_COMMAND=<command>`, where `<command>`
...
- Open [an issue](https://github.com/ayjays132/phill-cli/issues) to discuss
  your proposed changes.
- Reach out to the maintainers.

We appreciate your contributions to making Phill CLI documentation better!
