...
The Phill CLI includes a powerful policy engine that provides fine-grained
...
1.  **Create the policy directory** if it doesn't exist:
    ```bash
    mkdir -p ~/.phill/policies
    ```
2.  **Create a new policy file** (e.g., `~/.phill/policies/my-rules.toml`). You
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (e.g., ask Phill CLI to
...
| :------ | :--- | :------------------------------------------------------------------------- |
| Default | 1    | Built-in policies that ship with the Phill CLI.                           |
...
| Tier      | Type   | Location                    |
| :-------- | :----- | :-------------------------- |
| **User**  | Custom | `~/.phill/policies/*.toml` |
...
| :---------- | :------------------------------------------------ |
| **Linux**   | `/etc/phill-cli/policies`                        |
| **macOS**   | `/Library/Application Support/PhillCli/policies` |
| **Windows** | `C:\ProgramData\phill-cli\policies`              |
...
The Phill CLI ships with a set of default policies to provide a safe
