...
Sub-agents are specialized agents that operate within your main Phill CLI
...
Think of sub-agents as "specialists" that the main Phill agent can hire for a
specific job.
...
Phill CLI comes with powerful built-in sub-agents.
...
- **Name:** `cli_help`
- **Purpose:** Expert knowledge about Phill CLI itself, its commands,
...
1.  **Project-level:** `.phill/agents/*.md` (Shared with your team)
2.  **User-level:** `~/.phill/agents/*.md` (Personal agents)
...
**Example: `.phill/agents/security-auditor.md`**
...
Phill CLI can also delegate tasks to remote subagents using the Agent-to-Agent
(A2A) protocol.
