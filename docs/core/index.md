...
Phill CLI's core package (`packages/core`) is the backend portion of Phill
CLI, handling communication with the Phill API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of Phill CLI, see the
...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  PHILL.md import feature using @file.md syntax.
- **[Policy Engine](./policy-engine.md):** Use the Policy Engine for
  fine-grained control over tool execution.

## Role of the core

While the `packages/cli` portion of Phill CLI provides the user interface,
`packages/core` is responsible for:

- **Phill API interaction:** Securely communicating with the Google Phill API,
  sending user prompts, and receiving model responses.
- **Prompt engineering:** Constructing effective prompts for the Phill model,
  potentially incorporating conversation history, tool definitions, and
  instructional context from `PHILL.md` files.
- **Tool management & orchestration:**
  - Registering available tools (e.g., file system tools, shell command
    execution).
  - Interpreting tool use requests from the Phill model.
  - Executing the requested tools with the provided arguments.
  - Returning tool execution results to the Phill model for further processing.
...
- **API key management:** It handles the `PHILL_API_KEY` and ensures it's used
  securely when communicating with the Phill API.
...
To ensure that long conversations don't exceed the token limits of the Phill
...
Phill CLI includes a model fallback mechanism to ensure that you can continue
...
`PHILL.md` files that provide context to the model. It searches for these files
...
`refresh` the content of loaded `PHILL.md` files.

## Citations

When Phill finds it is reciting text from a source it appends the citation to
