...
To enable dev traces, set the `PHILL_DEV_TRACING=true` environment variable when
running Phill CLI.
...
2.  **Run Phill CLI with dev tracing:**

    In a separate terminal, run your Phill CLI command with the
    `PHILL_DEV_TRACING` environment variable:

    ```bash
    PHILL_DEV_TRACING=true phill
    ```
...
```typescript
import { runInDevTraceSpan } from 'phill-cli-core';
...
await runInDevTraceSpan({ name: 'my-custom-span' }, async ({ metadata }) => {
  ...
  metadata.attributes['gen_ai.request.model'] = 'phill-4.0-mega';
