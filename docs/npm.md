...
This monorepo contains two main packages: `phill-cli` and `phill-cli-core`.

## `phill-cli`

This is the main package for the Phill CLI. It is responsible for the user
...
`phill-cli-core`. This means that whether a user installs the package with
`npm install -g phill-cli` or runs it directly with `npx phill-cli`, they are
using this single, self-contained executable.

## `phill-cli-core`

This package contains the core logic for interacting with the Phill API. It is
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace phill-cli`.
