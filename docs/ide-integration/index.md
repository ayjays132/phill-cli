...
Phill CLI can integrate with your IDE to provide a more seamless and
...
- **Native diffing:** When Phill suggests code modifications, you can view the
...
- **VS Code commands:** You can access Phill CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Phill CLI: Run`: Starts a new Phill CLI session in the integrated
    terminal.
  - `Phill CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Phill CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Phill CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Phill CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Phill CLI:
...
> NOTE: The "Phill CLI Companion" extension may appear towards the bottom of
...
When enabled, Phill CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask Phill to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **Phill CLI: Accept Diff**.
...
- Open the Command Palette and run **Phill CLI: Close Diff Editor**.
...
If you are using Phill CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Phill CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Phill CLI could not find the necessary environment variables
    (`PHILL_CLI_IDE_WORKSPACE_PATH` or `PHILL_CLI_IDE_SERVER_PORT`) to connect
    to the IDE. This usually means the IDE companion extension is not running or
    did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **Phill CLI Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Phill CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Phill CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Phill CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Phill CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Phill CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Phill CLI
    Companion", and
