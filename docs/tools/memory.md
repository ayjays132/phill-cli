...
This document describes the `save_memory` tool for the Phill CLI.

## Description

Use `save_memory` to save and recall information across your Phill CLI
...
## How to use `save_memory` with the Phill CLI

The tool appends the provided `fact` to a special `PHILL.md` file located in
the user's home directory (`~/.phill/PHILL.md`). This file can be configured
to have a different name.

Once added, the facts are stored under a `## Phill Added Memories` section.
...
```
save_memory(fact="The project I'm currently working on is called 'phill-cli'.")
```
