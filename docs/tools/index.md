...
The Phill CLI includes built-in tools that the Phill model uses to interact
...
## Overview of Phill CLI tools

In the context of the Phill CLI, tools are specific functions or modules that
the Phill model can request to be executed. For example, if you ask Phill to
...
definitions (schemas) to the Phill model, executes them when requested, and
returns the results to the model for further processing into a user-facing
response.

These tools provide the following capabilities:

- **Access local information:** Tools allow Phill to access your local file
  system, read file contents, list directories, etc.
- **Execute commands:** With tools like `run_shell_command`, Phill can run
...
- **Ground responses:** By using tools to fetch real-time or specific local
  data, Phill's responses can be more accurate, relevant, and grounded in your
  actual context.

## How to use Phill CLI tools

To use Phill CLI tools, provide a prompt to the Phill CLI. The process works
as follows:

1.  You provide a prompt to the Phill CLI.
2.  The CLI sends the prompt to the core.
3.  The core, along with your prompt and conversation history, sends a list of
    available tools and their descriptions/schemas to the Phill API.
4.  The Phill model analyzes your request. If it determines that a tool is
...
6.  The output from the tool is sent back to the Phill model.
7.  The Phill model uses the tool's output to formulate its final answer, which
...
in mind. The Phill CLI will typically:
...
- **Utilize sandboxing:** All tools are subject to restrictions enforced by
  sandboxing (see [Sandboxing in the Phill CLI](../cli/sandbox.md)). This means
...
## Learn more about Phill CLI's tools

Phill CLI's built-in tools can be broadly categorized as follows:
...
- **[MCP servers](./mcp-server.md)**: MCP servers act as a bridge between the
  Phill model and your local environment or other services like APIs.
