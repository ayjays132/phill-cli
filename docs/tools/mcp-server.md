# MCP servers with the Phill CLI
...
An MCP server is an application that exposes tools and resources to the Phill
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the Phill model
and your local environment or other services like APIs.

An MCP server enables the Phill CLI to:
...
With an MCP server, you can extend the Phill CLI's capabilities to perform
actions beyond its built-in features, such as interacting with databases, APIs,
custom scripts, or specialized workflows.

## Core integration architecture

The Phill CLI integrates with MCP servers through a sophisticated discovery and
...
4. **Sanitizes and validates** tool schemas for compatibility with the Phill
   API
...
The Phill CLI supports three MCP transport types:
...
prompts. Phill CLI discovers these automatically and gives you the possibility
...
The Phill CLI uses the `mcpServers` configuration in your `settings.json` file
...
The Phill CLI supports OAuth 2.0 authentication for remote MCP servers using
...
- **Stored securely** in `~/.phill/mcp-oauth-tokens.json`
...
When the Phill CLI starts, it performs MCP server discovery through the
...
4. **Name sanitization:** Tool names are cleaned to meet Phill API
   requirements:
...
Tool parameter schemas undergo sanitization for Phill API compatibility:
...
When the Phill model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the Phill model like built-in
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for Phill API compatibility
...
Phill CLI's capabilities while maintaining security, reliability, and ease of
use.
...
The Phill CLI will correctly process this array, separating text from binary
...
When the Phill CLI receives this response, it will:
...
context to the Phill model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within the Phill CLI. This allows you to create
...
```bash
/poem-writer --title="Phill CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Phill CLI" reverent
```

When you run this command, the Phill CLI executes the `prompts/get` method on
...
## Managing MCP servers with `phill mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, the Phill CLI provides a convenient set of commands to
...
`~/.phill/settings.json` or the project config `.phill/settings.json` file.

**Command:**

```bash
phill mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
phill mcp add [options] <name> <command> [args...]

# Example: Adding a local server
phill mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
phill mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
phill mcp add --transport http <name> <url>

# Example: Adding an HTTP server
phill mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
phill mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
phill mcp add --transport sse <name> <url>

# Example: Adding an SSE server
phill mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
phill mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`phill mcp list`)
...
### Removing a server (`phill mcp remove`)
...
### Enabling/disabling a server (`phill mcp enable`, `phill mcp disable`)
...
```bash
phill mcp enable <name> [--session]
phill mcp disable <name> [--session]
```
...
`~/.phill/mcp-server-enablement.json`.
