...
The Phill CLI includes a Checkpointing feature that automatically saves a
...
1.  **A Git snapshot:** A commit is made in a special, shadow Git repository
    located in your home directory (`~/.phill/history/<project_hash>`). This
...
`~/.phill/tmp/<project_hash>/checkpoints`.
