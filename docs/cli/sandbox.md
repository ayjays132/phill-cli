# Sandboxing in the Phill CLI

This document provides a guide to sandboxing in the Phill CLI, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the Phill CLI:
...
phill -s -p "analyze the code structure"

# Use environment variable
export PHILL_SANDBOX=true
phill -p "run the test suite"
...
2. **Environment variable**: `PHILL_SANDBOX=true|docker|podman|sandbox-exec`
...
```bash
DEBUG=1 phill -s -p "debug command"
```
...
```bash
# Check environment
phill -s -p "run shell command: env | grep SANDBOX"

# List mounts
phill -s -p "run shell command: mount | grep workspace"
```
