# System Prompt Override (PHILL_SYSTEM_MD)

The core system instructions that guide Phill CLI can be completely replaced
with your own Markdown file. This feature is controlled via the
`PHILL_SYSTEM_MD` environment variable.

## Overview

The `PHILL_SYSTEM_MD` variable instructs the CLI to use an external Markdown
...
via a `.phill/.env` file. See
[Persisting Environment Variables](../get-started/authentication.md#persisting-environment-variables).

- Use the project default path (`.phill/system.md`):
  - `PHILL_SYSTEM_MD=true` or `PHILL_SYSTEM_MD=1`
  - The CLI reads `./.phill/system.md` (relative to your current project
    directory).

- Use a custom file path:
  - `PHILL_SYSTEM_MD=/absolute/path/to/my-system.md`
...
- Disable the override (use built‑in prompt):
  - `PHILL_SYSTEM_MD=false` or `PHILL_SYSTEM_MD=0` or unset the variable.
...
- One‑off session using a project file:
  - `PHILL_SYSTEM_MD=1 phill`
- Persist for a project using `.phill/.env`:
  - Create `.phill/system.md`, then add to `.phill/.env`:
    - `PHILL_SYSTEM_MD=1`
- Use a custom file under your home directory:
  - `PHILL_SYSTEM_MD=~/prompts/SYSTEM.md phill`

## UI indicator

When `PHILL_SYSTEM_MD` is active, the CLI shows a `|⌐■_■|` indicator in the UI
...
- Write the built‑in prompt to the project default path:
  - `PHILL_WRITE_SYSTEM_MD=1 phill`
- Or write to a custom path:
  - `PHILL_WRITE_SYSTEM_MD=~/prompts/DEFAULT_SYSTEM.md phill`

This creates the file and writes the current built‑in system prompt to it.

## Best practices: SYSTEM.md vs PHILL.md
...
PHILL.md focused on high‑level guidance and project specifics.

## Troubleshooting

- Error: `missing system prompt file '…'`
  - Ensure the referenced path exists and is readable.
  - For `PHILL_SYSTEM_MD=1|true`, create `./.phill/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.phill/.env` or export in your shell).
  - Paths are resolved from the current working directory; try an absolute path.
- Restore defaults
  - Unset `PHILL_SYSTEM_MD` or set it to `0`/`false`.
