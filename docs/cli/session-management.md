...
Phill CLI includes robust session management features that automatically save
...
Every time you interact with Phill CLI, your session is automatically saved.
...
- **Location:** Sessions are stored in `~/.phill/tmp/<project_hash>/chats/`.
...
- **Resume by ID:** You can also provide the full session UUID:
  ```bash
  phill --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890
  ```
...
You can configure how Phill CLI manages your session history in your
`settings.json` file.
