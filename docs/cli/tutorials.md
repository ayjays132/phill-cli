...
This page contains tutorials for interacting with Phill CLI.
...
[`.phill/settings.json` file](../get-started/configuration.md). Within the
...
Phill CLI uses this value in the `mcpServers` configuration that you defined in
the `settings.json` file.

#### Launch Phill CLI and verify the connection

When you launch Phill CLI, it automatically reads your configuration and
launches the GitHub MCP server in the background. You can then use natural
language prompts to ask Phill CLI to perform GitHub actions. For example:
