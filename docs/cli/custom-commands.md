...
prompts as personal shortcuts within Phill CLI. You can create commands that
...
Phill CLI discovers commands from two locations, loaded in a specific order:

1.  **User commands (global):** Located in `~/.phill/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.phill/commands/`. These commands are specific to the
...
- A file at `~/.phill/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.phill/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the Phill model when the
...
```toml
# In: <project>/.phill/commands/changelog.toml
...
When a custom command attempts to execute a shell command, Phill CLI will now
...
````toml
# In: <project>/.phill/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.phillignore` if enabled.
...
```toml
# In: <project>/.phill/commands/review.toml
...
```bash
mkdir -p ~/.phill/commands/refactor
touch ~/.phill/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.phill/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.phill/commands/refactor/pure.toml
...
Phill CLI will then execute the multi-line prompt defined in your TOML file.
