# Phill CLI keyboard shortcuts

Phill CLI ships with a set of default keyboard shortcuts for editing input,
...
- `Option+B/F/M` (macOS only): Are interpreted as `Alt+B/F/M` even if your
  terminal isn't configured to send Meta with Option.
- `Alt+Shift+T` / `Ctrl+Alt+T` / `Ctrl+Shift+T`: Toggle TTS in voice mode
  (fallbacks provided for terminals that do not emit reliable `Alt+Shift` key
  events).
