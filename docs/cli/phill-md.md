# Provide context with PHILL.md files

Context files, which use the default name `PHILL.md`, are a powerful feature
for providing instructional context to the Phill model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.phill/PHILL.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `PHILL.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `PHILL.md` files in subdirectories
...
### Example `PHILL.md` file

Here is an example of what you can include in a `PHILL.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `PHILL.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.phill/PHILL.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `PHILL.md` files into smaller, more manageable
...
While `PHILL.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "PHILL.md"]
  }
}
```
