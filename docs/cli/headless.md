...
Headless mode allows you to run Phill CLI programmatically from command line
...
The headless mode provides a headless interface to Phill CLI that:
...
```bash
phill --prompt "What is machine learning?"
```

### Stdin input

Pipe input to Phill CLI from your terminal:

```bash
echo "Explain this code" | phill
```

### Combining with file input

Read from files and process with Phill:

```bash
cat README.md | phill --prompt "Summarize this documentation"
```
...
```bash
phill -p "What is the capital of France?"
```
...
```bash
phill -p "What is the capital of France?" --output-format json
```
...
phill --output-format stream-json --prompt "What is 2+2?"

# Save event stream to file
phill --output-format stream-json --prompt "Analyze this code" > events.jsonl

# Parse with jq
phill --output-format stream-json --prompt "List files" | jq -r '.type'
```
...
```bash
# Save to file
phill -p "Explain Docker" > docker-explanation.txt
phill -p "Explain Docker" --output-format json > docker-explanation.json

# Append to file
phill -p "Add more details" >> docker-explanation.txt

# Pipe to other tools
phill -p "What is Kubernetes?" --output-format json | jq '.response'
phill -p "Explain microservices" | wc -w
phill -p "List programming languages" | grep -i "python"
```
...
| ----------------------- | ---------------------------------- | -------------------------------------------------- |
| `--prompt`, `-p`        | Run in headless mode               | `phill -p "query"`                                |
| `--output-format`       | Specify output format (text, json) | `phill -p "query" --output-format json`           |
| `--model`, `-m`         | Specify the Phill model           | `phill -p "query" -m gemini-2.5-flash`            |
| `--debug`, `-d`         | Enable debug mode                  | `phill -p "query" --debug`                        |
| `--include-directories` | Include additional directories     | `phill -p "query" --include-directories src,docs` |
| `--yolo`, `-y`          | Auto-approve all actions           | `phill -p "query" --yolo`                         |
| `--approval-mode`       | Set approval mode                  | `phill -p "query" --approval-mode auto_edit`      |
...
```bash
cat src/auth.py | phill -p "Review this authentication code for security issues" > security-review.txt
```

#### Generate commit messages

```bash
result=$(git diff --cached | phill -p "Write a concise commit message for these changes" --output-format json)
...
```bash
result=$(cat api/routes.js | phill -p "Generate OpenAPI spec for these routes" --output-format json)
...
for file in src/*.py; do
    echo "Analyzing $file..."
    result=$(cat "$file" | phill -p "Find potential bugs and suggest improvements" --output-format json)
...
```bash
result=$(git diff origin/main...HEAD | phill -p "Review these changes for bugs, security issues, and code quality" --output-format json)
...
```bash
grep "ERROR" /var/log/app.log | tail -20 | phill -p "Analyze these errors and suggest root cause and fixes" > error-analysis.txt
```

#### Release notes generation

```bash
result=$(git log --oneline v1.0.0..HEAD | phill -p "Generate release notes from these commits" --output-format json)
...
```bash
result=$(phill -p "Explain this database schema" --include-directories db --output-format json)
