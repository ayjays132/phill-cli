...
"uninstall" the CLI, you must clear this cache, which will remove phill-cli and
...
If you installed the CLI globally (e.g., `npm install -g phill-cli`),
use the `npm uninstall` command with the `-g` flag to remove it.

```bash
npm uninstall -g phill-cli
```
...
`brew install phill-cli`), use the `brew uninstall` command to remove it.

```bash
brew uninstall phill-cli
```

## Method 4: MacPorts

If you installed the CLI globally using MacPorts (e.g.,
`sudo port install phill-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall phill-cli
```
