...
Agent Skills allow you to extend Phill CLI with specialized expertise,
...
Unlike general context files ([`PHILL.md`](./phill-md.md)), which provide
persistent workspace-wide background, Skills represent **on-demand expertise**.
This allows Phill to maintain a vast library of specialized capabilities—such
as security auditing, cloud deployments, or codebase migrations—without
cluttering the model's immediate context window.

Phill autonomously decides when to employ a skill based on your request and the
...
Phill CLI discovers skills from three primary locations:

1.  **Workspace Skills** (`.phill/skills/`): Workspace-specific skills that are
    typically committed to version control and shared with the team.
2.  **User Skills** (`~/.phill/skills/`): Personal skills available across all
    your workspaces.
...
The `phill skills` command provides management utilities:
...
# Uses the user scope by default (~/.phill/skills)
phill skills install https://github.com/user/repo.git
phill skills install /path/to/local/skill
phill skills install /path/to/local/my-expertise.skill

# Install a specific skill from a monorepo or subdirectory using --path
phill skills install https://github.com/my-org/my-skills.git --path skills/frontend-design

# Install to the workspace scope (.phill/skills)
phill skills install /path/to/skill --scope workspace

# Uninstall a skill by name
phill skills uninstall my-expertise --scope workspace

# Enable a skill (globally)
phill skills enable my-expertise

# Disable a skill. Can use --scope to specify workspace or user (defaults to workspace)
phill skills disable my-expertise --scope workspace
```
...
---
name: <unique-name>
description: <what the skill does and when Phill should use it>
---
...
- **`description`**: The most critical field. Phill uses this to decide when
...
Create `~/.phill/skills/code-reviewer/SKILL.md`:
...
2.  **Style**: Ensure code follows the workspace's conventions and idiomatic
    patterns as described in the `PHILL.md` file.
...
When a skill is activated, Phill CLI provides the model with a tree view of the
entire skill directory, allowing it to discover and utilize these assets.

## How it Works (Security & Privacy)

1.  **Discovery**: At the start of a session, Phill CLI scans the discovery
    tiers and injects the name and description of all enabled skills into the
    system prompt.
2.  **Activation**: When Phill identifies a task matching a skill's
