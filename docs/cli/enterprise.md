# Phill CLI for the enterprise

This document outlines configuration patterns and best practices for deploying
and managing Phill CLI in an enterprise environment. By leveraging system-level
...
> using Phill CLI. However, they should not be considered a foolproof security
...
2. User Settings (`~/.phill/settings.json`)
3. Workspace Settings (`<project>/.phill/settings.json`)
...
- **System defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/phill-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.phill/settings.json`):**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["~/phill-context"]
    }
  }
  ```

- **Workspace `settings.json` (`<project>/.phill/settings.json`):**
...
- **System overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/phill-cli/global-context"]
    }
  }
  ```
...
- **Final merged configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/phill-cli/common-context",
        "~/phill-context",
        "./project-context",
        "/etc/phill-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/phill-cli/settings.json`
  - **Windows**: `C:\ProgramData\phill-cli\settings.json`
  - **macOS**: `/Library/Application Support/PhillCli/settings.json`
  - The path can be overridden using the `PHILL_CLI_SYSTEM_SETTINGS_PATH`
...
While the `PHILL_CLI_SYSTEM_SETTINGS_PATH` environment variable provides
...
This approach ensures that no matter how the user calls the `phill` command,
the enterprise settings are always loaded with the highest precedence.

**Example wrapper script:**

Administrators can create a script named `phill` and place it in a directory
that appears earlier in the user's `PATH` than the actual Phill CLI binary
(e.g., `/usr/local/bin/phill`).
...
export PHILL_CLI_SYSTEM_SETTINGS_PATH="/etc/phill-cli/settings.json"

# Find the original phill executable.
# This is a simple example; a more robust solution might be needed
# depending on the installation method.
REAL_PHILL_PATH=$(type -aP phill | grep -v "^$(type -P phill)$" | head -n 1)

if [ -z "$REAL_PHILL_PATH" ]; then
  echo "Error: The original 'phill' executable was not found." >&2
  exit 1
fi

# Pass all arguments to the real Phill CLI executable.
exec "$REAL_PHILL_PATH" "$@"
```

By deploying this script, the `PHILL_CLI_SYSTEM_SETTINGS_PATH` is set within
the script's environment, and the `exec` command replaces the script process
with the actual Phill CLI process, which inherits the environment variable.
This makes it significantly more difficult for a user to bypass the enforced
settings.

## Restricting tool access

You can significantly enhance security by controlling which tools the Phill
...
Phill CLI loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure Phill
...
For auditing and monitoring purposes, you can configure Phill CLI to send
...
control that is configured on a proxy server, not within Phill CLI itself. It
...
patterns discussed above to create a secure, controlled environment for Phill
CLI.
...
```json
{
  ...
  "mcpServers": {
    "corp-tools": {
      "command": "/opt/phill-tools/start.sh",
      "timeout": 5000
    }
  },
  "telemetry": {
