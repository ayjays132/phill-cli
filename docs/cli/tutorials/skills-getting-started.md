...
Agent Skills allow you to extend Phill CLI with specialized expertise. This
...
1.  **Create the skill directory structure:**
    ...
    ```bash
    mkdir -p .phill/skills/api-auditor/scripts
    ```

2.  **Create the `SKILL.md` file:** Create a file at
    `.phill/skills/api-auditor/SKILL.md` with the following content:
...
3.  **Create the bundled Node.js script:** Create a file at
    `.phill/skills/api-auditor/scripts/audit.js`. This script will be used by
    the agent to perform the actual check:

    ```javascript
    // .phill/skills/api-auditor/scripts/audit.js
...
Use the `/skills` slash command (or `phill skills list` from your terminal) to
see if Phill CLI has found your new skill.

In a Phill CLI session:
...
**User:** "Can you audit http://phillli.com"

Phill will recognize the request matches the `api-auditor` description and will
...
Phill will then use the `run_shell_command` tool to execute your bundled Node
script:

`node .phill/skills/api-auditor/scripts/audit.js http://phillli.com`
