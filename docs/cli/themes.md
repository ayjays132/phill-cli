...
Phill CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Phill CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Phill CLI:
...
Selected themes are saved in Phill CLI's
...
Phill CLI allows you to create your own custom color themes by specifying them
...
**Security note:** For your safety, Phill CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in Phill CLI. Your custom
