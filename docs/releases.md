...
package names beginning with `@google-phill/**` instead of `@google/**`.
...
| ---------- | ----------------------------- | --------------------------------------- |
| CLI        | phill-cli                     | @google-phill/phill-cli                 |
| Core       | phill-cli-core                | @google-phill/phill-cli-core A2A Server |
| A2A Server | phill-cli-a2a-server          | @google-phill/phill-cli-a2a-server      |
...
```bash
npm install -g phill-cli@nightly
```
...
The workflow will then run `npm dist-tag add` for the appropriate `phill-cli`,
`phill-cli-core` and `phill-cli-a2a-server` packages, pointing the specified
...
- `npx -y phill-cli@latest --version` to validate the push worked as expected if
  you were not doing a rc or dev tag
- `npx -y phill-cli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall phill-cli && npm uninstall -g phill-cli && npm cache clean --force &&  npm install phill-cli@<version>`
...
1.  Go to the
    [Actions tab](https://github.com/ayjays132/phill-cli/actions/workflows/release-manual.yml)
    of the repository.
...
`packages/cli/google-phill-cli-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the `phill-cli-core`
  and `phill-cli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g phill-cli` will download these packages, and `npm` will
  handle installing the `phill-cli-core` dependency automatically. The code in
...
1.  **The JavaScript bundle is created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      `phill.js`). The `node-pty` library is excluded from this bundle as it
...
2.  **The `bundle` directory is assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `phill.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `phill.js` (from esbuild) -> `bundle/phill.js`
...
3.  **The GitHub release is created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `phill.js` executable, are attached as assets to a new GitHub Release.
    - **Why:** This makes the single-file version of the CLI available for
      direct download and enables the
      `npx https://github.com/ayjays132/phill-cli` command, which downloads and
      runs this specific bundled asset.

**Summary of artifacts**

- **NPM:** Publishes standard, un-bundled Node.js packages. The primary artifact
  is the code in `packages/cli/dist`, which depends on `phill-cli-core`.
- **GitHub release:** Publishes a single, bundled `phill.js` file that contains
