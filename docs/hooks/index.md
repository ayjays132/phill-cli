...
Hooks are scripts or programs that Phill CLI executes at specific points in the
...
Phill CLI waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in Phill CLI's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (e.g.,
   `echo "debug" >&2`). Phill CLI captures `stderr` but never attempts to parse
   it as JSON.

#### Exit codes

Phill CLI uses exit codes to determine the high-level outcome of a hook
execution:
...
Hooks are configured in `settings.json`. Phill CLI merges configurations from
multiple layers in the following order of precedence (highest to lowest):

1.  **Project settings**: `.phill/settings.json` in the current directory.
2.  **User settings**: `~/.phill/settings.json`.
3.  **System settings**: `/etc/phill-cli/settings.json`.
        ...
        "hooks": [
          {
            "name": "security-check",
            "type": "command",
            "command": "$PHILL_PROJECT_DIR/.phill/hooks/security.sh",
            "timeout": 5000
          }
        ]
      }
...
- `PHILL_PROJECT_DIR`: The absolute path to the project root.
- `PHILL_SESSION_ID`: The unique ID for the current session.
- `PHILL_CWD`: The current working directory.
...
Phill CLI **fingerprints** project hooks. If a hook's name or command changes
