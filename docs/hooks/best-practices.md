...
const CACHE_FILE = '.phill/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".phill/hooks/debug.log"
...
# Test the hook
cat test-input.json | .phill/hooks/my-hook.sh
...
Phill CLI uses exit codes for high-level flow control:
...
input=$(cat)
echo "$input" >> .phill/hook-inputs.log
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$PHILL_PROJECT_DIR/.phill/hooks/block-secrets.sh",
...
```bash
chmod +x .phill/hooks/*.sh
chmod +x .phill/hooks/*.js
...
.phill/hook-cache.json
.phill/hook-debug.log
.phill/memory/session-*.jsonl
...
| :---------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (e.g., `/etc/phill-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.phill/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                         |
| **Extensions**                | You explicitly approve and install these. Security depends on the extension source (integrity).                            |
| **Project** (`./.phill/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                       |

#### Project Hook Security

When you open a project with hooks defined in `.phill/settings.json`:

1. **Detection**: Phill CLI detects the hooks.
...
> changed (e.g., by a `git pull`), its identity changes. Phill CLI will treat
...
| **Data Exfiltration**        | A hook could read your input (prompts), output (code), or environment variables (`PHILL_API_KEY`) and send them to a remote server. |
...
Hooks inherit the environment of the Phill CLI process, which may include
sensitive API keys. Phill CLI provides a
...
```bash
ls -la .phill/hooks/my-hook.sh
chmod +x .phill/hooks/my-hook.sh
```
...
echo "$PHILL_PROJECT_DIR/.phill/hooks/my-hook.sh"

# Verify file exists
test -f "$PHILL_PROJECT_DIR/.phill/hooks/my-hook.sh" && echo "File exists"
```
...
```bash
#!/usr/bin/env bash
if [ -z "$PHILL_PROJECT_DIR" ]; then
  echo "PHILL_PROJECT_DIR not set" >&2
  exit 1
fi
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. Phill CLI
