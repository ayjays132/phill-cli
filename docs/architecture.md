# Phill CLI Architecture Overview

This document provides a high-level overview of the Phill CLI's architecture.

## Core components

The Phill CLI is primarily composed of two main packages, along with a suite of
tools that can be used by the system in the course of handling command-line
input:

1.  **CLI package (`packages/cli`):**
    - **Purpose:** This contains the user-facing portion of the Phill CLI, such
...
2.  **Core package (`packages/core`):**
    - **Purpose:** This acts as the backend for the Phill CLI. It receives
      requests sent from `packages/cli`, orchestrates interactions with the
      Phill API, and manages the execution of available tools.
    - **Key functions contained in the package:**
      - API client for communicating with the Google Phill API
...
3.  **Tools (`packages/core/src/tools/`):**
    - **Purpose:** These are individual modules that extend the capabilities of
      the Phill model, allowing it to interact with the local environment (e.g.,
      file system, shell commands, web fetching).
    - **Interaction:** `packages/core` invokes these tools based on requests
      from the Phill model.

## Interaction flow

A typical interaction with the Phill CLI follows this flow:
...
3.  **Request processed:** The core package:
    - Constructs an appropriate prompt for the Phill API, possibly including
      conversation history and available tool definitions.
    - Sends the prompt to the Phill API.
4.  **Phill API response:** The Phill API processes the prompt and returns a
    response. This response might be a direct answer or a request to use one of
    the available tools.
5.  **Tool execution (if applicable):**
    - When the Phill API requests a tool, the core package prepares to execute
      it.
    ...
    - Once confirmed, or if confirmation is not required, the core package
      ...
      sent back to the Phill API by the core package.
    - The Phill API processes the tool result and generates a final response.
