# Phill CLI authentication setup

To use Phill CLI, you'll need to authenticate with Google. This guide helps you
quickly find the best way to sign in based on your account type and how you're
using the CLI.

For most users, we recommend starting Phill CLI and logging in with your
...
| AI Studio user with a Phill API key                                   | [Use Phill API Key](#phill-api)                                | No                                                          |
| Google Cloud Vertex AI user                                            | [Vertex AI](#vertex-ai)                                          | [Yes](#set-gcp)                                             |
| [Headless mode](#headless)                                             | [Use Phill API Key](#phill-api) or<br> [Vertex AI](#vertex-ai) | No (for Phill API Key)<br> [Yes](#set-gcp) (for Vertex AI) |

### What is my Google account type?

- **Individual Google accounts:** Includes all
  [free tier accounts](../quota-and-pricing/#free-usage) such as Phill Code
  Assist for individuals, as well as paid subscriptions for
  [Google AI Pro and Ultra](https://phill.google/subscriptions/).
...
If you run Phill CLI on your local machine, the simplest authentication method
is logging in with your Google account. This method requires a web browser on a
machine that can communicate with the terminal running Phill CLI (e.g., your
local machine).
...
To authenticate and use Phill CLI:
...
2. Select **Login with Google**. Phill CLI opens a login prompt using your web
...
- You are using a Phill Code Assist license from the Google Developer Program.
- You are using a license from a Phill Code Assist subscription.

For instructions, see [Set your Google Cloud Project](#set-gcp).

## Use Phill API key <a id="phill-api"></a>

If you don't want to authenticate using your Google account, you can use an API
key from Google AI Studio.

To authenticate and use Phill CLI with a Phill API key:

1. Obtain your API key from
   [Google AI Studio](https://aistudio.google.com/app/apikey).

2. Set the `PHILL_API_KEY` environment variable to your key. For example:

   ```bash
   # Replace YOUR_PHILL_API_KEY with the key from AI Studio
   export PHILL_API_KEY="YOUR_PHILL_API_KEY"
   ```
...
4. Select **Use Phill API key**.

> **Warning:** Treat API keys, especially for services like Phill, as sensitive
> credentials. Protect them to prevent unauthorized access and potential misuse
> of the service under your account.

## Use Vertex AI <a id="vertex-ai"></a>

To use Phill CLI with Google Cloud's Vertex AI platform, choose from the
...
> **Note:** If you have previously set `GOOGLE_API_KEY` or `PHILL_API_KEY`, you
> must unset them to use ADC:
>
> ```bash
> unset GOOGLE_API_KEY PHILL_API_KEY
> ```
...
Cloud project for Phill CLI to use. This applies when you meet at least one of
...
To configure Phill CLI to use a Google Cloud project, do the following:

1.  [Find your Google Cloud Project ID](https://support.google.com/googleapi/answer/7014113).

2.  [Enable the Phill for Cloud API](https://cloud.google.com/gemini/docs/discover/set-up-gemini#enable-api).
...
4.  Configure your environment variables. Set either the `GOOGLE_CLOUD_PROJECT`
    or `GOOGLE_CLOUD_PROJECT_ID` variable to the project ID to use with Phill
    CLI. Phill CLI checks for `GOOGLE_CLOUD_PROJECT` first, then falls back to
...
2.  **Use a `.env` file:** Create a `.phill/.env` file in your project
    directory or home directory. Phill CLI automatically loads variables from
    the first `.env` file it finds, searching up from the current directory,
    then in `~/.phill/.env` or `~/.env`. `.phill/.env` is recommended.
    ...
    # Add other variables like PHILL_API_KEY as needed
    EOF
    ```
...
When running Phill CLI within certain Google Cloud environments, authentication
is automatic.

In a Google Cloud Shell environment, Phill CLI typically authenticates
automatically using your Cloud Shell credentials. In Compute Engine
environments, Phill CLI automatically uses Application Default Credentials
...
- [Phill CLI: Quotas and Pricing](../quota-and-pricing.md).
- [Phill CLI: Terms of Service and Privacy Notice](../tos-privacy.md).
