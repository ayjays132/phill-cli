...
Phill CLI offers several ways to configure its behavior, including environment
...
Phill CLI uses JSON settings files for persistent configuration. There are four
...
> `https://raw.githubusercontent.com/ayjays132/phill-cli/main/schemas/settings.schema.json`.

- **System defaults file:**
  - **Location:** `/etc/phill-cli/system-defaults.json` (Linux),
    `C:\ProgramData\phill-cli\system-defaults.json` (Windows) or
    `/Library/Application Support/PhillCli/system-defaults.json` (macOS). The
    path can be overridden using the `PHILL_CLI_SYSTEM_DEFAULTS_PATH`
    environment variable.
  - **Scope:** Provides a base layer of system-wide default settings. These
    settings have the lowest precedence and are intended to be overridden by
    user, project, or system override settings.
- **User settings file:**
  - **Location:** `~/.phill/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Phill CLI sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.phill/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Phill CLI from that specific project.
    Project settings override user settings and system defaults.
- **System settings file:**
  - **Location:** `/etc/phill-cli/settings.json` (Linux),
    `C:\ProgramData\phill-cli\settings.json` (Windows) or
    `/Library/Application Support/PhillCli/settings.json` (macOS). The path can
    be overridden using the `PHILL_CLI_SYSTEM_SETTINGS_PATH` environment
    variable.
  - **Scope:** Applies to all Phill CLI sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' Phill CLI setups.

**Note on environment variables in settings:** String values within your
`settings.json` and `phill-extension.json` files can reference environment
...
> **Note for Enterprise Users:** For guidance on deploying and managing Phill
> CLI in a corporate environment, please see the
> [Enterprise Configuration](../cli/enterprise.md) documentation.

### The `.phill` directory in your project

In addition to a project settings file, a project's `.phill` directory can
contain other project-specific files related to Phill CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g.,
  `.phill/sandbox-macos-custom.sb`, `.phill/sandbox.Dockerfile`).
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show Phill CLI model thoughts in the terminal window title
...
- **`ui.showHomeDirectoryWarning`** (boolean):
  - **Description:** Show a warning when running Phill CLI in the home
    directory.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (PHILL.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`ui.voice.enabled`** (boolean):
  - **Description:** Whether voice input mode is enabled by default.
  - **Default:** `false`

- **`ui.voice.ttsEnabled`** (boolean):
  - **Description:** Whether text-to-speech is enabled by default.
  - **Default:** `false`
...
- **`model.name`** (string):
  - **Description:** The Phill model to use for conversations.
  - **Default:** `undefined`
...
- **`huggingFace.endpoint`** (string):
  - **Description:** HuggingFace Inference API endpoint.
  - **Default:** `undefined`
  - **Requires restart:** Yes

- **`huggingFace.apiKey`** (string):
  - **Description:** HuggingFace API Key (optional for public models).
  - **Default:** `undefined`
...
- **`customApi.model`** (string):
  - **Description:** Model name to use (passed through exactly).
  - **Default:** `"custom-model"`
  - **Requires restart:** Yes

#### `browser`

- **`browser.headed`** (boolean):
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory refresh loads PHILL.md files. When
...
- **`context.fileFiltering.respectGeminiIgnore`** (boolean):
  - **Description:** Respect .phillignore files when searching.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`context.fileFiltering.customIgnoreFilePaths`** (array):
  - **Description:** Additional ignore file paths to respect. These files take
    precedence over .phillignore and .gitignore. Files earlier in the array take
    precedence over files later in the array, e.g. the first file takes
...
- **`security.autoApproveAllActionsForever`** (boolean):
  - **Description:** If enabled, Phill starts in full auto-approve mode (YOLO)
    for trusted folders. High risk: commands and tool actions can execute
    without prompts.
  - **Default:** `false`
...
discovering and using custom tools. Phill CLI attempts to connect to each
...
Configures logging and metrics collection for Phill CLI. For more information,
...
- **Location:** `~/.phill/tmp/<project_hash>/shell_history`
...
files to prevent interference with phill-cli behavior. Variables from
`.phill/.env` files are never excluded. You can customize this behavior using
the `advanced.excludedEnvVars` setting in your `settings.json` file.

- **`PHILL_API_KEY`**:
  - Your API key for the Phill API.
  - One of several available [authentication methods](./authentication.md).
  - Set this in your shell profile (e.g., `~/.bashrc`, `~/.zshrc`) or an `.env`
    file.
- **`PHILL_MODEL`**:
  - Specifies the default Phill model to use.
  - Overrides the hardcoded default
  - Example: `export PHILL_MODEL="gemini-3-flash-preview"`
- **`GOOGLE_API_KEY`**:
...
- **`PHILL_SYSTEM_MD`**:
  ...
  - `true`/`1`: Use project default path `./.phill/system.md`.
...
- **`PHILL_WRITE_SYSTEM_MD`**:
  - Writes the current built‑in system prompt to a file for review.
  - `true`/`1`: Write to `./.phill/system.md`. Otherwise treat the value as a
    path.
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create
    a file named `sandbox-macos-<profile_name>.sb` in your project's `.phill/`
    directory (e.g., `my-project/.phill/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI
  itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful
    for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env`
    files by default to prevent interference with phill-cli behavior. Use
    `.phill/.env` files if you need to set these for phill-cli specifically.
- **`NO_COLOR`**:
...
To prevent accidental leakage of sensitive information, Phill CLI automatically
...
- Common system variables (e.g., `PATH`, `HOME`, `USER`, `SHELL`, `TERM`,
  `LANG`).
- Variables starting with `PHILL_CLI_`.
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the Phill model to use for this session.
  - Example: `npm start -- --model gemini-3-pro-preview`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes Phill CLI in a
...
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  ...
  - Example: `phill -i "explain this code"`
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `phill --approval-mode auto_edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation
    dialog.
  - Example: `phill --allowed-tools "ShellTool(git status)"`
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  - Specifies a list of extensions to use for the session. If not provided, all
    available extensions are used.
  - Use the special term `phill -e none` to disable all extensions.
  - Example: `phill -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
- **`--resume [session_id]`** (**`-r [session_id]`**):
  ...
  - Example: `phill --resume 5` or `phill --resume latest` or
    `phill --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890` or `phill --resume`
...
- **`--delete-session <identifier>`**:
  ...
  - Use `--list-sessions` first to see available sessions, their indices, and
    UUIDs.
  - Example: `phill --delete-session 3` or
    `phill --delete-session a1b2c3d4-e5f6-7890-abcd-ef1234567890`
...
(defaulting to `PHILL.md` but configurable via the `context.fileName` setting)
are crucial for configuring the _instructional context_ (also referred to as
"memory") provided to the Phill model. This powerful feature allows you to give
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  that you want the Phill model to be aware of during your interactions. The
  system is designed to manage this instructional context hierarchically.

### Example context file content (e.g., `PHILL.md`)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (e.g., `PHILL.md`) from
  several locations. Content from files lower in this list (more specific)
  typically overrides or supplements content from files higher up (more
  general). The exact concatenation order and final context can be inspected
  using the `/memory show` command. The typical loading order is:
  1.  **Global context file:**
      - Location: `~/.phill/<configured-context-filename>` (e.g.,
        `~/.phill/PHILL.md` in your user home directory).
...
- **Concatenation and UI indication:** The contents of all found context files
  ...
  provided as part of the system prompt to the Phill model. The CLI footer
...
the Phill CLI's responses to your specific needs and projects.

## Sandboxing

The Phill CLI can execute potentially unsafe operations (like shell commands and
file modifications) within a sandboxed environment to protect your system.

Sandboxing is disabled by default, but you can enable it in a few ways:

- Using `--sandbox` or `-s` flag.
- Setting `PHILL_SANDBOX` environment variable.
- Sandbox is enabled when using `--yolo` or `--approval-mode=yolo` by default.

By default, it uses a pre-built `phill-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at
`.phill/sandbox.Dockerfile` in your project's root directory. This Dockerfile
can be based on the base sandbox image:

```dockerfile
FROM phill-cli-sandbox
...
When `.phill/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment
variable when running Phill CLI to automatically build the custom sandbox image:

```bash
BUILD_SANDBOX=1 phill -s
```

## Usage statistics

To help us improve the Phill CLI, we collect anonymized usage statistics. This
...
- **API requests:** We log the Phill model used for each request, the duration
...
- **Prompt and response content:** We do not log the content of your prompts or
  the responses from the Phill model.
