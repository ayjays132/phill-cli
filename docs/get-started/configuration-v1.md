...
- **`contextFileName`** (string or array of strings):
  - **Description:** Specifies the filename for context files (e.g.,
    `PHILL.md`, `AGENTS.md`). Can be a single filename or a list of accepted
    filenames.
  - **Default:** `PHILL.md`
...
- **`bugCommand`** (object):
  ...
  - **Default:**
    `"urlTemplate": "https://github.com/ayjays132/phill-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
1.  **Use `.phillignore`:** Create a `.phillignore` file in your project root
...
- **`coreTools`** (array of strings):
  - **Description:** Allows you to specify a list of core tool names that should
    ...
    be executed.
  - **Default:** All tools available for use by the Phill model.
...
- **`allowMCPServers`** (array of strings):
  ...
  - **Default:** All MCP servers are available for use by the Phill model.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](../cli/themes.md) for Phill CLI.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool
    execution. If set to `true`, Phill CLI uses a pre-built
    `phill-cli-sandbox` Docker image. For more information, see
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context
    Protocol (MCP) servers for discovering and using custom tools. Phill CLI
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Phill CLI.
...
- **`excludedProjectEnvVars`** (array of strings):
  - **Description:** Specifies environment variables that should be excluded
    ...
    environment variables (like `DEBUG=true`) from interfering with phill-cli
    behavior. Variables from `.phill/.env` files are never excluded.
...
- **`loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls the behavior of the `/memory refresh` command. If
    set to `true`, `PHILL.md` files should be loaded from all directories that
    are added. If set to `false`, `PHILL.md` should only be loaded from the
    current directory.
  - **Default:** `false`
  - **Example:**
    ```json
...
- **`PHILL_MODEL`**:
  ...
  - Example: `export PHILL_MODEL="gemini-2.5-flash"`
- **`GOOGLE_API_KEY`**:
...
(defaulting to `PHILL.md` but configurable via the `contextFileName` setting)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  ...
  1.  **Global context file:**
      - Location: `~/.phill/<contextFileName>` (e.g., `~/.phill/PHILL.md` in
...
The Phill CLI can execute potentially unsafe operations (like shell commands
...
When `.phill/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX`
environment variable when running Phill CLI to automatically build the custom
sandbox image:
