...
the Phill CLI. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y phill-cli@<tag> --version` must return the correct
...
- **Setup:**
  - [ ] Uninstall any existing global version: `npm uninstall -g phill-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g phill-cli@preview`
  - [ ] Verify version: `phill --version`
...
- **Basic prompting:**
  - [ ] Run `phill "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `phill`. Ask a follow-up question to test
        context.

- **Piped input:**
  - [ ] Run `echo "Summarize this" | phill` and verify it processes stdin.
...
- **Function calling:**
  - [ ] In interactive mode, ask phill to "create a file named hello.md with the
...
- [ ] Navigate to `go/phill-cli-offline-evals-dash`.
