...
- **Error:
  `You must be a named user on your organization's Phill Code Assist Standard edition subscription to use this service. Please contact your administrator to request an entitlement to Phill Code Assist Standard edition.`**
  - **Cause:** This error might occur if Phill CLI detects the
...
- **Error: Command not found (when attempting to run Phill CLI with `phill`).**
  - **Cause:** Phill CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed Phill CLI:
    - If you installed `phill` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update Phill CLI using the command
      `npm install -g phill-cli@latest`.
    - If you are running `phill` from source, ensure you are using the correct
      command to invoke it (e.g., `node packages/cli/dist/index.js ...`). To
      update Phill CLI, pull the latest changes from the repository, and then
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Phill CLI may attempt operations that
...
- **Phill CLI is not running in interactive mode in "CI" environments**
  - **Issue:** The Phill CLI does not enter interactive mode (no prompt appears)
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the Phill CLI from starting in its
    interactive mode.
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to
    ...
    `env -u CI_TOKEN phill`

- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for phill-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with phill-cli behavior.
  - **Solution:** Use a `.phill/.env` file instead, or configure the
    `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer
    variables.

## Exit codes

The Phill CLI uses specific exit codes to indicate the reason for termination.
...
guide_, consider searching the Phill CLI
[Issue tracker on GitHub](https://github.com/ayjays132/phill-cli/issues). If you
