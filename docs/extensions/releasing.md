...
can install your extension using `phill extensions install <your-repo-uri>`.
...
version in the `phill-extension.json` file.
...
way `phill extensions install <your-repo-uri> --ref=stable`. Or, you could make
...
`phill extensions install <your-repo-uri>` is to be on the stable release
branch.
...
Phill CLI extensions can be distributed through
...
When checking for updates, phill will just look for the "latest" release on
...
To ensure Phill CLI can automatically find the correct release asset for each
...
requirements - specifically the `phill-extension.json` file must be at the root
of the archive.
...
Phill CLI extension for multiple platforms:
