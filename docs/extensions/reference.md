...
This guide covers the `phill extensions` commands and the structure of the
`phill-extension.json` configuration file.

## Extension management

We offer a suite of extension management tools using `phill extensions`
commands.

Note that these commands (e.g. `phill extensions install`) are not supported
...
You can install an extension using `phill extensions install` with either a
GitHub URL or a local path.

Note that we create a copy of the installed extension, so you will need to run
`phill extensions update` to pull in changes from both locally-defined
...
```
phill extensions install <source> [--ref <ref>] [--auto-update] [--pre-release] [--consent]
```
...
`phill extensions uninstall <name...>`:

```
phill extensions uninstall phill-cli-security phill-cli-another-extension
```
...
```
phill extensions disable <name> [--scope <scope>]
```
...
You can enable extensions using `phill extensions enable <name>`. You can also
enable an extension for a specific workspace using
`phill extensions enable <name> --scope=workspace` from within that workspace.

```
phill extensions enable <name> [--scope <scope>]
```
...
`phill-extension.json` `version` field) with `phill extensions update <name>`.

You can update all extensions with:

```
phill extensions update --all
```
...
[here](https://github.com/ayjays132/phill-cli/tree/main/packages/cli/src/commands/extensions/examples).

To copy one of these examples into a development directory using the type of
your choosing, run:

```
phill extensions new <path> [template]
```
...
The `phill extensions link` command will create a symbolic link from the
extension installation directory to the development path.

This is useful so you don't have to run `phill extensions update` every time
you make changes you'd like to test.

```
phill extensions link <path>
```
...
On startup, Phill CLI looks for extensions in `<home>/.phill/extensions`

Extensions exist as a directory that contains a `phill-extension.json` file.
For example:

`<home>/.phill/extensions/my-extension/phill-extension.json`

### `phill-extension.json`

The `phill-extension.json` file contains the configuration for the extension.
...
- `description`: A short description of the extension. This will be displayed on
  [phillcli.com/extensions](https://phillcli.com/extensions).
...
- `contextFileName`: The name of the file that contains the context for the
  ...
  If this property is not used but a `PHILL.md` file is present in your
...
When Phill CLI starts, it loads all the extensions and merges their
...
To define settings, add a `settings` array to your `phill-extension.json` file.
...
(e.g., `<home>/.phill/extensions/my-api-extension/.env`).

You can view a list of an extension's settings by running:

```
phill extensions list
```

and you can update a given setting using:

```
phill extensions config <extension name> [setting name] [--scope <scope>]
```
...
Phill CLI behavior at specific lifecycle events. Hooks provided by an extension
must be defined in a `hooks/hooks.json` file within the extension directory.

> [!IMPORTANT] Hooks are not defined directly in `phill-extension.json`. The
...
Phill CLI will automatically discover and load these agents when the extension
...
Phill CLI extensions allow variable substitution in both
`phill-extension.json` and `hooks/hooks.json`. This can be useful if e.g., you
...
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `${extensionPath}`         | The fully-qualified path of the extension in the user's filesystem e.g., '/Users/username/.phill/extensions/example-extension'. This will not unwrap symlinks. |
