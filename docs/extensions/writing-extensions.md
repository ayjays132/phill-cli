# Getting started with Phill CLI extensions

This guide will walk you through creating your first Phill CLI extension.
You'll learn how to set up a new extension, add a custom tool via an MCP server,
create a custom command, and provide context to the model with a `PHILL.md`
file.

## Prerequisites

Before you start, make sure you have the Phill CLI installed and a basic
understanding of Node.js.

## When to use what

Extensions offer a variety of ways to customize Phill CLI.
...
```bash
phill extensions new my-first-extension mcp-server
```
...
This is the manifest file for your extension. It tells Phill CLI how to load
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP)
  ...
  - `command`, `args`, `cwd`: These fields specify how to start your server.
    Notice the use of the `${extensionPath}` variable, which Phill CLI replaces
...
Before you can use the extension, you need to link it to your Phill CLI
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the Phill CLI extensions
    ...
    ```bash
    phill extensions link .
    ```

Now, restart your Phill CLI session. The new `fetch_posts` tool will be
...
After saving the file, restart the Phill CLI. You can now run
`/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add a custom `PHILL.md`

You can provide persistent context to the model by adding a `PHILL.md` file to
...
1.  Create a file named `PHILL.md` in the root of your extension directory:
...
2.  Update your `phill-extension.json` to tell the CLI to load this file:
...
Restart the CLI again. The model will now have the context from your `PHILL.md`
...
procedural workflows. Unlike `PHILL.md`, which provides persistent context,
...
You've successfully created a Phill CLI extension! You learned how to:
...
capabilities into the Phill CLI.
