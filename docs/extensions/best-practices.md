# Extensions on Phill CLI: Best practices
...
Developing extensions for Phill CLI is intended to be a lightweight, iterative
process.
...
Use `phill extensions link` to develop locally without constantly reinstalling:
...
### Use `PHILL.md` effectively

Your `PHILL.md` file provides context to the model. Keep it focused:
...
When building a Phill CLI extension, follow general security best practices
...
them to specific commands in your `phill-extension.json`:
...
`phill-extension.json`. This ensures keys are stored securely in the system
...
Phill CLI extensions also offers support for more complicated
...
```bash
# Stable
phill extensions install github.com/user/repo

# Dev
phill extensions install github.com/user/repo --ref dev
```

### Clean artifacts

If you are using GitHub Releases, ensure your release artifacts only contain the
necessary files (`dist/`, `phill-extension.json`, `package.json`). Exclude
