...
import { PHILL_DIR } from 'phill-cli-core';
...
// Returns the home directory, respecting PHILL_CLI_HOME
const homedir = () => process.env['PHILL_CLI_HOME'] || os.homedir();

// User-level .phill directory in home
const USER_PHILL_DIR = path.join(homedir(), PHILL_DIR);
// Project-level .phill directory in the workspace
const WORKSPACE_PHILL_DIR = path.join(projectRoot, PHILL_DIR);

// Telemetry artifacts are stored in a hashed directory under the user's ~/.phill/tmp
export const OTEL_DIR = path.join(USER_PHILL_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .phill directory
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `phill-cli-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: phill-cli-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'phill-cli-telemetry-'),
  );
