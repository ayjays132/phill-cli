#!/bin/bash
# -----------------------------------------------------------------------------
# Phill API Replay Script
# -----------------------------------------------------------------------------
# Purpose:
#   This script is used to replay a Phill API request using a raw JSON payload.
#   It is particularly useful for debugging the exact requests made by the
#   Phill CLI.
#
# Prerequisites:
#   1. Export your Phill API key:
#      export PHILL_API_KEY="your_api_key_here"
#
#   2. Generate a request payload from the Phill CLI:
...
#
# Usage:
#   ./scripts/send_phill_request.sh --payload <path_to_json> --model <model_id> [--stream]
#
# Options:
#   --payload <file>  Path to the JSON request payload.
#   --model <id>      The Phill model ID (e.g., gemini-3-flash-preview).
#   --stream          (Optional) Use the streaming API endpoint. Defaults to non-streaming.
#
# Example:
#   ./scripts/send_phill_request.sh --payload gcli-request.json --model gemini-3-flash-preview
# -----------------------------------------------------------------------------
...
usage() {
    ...
    echo "Ensure PHILL_API_KEY environment variable is set."
    exit 1
}
...
if [[ -z "${PHILL_API_KEY}" ]]; then
    echo "Error: PHILL_API_KEY environment variable is not set."
    exit 1
fi
...
if [[ "${STREAM_MODE}" = false ]] && command -v jq &> /dev/null; then
    ...
    output=$(curl -s -X POST \
      ...
      "https://generativelanguage.googleapis.com/v1beta/models/${MODEL_ID}:${GENERATE_CONTENT_API}?key=${PHILL_API_KEY}" \
      -d "@${PAYLOAD_FILE}")
    echo "${output}" | jq .
else
    curl -X POST \
