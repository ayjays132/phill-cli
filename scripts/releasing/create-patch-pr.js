...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description:
        'fully qualified package name with scope (e.g phill-cli)',
      string: true,
      default: 'phill-cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "phill-cli-robot"', dryRun);
  run('git config user.email "phill-cli-robot@google.com"', dryRun);
