...
fs.writeFileSync(
  coreNpmrcPath,
  '@google-phill:registry=https://npm.pkg.github.com/',
);
console.log('Wrote .npmrc for @google-phill scope to packages/core/');

// Update phill-cli
updatePackageJson('packages/cli/package.json', (pkg) => {
  pkg.name = '@google-phill/phill-cli';
  pkg.files = ['bundle/'];
  pkg.bin = {
    phill: 'bundle/phill.js',
  };
...
// Update phill-cli-a2a-server
updatePackageJson('packages/a2a-server/package.json', (pkg) => {
  pkg.name = '@google-phill/phill-cli-a2a-server';
});

// Update phill-cli-core
updatePackageJson('packages/core/package.json', (pkg) => {
  pkg.name = '@google-phill/phill-cli-core';
});
