...
if (!image.length) {
  console.warn(
    'No default image tag specified in phill-cli/packages/cli/package.json',
  );
}
...
console.log('packing phill-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-phill-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w phill-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing phill-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-phill-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w phill-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `google-phill-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-phill-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.PHILL_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
