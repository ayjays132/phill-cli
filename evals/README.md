...
behavioral evals focus on specific, granular behaviors relevant to the Phill
CLI's features.
...
First, build the bundled Phill CLI. You must do this after every code change.
...
- **CI Evals**: Included in the
  [E2E (Chained)](https://github.com/ayjays132/phill-cli/actions/workflows/chained_e2e.yml)
  workflow. These must pass 100% for every PR.
- **Nightly Evals**: Run daily via the
  [Evals: Nightly](https://github.com/ayjays132/phill-cli/actions/workflows/evals-nightly.yml)
...
`/fix-behavioral-eval` command within Phill CLI to help investigate and fix the
issue.
...
2.  **Fix**: Suggest and apply targeted fixes to the prompt or tool definitions.
    ...
    the test itself.
3.  **Verify**: Re-run the test 3 times across multiple models (e.g., Phill
    3.0, Phill 3 Flash, Phill 2.5 Pro) to ensure stability and calculate a
    success rate.
...
```bash
phill /fix-behavioral-eval https://github.com/ayjays132/phill-cli/actions/runs/123456789
```

When investigating failures manually, you can also enable verbose agent logs by
setting the `PHILL_DEBUG_LOG_FILE` environment variable.
...
unable to accomplish the goal with positive traits. Phill is quite good at
