...
${renderCoherencyLayer(options.coherencyLayer)}
${renderVoiceStatus(options.voiceStatus)}
${renderBiologicalState(options.biologicalState)}
`.trim();
}
...
export function renderVoiceStatus(options?: VoiceStatusOptions): string {
  if (!options) return '';
  const status = [];
  if (options.enabled) status.push('Voice capture (STT) is ACTIVE');
  if (options.ttsEnabled) status.push('Text-to-Speech (TTS) is ACTIVE');

  if (status.length === 0 && !options.activePersona) return '';

  let personaPrompt = '';
  if (options.activePersona) {
    personaPrompt = `
## Active Vocal Persona: ${options.activePersona.name}
- You are currently using a persistent vocal persona.
- Parameters: ${JSON.stringify(options.activePersona)}
- You should maintain this persona's tone and style in all spoken responses until the mode is deactivated.
`;
  }
...
# Voice Mode Status
- ${status.join('\n- ')}${personaPrompt}
- Since you are in voice mode, you should use the \`vocal_prosody_controller\` tool to adjust your delivery (pitch, rate, style) to be more conversational and expressive where appropriate.
`.trim();
}

export function renderBiologicalState(
  options?: BiologicalStateOptions,
): string {
  if (!options) return '';
...
- **Call-to-Action Tokens:** When calling a tool that performs a significant action (e.g., writing a file, running a test, browsing a page), prefix your thought process with **[ACTION]** or **[THOUGHT]** inside your \`<thinking>\` block to clearly signal your intent.
...
export function renderCoherencyLayer(options?: CoherencyLayerOptions): string {
  if (!options) return '';
  if (!options.hasLatentContext && !options.hasPlanningLatches) return '';

  let prompt = '\n# Coherency Layer\n';
  if (options.hasPlanningLatches) {
    prompt +=
      '- **Planning Latches Detected:** You have active planning latches in your memory. These are critical architectural or strategic decisions that MUST be prioritized. If any current action contradicts a latch, you must stop and reconcile or explain why.\n';
  }
  if (options.hasLatentContext) {
    prompt +=
      '- **Latent Context Snapshots:** You have dense semantic snapshots in your history. These contain high-fidelity goals and discoveries from the past. Use them to maintain deep context even after history compression.\n';
  }
...
function mandateExplainBeforeActing(isPhill3: boolean): string {
  if (!isPhill3) return '';
  return `
...
function toneAndStyleNoChitchat(isPhill3: boolean): string {
  return isPhill3
    ? `
