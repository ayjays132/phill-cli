...
export class PromptProvider {
  ...
  getCoreSystemPrompt(
    ...
    if (systemMdResolution.value && !systemMdResolution.isDisabled) {
      let systemMdPath = path.resolve(path.join(PHILL_DIR, 'system.md'));
      ...
      const options: snippets.SystemPromptOptions = {
        ...
        coherencyLayer: this.withSection('coherencyLayer', () => ({
          hasLatentContext: !!userMemory?.includes('[LATENT_SNAPSHOT'),
          hasPlanningLatches: !!userMemory?.includes('[LATCH]'),
        })),
        voiceStatus: this.withSection('voiceStatus', () => ({
          enabled: config.getVoice()?.enabled ?? false,
          ttsEnabled: config.getVoice()?.ttsEnabled ?? false,
          activePersona: config.getVoice()?.activePersona,
        })),
        biologicalState: this.withSection('biologicalState', () => {
          const drives = config.getBiologicalDrives();
          return {
            dopamine_level: drives.dopamine_level,
            boredom_level: drives.boredom_level,
            prime_directive: drives.prime_directive,
          };
        }),
      };
    ...
    this.maybeWriteSystemMd(
      sanitizedPrompt,
      systemMdResolution,
      path.resolve(path.join(PHILL_DIR, 'system.md')),
    );
  ...
  private maybeWriteSystemMd(
    ...
    const writeSystemMdResolution = resolvePathFromEnv(
      process.env['PHILL_WRITE_SYSTEM_MD'],
    );
