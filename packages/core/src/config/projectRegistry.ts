...
export class ProjectRegistry {
  ...
  private async withRegistryLock<T>(fn: () => Promise<T>): Promise<T> {
    // Keep the replacement registry dependency-free for this repo. The atomic
    // write path in save() handles most local races; this in-process queue
    // prevents concurrent updates from this CLI instance.
    return fn();
  }
