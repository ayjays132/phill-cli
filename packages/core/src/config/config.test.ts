...
import { setPhillMdFilename as mockSetPhillMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryRecallTool', () => ({
  MemoryRecallTool: vi.fn().mockImplementation(() => ({
    name: 'recall_memory',
    execute: vi.fn(),
  })),
  MemoryIngestTool: vi.fn().mockImplementation(() => ({
    name: 'ingest_memory',
    execute: vi.fn(),
  })),
}));
vi.mock('../tools/memoryTool', () => ({
  MemoryTool: vi.fn().mockImplementation(() => ({
    name: 'save_memory',
    execute: vi.fn(),
  })),
  PurgeMemoryTool: vi.fn().mockImplementation(() => ({
    name: 'purge_memory',
    execute: vi.fn(),
  })),
  setPhillMdFilename: vi.fn(),
  getCurrentPhillMdFilename: vi.fn(() => 'PHILL.md'), // Mock the original filename
  getGlobalMemoryFilePath: vi.fn(() => '/global/.phill/PHILL.md'),
  computeNewContent: vi.fn((current) => current + '\nMocked Fact'),
  DEFAULT_CONTEXT_FILENAME: 'PHILL.md',
  PHILL_DIR: '.phill',
  VITALS_SECTION_HEADER: '## Vitals',
  USER_IDENTITY_SECTION_HEADER: '## User Identity & Life Goals',
  PLANNING_LATCH_SECTION_HEADER: '## Planning Latches',
  LATENT_SNAPSHOT_SECTION_HEADER: '## Latent Snapshots',
  MEMORY_SECTION_HEADER: '## Phill Added Memories',
}));

vi.mock('../core/contentGenerator.js');

vi.mock('../core/client.js', () => ({
  PhillClient: vi.fn().mockImplementation(() => ({
...
describe('Server Config (config.ts)', () => {
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      expect(PhillClient).toHaveBeenCalledWith(config);
    });
    ...
    it('should strip thoughts when switching from GenAI to Vertex', async () => {
      ...
      expect(
        config.getPhillClient().stripThoughtsFromHistory,
  ...
  it('Config constructor should call setPhillMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetPhillMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setPhillMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetPhillMdFilename).not.toHaveBeenCalled();
  });
  ...
  describe('createToolRegistry', () => {
    ...
    it('should not register subagents as tools when agents are disabled', async () => {
      ...
      const { AgentRegistry: AgentRegistryMock } = (await vi.importMock(
        '../agents/registry.js',
      )) as { AgentRegistry: Mock };
      AgentRegistryMock.prototype.getAllDefinitions.mockReturnValue([]);
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockContextManager = {
      ...
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/PHILL.md'])),
    };
  ...
  it('should initialize ContextManager, load memory, and delegate to it when experimentalJitContext is enabled', async () => {
    ...
    expect(config.getPhillMdFileCount()).toBe(1);
    expect(config.getPhillMdFilePaths()).toEqual(['/path/to/PHILL.md']);
  });
