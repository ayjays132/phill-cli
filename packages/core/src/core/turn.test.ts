...
import type {
  ServerPhillToolCallRequestEvent,
  ServerPhillErrorEvent,
} from './turn.js';
import { Turn, PhillEventType } from './turn.js';
import type { GenerateContentResponse, Part, Content } from '@google/genai';
import { reportError } from '../utils/errorReporting.js';
import type { PhillChat } from './phillChat.js';
import { InvalidStreamError, StreamEventType } from './phillChat.js';
...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as PhillChat, 'prompt-id-1');
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerPhillToolCallRequestEvent;
      expect(event1.type).toBe(PhillEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerPhillToolCallRequestEvent;
      expect(event2.type).toBe(PhillEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: PhillEventType.Content, value: 'First part' },
    ...
    it('should yield InvalidStream event if sendMessageStream throws InvalidStreamError', async () => {
      ...
      expect(events).toEqual([{ type: PhillEventType.InvalidStream }]);
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerPhillErrorEvent;
      expect(errorEvent.type).toBe(PhillEventType.Error);
      ...
      expect(reportError).toHaveBeenCalledWith(
        error,
        'Error when talking to Phill API',
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerPhillToolCallRequestEvent;
    ...
    it.each([
      ...
      expect(events).toEqual([
        { type: PhillEventType.Content, value: contentText },
        {
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: PhillEventType.Content, value: 'First part' },
        { type: PhillEventType.Content, value: 'Second part' },
        {
          type: PhillEventType.Finished,
    ...
    it('should yield citation and finished events when response has citationMetadata', async () => {
      ...
      expect(events).toEqual([
        { type: PhillEventType.Content, value: 'Some text.' },
        {
    ...
    it('should not yield citation event if there is no finish reason', async () => {
      ...
      expect(events.some((e) => e.type === PhillEventType.Citation)).toBe(
        false,
      );
    });
    ...
    it('should not crash when cancelled request has malformed error', async () => {
      ...
      expect(events).toEqual([{ type: PhillEventType.UserCancelled }]);
    ...
    it('should yield a Retry event when it receives one from the chat stream', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: PhillEventType.Content, value: 'Success' },
      ]);
    });
    ...
    it('should process all parts when thought is first part in chunk', async () => {
      ...
      const thoughtEvent = events.find(
        (e) => e.type === PhillEventType.Thought,
      );
      ...
      const contentEvent = events.find(
        (e) => e.type === PhillEventType.Content,
      );
      ...
      const toolCallEvent = events.find(
        (e) => e.type === PhillEventType.ToolCallRequest,
      );
      ...
      const citationEvent = events.find(
        (e) => e.type === PhillEventType.Citation,
      );
      ...
      const finishedEvent = events.find(
        (e) => e.type === PhillEventType.Finished,
      );
