...
import { partListUnionToString } from './phillRequest.js';
...
export class PhillChat {
  ...
  async sendMessageStream(
    modelConfigKey: ModelConfigKey | string,
    ...
    // Reset turn-based availability state (clears sticky retires from prior turns)
    this.config.getModelAvailabilityService().resetTurn();
    ...
    const normalizedModelConfigKey: ModelConfigKey =
      typeof modelConfigKey === 'string'
        ? { model: modelConfigKey }
        : modelConfigKey;

    const userContent = createUserContent(finalMessage as PartListUnion);
    const model = normalizedModelConfigKey.model || this.config.getModel();
    ...
    return streamWithRetries.call(this, tempSystemInstruction);
  }
  ...
  private async makeApiCallAndProcessStream(
    ...
    const apiCall = async () => {
      ...
      const config: GenerateContentConfig = {
        ...
        // passed via config.
        systemInstruction: turnSystemInstruction ?? this.systemInstruction,
        tools: this.tools,
        abortSignal,
      };
  ...
  private async *processStreamResponse(
    ...
    if (responseText) {
      ...
      // Molt-Guard: Hallucination Check (Autonomous self-correction trigger)
      const guard = EthicalGuardService.getInstance();
   ...
   * This is called by external components when tool calls complete, before sending responses to Phill.
   */
