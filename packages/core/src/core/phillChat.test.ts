...
vi.mock('./prompts.js', () => ({
  getCoreSystemPrompt: vi.fn().mockReturnValue(''),
  getCompressionPrompt: vi.fn().mockReturnValue(''),
}));
...
vi.mock('./ttcEngine.js', () => ({
  TTCEngine: {
    getInstance: vi.fn(() => ({
      getGuidingContext: async () => '',
    })),
  },
}));
...
describe('PhillChat', () => {
    ...
    mockConfig = {
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((modelConfigKey) => {
          ...
          const thinkingConfig = model.startsWith('phill-3')
            ? {
  ...
  describe('constructor', () => {
    it('should initialize lastPromptTokenCount based on history size', () => {
      ...
      const chatWithHistory = new PhillChat(mockConfig, '', [], history);
    ...
    it('should initialize lastPromptTokenCount for empty history', () => {
      const chatEmpty = new PhillChat(mockConfig);
  ...
  describe('setHistory', () => {
    it('should recalculate lastPromptTokenCount when history is updated', () => {
      ...
      const chatWithHistory = new PhillChat(
        mockConfig,
        '',
        [],
        initialHistory,
  ...
  describe('sendMessageStream', () => {
    ...
    it('should use thinkingLevel and remove thinkingBudget for phill-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
      ...
      const stream = await chat.sendMessageStream(
        { model: 'phill-3-test-only-model-string-for-testing' },
        'hello',
    ...
    it('should use thinkingBudget and remove thinkingLevel for non-phill-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
  ...
  describe('sendMessageStream with retries', () => {
    it('should not retry on invalid content if model does not start with phill-2', async () => {
  ...
  describe('ensureActiveLoopHasThoughtSignatures', () => {
    it('should add thoughtSignature to the first functionCall in each model turn of the active loop', () => {
      const chat = new PhillChat(mockConfig, '', [], []);
      const history: Content[] = [
