...
import { InvalidStreamError } from './phillChat.js';
...
export type ServerPhillStreamEvent =
  ...
  | ServerPhillCitationEvent
  | ServerPhillContentEvent
  | ServerPhillErrorEvent
  | ServerPhillFinishedEvent
  | ServerPhillLoopDetectedEvent
  | ServerPhillMaxSessionTurnsEvent
  | ServerPhillThoughtEvent
  | ServerPhillToolCallConfirmationEvent
  | ServerPhillToolCallRequestEvent
  | ServerPhillToolCallResponseEvent
  | ServerPhillUserCancelledEvent
  | ServerPhillRetryEvent
  | ServerPhillContextWindowWillOverflowEvent
  | ServerPhillInvalidStreamEvent
  | ServerPhillModelInfoEvent
  | ServerPhillAgentExecutionStoppedEvent
...
export class Turn {
  ...
  constructor(
    private readonly chat: PhillChat,
  ...
  async *run(
    ...
    try {
      ...
      for await (const streamEvent of responseStream) {
        if (signal?.aborted) {
          yield { type: PhillEventType.UserCancelled };
          return;
        }

        // Handle the new RETRY event
        if (streamEvent.type === 'retry') {
          yield { type: PhillEventType.Retry };
        ...
        if (text) {
          yield { type: PhillEventType.Content, value: text, traceId };
        }
      ...
      yield { type: PhillEventType.Error, value: { error: structuredError } };
      return;
    }
  }
  ...
  private handlePendingFunctionCall(
    ...
    return { type: PhillEventType.ToolCallRequest, value: toolCallRequest };
  }
