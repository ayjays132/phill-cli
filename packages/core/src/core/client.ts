...
import type { ServerPhillStreamEvent, ChatCompressionInfo } from './turn.js';
...
export class PhillClient {
  private chat?: PhillChat;
  ...
  async startChat(
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing Phill chat session.',
        history,
        'startChat',
      );
  ...
  private async *processTurn(
  ...
  ): AsyncGenerator<ServerPhillStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: PhillEventType.MaxSessionTurns };
      return turn;
    }
    ...
    if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
      yield { type: PhillEventType.ChatCompressed, value: compressed };
    }
    ...
    // waiting for a response. The Phill API requires that a functionResponse
    ...
    if (loopDetected) {
      yield { type: PhillEventType.LoopDetected };
      return turn;
    }
    ...
    if (!signal.aborted && !this.currentSequenceModel) {
      yield { type: PhillEventType.ModelInfo, value: modelToUse };
    }
    ...
    for await (const event of resultStream) {
      ...
      if (event.type === PhillEventType.InvalidStream) {
        isInvalidStream = true;
      }
      if (event.type === PhillEventType.Error) {
        isError = true;
      }
    }
  ...
  async *sendMessageStream(
    ...
    if (hooksEnabled && messageBus) {
      ...
      if (hookResult) {
        if (
          'type' in hookResult &&
          hookResult.type === PhillEventType.AgentExecutionStopped
        ) {
          ...
          'type' in hookResult &&
          hookResult.type === PhillEventType.AgentExecutionBlocked
        ) {
          yield hookResult;
          return new Turn(this.getChat(), prompt_id);
