...
export class Logger {
  private phillDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.phillDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.phillDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.phillDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.phillDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.phillDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.phillDir) {
      debugLogger.error(
