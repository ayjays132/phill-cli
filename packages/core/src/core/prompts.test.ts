...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('PHILL_SYSTEM_MD', undefined);
    vi.stubEnv('PHILL_WRITE_SYSTEM_MD', undefined);
    mockConfig = {
  ...
  describe('PHILL_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when PHILL_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('PHILL_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if PHILL_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('PHILL_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when PHILL_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(PHILL_DIR, 'system.md'));
        vi.stubEnv('PHILL_SYSTEM_MD', value);
    ...
    it('should read from custom path when PHILL_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when PHILL_SYSTEM_MD is set', () => {
  ...
  describe('PHILL_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when PHILL_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('PHILL_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when PHILL_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(PHILL_DIR, 'system.md'));
        vi.stubEnv('PHILL_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when PHILL_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('PHILL_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it.each([
      ...
      'should expand tilde in custom path when PHILL_WRITE_SYSTEM_MD is "%s"',
