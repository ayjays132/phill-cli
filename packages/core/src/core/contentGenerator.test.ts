...
describe('createContentGenerator', () => {
  ...
  it('should create a GoogleGenAI content generator', async () => {
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith({
      ...
      httpOptions: {
        headers: {
          'User-Agent': expect.stringContaining('PhillCLI/1.2.3/gemini-pro'),
          'x-phill-api-privileged-user-id': expect.any(String),
        },
      },
    });
    expect(generator).toEqual(
  ...
  it('should include custom headers from PHILL_CLI_CUSTOM_HEADERS for Code Assist requests', async () => {
    const mockGenerator = {} as unknown as ContentGenerator;
    vi.mocked(createCodeAssistContentGenerator).mockResolvedValue(
      mockGenerator as never,
    );
    vi.stubEnv(
      'PHILL_CLI_CUSTOM_HEADERS',
  ...
  it('should include custom headers from PHILL_CLI_CUSTOM_HEADERS for GoogleGenAI requests without inferring auth mechanism', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
  ...
  it('should pass api key as Authorization Header when PHILL_API_KEY_AUTH_MECHANISM is set to bearer', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('PHILL_API_KEY_AUTH_MECHANISM', 'bearer');
  ...
  it('should not pass api key as Authorization Header when PHILL_API_KEY_AUTH_MECHANISM is not set (default behavior)', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    // PHILL_API_KEY_AUTH_MECHANISM is not stubbed, so it will be undefined, triggering default 'x-goog-api-key'
...
describe('createContentGeneratorConfig', () => {
  ...
  it('should configure for Phill using PHILL_API_KEY when set', async () => {
  ...
  it('should not configure for Phill if PHILL_API_KEY is empty', async () => {
  ...
  it('should not configure for Phill if PHILL_API_KEY is not set and storage is empty', async () => {
    vi.stubEnv('PHILL_API_KEY', '');
