...
import { PhillClient } from './client.js';
import {
  AuthType,
...
describe('Phill Client (client.ts)', () => {
  let mockContentGenerator: ContentGenerator;
  let mockConfig: Config;
  let client: PhillClient;
  ...
  beforeEach(async () => {
    ...
    // Because the PhillClient constructor kicks off an async process (startChat)
    ...
    client = new PhillClient(mockConfig);
    await client.initialize();
    vi.mocked(mockConfig.getPhillClient).mockReturnValue(client);
  ...
  describe('startChat', () => {
    it('should include environment context when resuming a session', async () => {
      ...
      expect(history[0].parts?.[0]?.text).toContain('This is the Phill CLI');
  ...
  describe('tryCompressChat', () => {
    ...
    function setup({
      ...
      const mockOriginalChat: Partial<PhillChat> = {
      ...
      client['chat'] = mockOriginalChat as PhillChat;
      ...
      const mockNewChat: Partial<PhillChat> = {
      ...
      client['startChat'] = vi
        .fn()
        .mockResolvedValue(mockNewChat as PhillChat);
...
\`\`\`
    ...
    it('should not add context if ideMode is enabled but no open files', async () => {
      ...
      const mockChat: Partial<PhillChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
        getLastPromptTokenCount: vi.fn(),
      };
      client['chat'] = mockChat as PhillChat;
...
\`\`\`
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: PhillEventType.MaxSessionTurns }]);
    ...
    it('should attempt compression before overflow check and proceed if compression frees space', async () => {
      ...
      // Use the real PhillChat to manage state and token counts more realistically
    ...
    it('should handle massive function responses by truncating them and then yielding overflow warning', async () => {
      ...
      const realChat = new PhillChat(mockConfig, '', [], history);
    ...
    it('should recursively call sendMessageStream with "Please continue." when InvalidStream event is received', async () => {
      ...
      const mockStream1 = (async function* () {
        yield { type: PhillEventType.InvalidStream };
      })();
      const mockStream2 = (async function* () {
        yield { type: PhillEventType.Content, value: 'Continued content' };
      })();
      ...
      expect(events).toEqual([
        { type: PhillEventType.ModelInfo, value: 'default-routed-model' },
        { type: PhillEventType.InvalidStream },
        { type: PhillEventType.Content, value: 'Continued content' },
      ]);
    ...
    it('should stop recursing after one retry when InvalidStream events are repeatedly received', async () => {
      ...
      expect(
        events
          .filter((e) => e.type === PhillEventType.ModelInfo)
          .map((e) => e.value),
    ...
    describe('IDE context with pending tool calls', () => {
      let mockChat: Partial<PhillChat>;
    ...
    it('should not call checkNextSpeaker when turn.run() yields a value then an error', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: PhillEventType.Content, value: 'some content' };
        yield {
          type: PhillEventType.Error,
          value: { error: { message: 'test error' } },
        };
    ...
    it('should abort linked signal when loop is detected', async () => {
      ...
      // Assert
      expect(events).toContainEqual({ type: PhillEventType.LoopDetected });
    ...
    describe('Hook System', () => {
      ...
      it('should fire BeforeAgent and AfterAgent exactly once for a simple turn', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: PhillEventType.Content, value: 'Hook Response' };
        });
      ...
      it('should fire BeforeAgent once and AfterAgent once even with recursion', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: PhillEventType.Content, value: response };
        });
      ...
      it('should use original request in AfterAgent hook even when continuation happened', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: PhillEventType.Content, value: 'Ok' };
        });
      ...
      it('should stop execution in AfterAgent when hook returns continue: false', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* () {
          yield { type: PhillEventType.Content, value: 'Hello' };
        });
