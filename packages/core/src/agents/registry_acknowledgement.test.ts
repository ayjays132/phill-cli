...
describe('AgentRegistry Acknowledgement', () => {
  ...
  let config: Config;
  let tempDir: string;
  let originalPhillCliHome: string | undefined;
  let ackService: AcknowledgedAgentsService;

  beforeEach(async () => {
    // Create a unique temp directory for each test
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'phill-cli-test-'));

    // Override PHILL_CLI_HOME to point to the temp directory
    originalPhillCliHome = process.env['PHILL_CLI_HOME'];
    process.env['PHILL_CLI_HOME'] = tempDir;
  ...
  afterEach(async () => {
    ...
    if (originalPhillCliHome) {
      process.env['PHILL_CLI_HOME'] = originalPhillCliHome;
    } else {
      delete process.env['PHILL_CLI_HOME'];
    }
