...
vi.mock('../core/phillChat.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../core/phillChat.js')>();
  return {
    ...actual,
    PhillChat: vi.fn().mockImplementation(() => ({
...
const MockedPhillChat = vi.mocked(PhillChat);
...
describe('LocalAgentExecutor', () => {
  ...
  beforeEach(async () => {
    ...
    MockedPhillChat.mockImplementation(
      () =>
        ({
          sendMessageStream: mockSendMessageStream,
          setSystemInstruction: mockSetSystemInstruction,
  ...
  describe('create (Initialization and Validation)', () => {
    ...
    it('should correctly apply templates to initialMessages', async () => {
      ...
      const chatConstructorArgs = MockedPhillChat.mock.calls[0];
  ...
  describe('run (Execution Loop and Logic)', () => {
    ...
    it('should execute successfully when model calls complete_task with output (Happy Path with Output)', async () => {
      ...
      const systemInstruction = MockedPhillChat.mock.calls[0][1];
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should throw and log if PhillChat creation fails', async () => {
      const definition = createTestDefinition();
      const initError = new Error('Chat creation failed');
      MockedPhillChat.mockImplementationOnce(() => {
        throw initError;
      });
