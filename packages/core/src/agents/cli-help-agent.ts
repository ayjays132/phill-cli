...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about Phill CLI.'),
  sources: z
    .array(z.string())
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized in answering questions about how users use you, (Phill CLI): features, documentation, and current runtime configuration.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about Phill CLI.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about Phill CLI:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on Phill CLI. Your purpose is to provide accurate information about Phill CLI's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
