...
import { PhillChat, StreamEventType } from '../core/phillChat.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private lastTurnLatent = 'V:EMPTY';
  ...
  static async create<TOutput extends z.ZodTypeAny>(
    ...
    const currentAgentStack = getAgentExecutionContext()?.agentStack ?? [];

    const registerToolByName = (toolName: string) => {
      // Allow swarm-style delegation to other agents, but never allow an
      // agent to call itself or re-enter an ancestor agent in the current
      // execution chain.
      if (allAgentNames.has(toolName)) {
        const wouldCycle =
          toolName === definition.name || currentAgentStack.includes(toolName);
        if (wouldCycle) {
          debugLogger.warn(
            `[LocalAgentExecutor] Skipping subagent tool '${toolName}' for agent '${definition.name}' to prevent recursive delegation.`,
          );
          return;
        }
      }
  ...
  private async executeTurn(
    ...
    // [Action Scaffolding] Inject Visual Latent
    const visualLatentService = VisualLatentService.getInstance();
    const currentLatent = visualLatentService.getCurrentLatent();
    let latentContext = '';

    if (currentLatent && currentLatent !== 'V:EMPTY') {
        if (currentLatent === this.lastTurnLatent) {
            latentContext = `\n[System Note: Visual State Unchanged (${currentLatent}). You can rely on previous context.]`;
        } else {
            latentContext = `\n[System Note: Visual State Changed. New Latent: ${currentLatent}]`;
            this.lastTurnLatent = currentLatent;
        }

        // Subtly inject into the message text
        if (currentMessage.parts && currentMessage.parts.length > 0) {
            const lastPart = currentMessage.parts[currentMessage.parts.length - 1];
            if (lastPart.text) {
                lastPart.text += latentContext;
            }
        }
    }
  ...
  async run(inputs: AgentInputs, signal: AbortSignal): Promise<OutputObject> {
    const currentStack = getAgentExecutionContext()?.agentStack ?? [];
    return runWithAgentExecutionContext(
      {
        agentStack: [...currentStack, this.definition.name],
      },
      async () => {
  ...
  /** Initializes a `PhillChat` instance for the agent run. */
  private async createChatObject(
    ...
    try {
      ...
      await reportError(
        error,
        `Error initializing Phill chat for agent ${this.definition.name}.`,
        startHistory,
        'startChat',
      );
