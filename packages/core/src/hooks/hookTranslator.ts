...
/**
 * Decoupled LLM request format - stable across Phill CLI versions
 */
...
/**
 * Decoupled LLM response format - stable across Phill CLI versions
 */
...
/**
 * Decoupled tool configuration - stable across Phill CLI versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_PHILL_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
