...
export class HookRegistry {
  ...
  /**
   * Update enabled state of all hooks based on the disabled hooks list
   */
  updateDisabledHooks(disabledHooks: string[]): void {
    for (const entry of this.entries) {
      const hookName = this.getHookName(entry);
      // If the hook is in the disabled list, enable=false. Otherwise enable=true (default state).
      // Note: This overrides individual enable/disable calls if they conflict with the list? 
      // Actually, if a user manually disabled a hook, and then we update the list, should it be re-enabled?
      // "Disabled Hooks" list typically comes from settings. So yes, it defines the ground truth of what is disabled via settings.
      entry.enabled = !disabledHooks.includes(hookName);
    }
    debugLogger.debug(`Updated disabled hooks list: ${disabledHooks.join(', ')}`);
  }
...
please review the project settings (.phill/settings.json) and remove them.`;
