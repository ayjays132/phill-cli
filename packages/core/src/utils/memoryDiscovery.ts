...
import { getAllPhillMdFilenames } from '../tools/memoryTool.js';
import type { FileDiscoveryService } from '../services/fileDiscoveryService.js';
import { processImports } from './memoryImportProcessor.js';
import type { FileFilteringOptions } from '../config/constants.js';
import { DEFAULT_MEMORY_FILE_FILTERING_OPTIONS } from '../config/constants.js';
import { PHILL_DIR, homedir } from './paths.js';
...
export interface PhillFileContent {
  filePath: string;
  content: string | null;
}
...
async function getPhillMdFilePathsInternal(
  currentWorkingDirectory: string,
  includeDirectoriesToReadPhill: readonly string[],
  userHomePath: string,
  debugMode: boolean,
  ...
  for (let i = 0; i < dirsArray.length; i += CONCURRENT_LIMIT) {
    ...
    const batchPromises = batch.map((dir) =>
      getPhillMdFilePathsInternalForEachDir(
        dir,
        userHomePath,
        debugMode,
        fileService,
...
async function getPhillMdFilePathsInternalForEachDir(
  dir: string,
  userHomePath: string,
  debugMode: boolean,
  fileService: FileDiscoveryService,
  ...
  const phillMdFilenames = getAllPhillMdFilenames();

  for (const phillMdFilename of phillMdFilenames) {
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${phillMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
    ...
    if (dir && folderTrust) {
      ...
      if (debugMode)
        logger.debug(
          `Searching for ${phillMdFilename} starting from CWD: ${resolvedCwd}`,
        );
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, PHILL_DIR)) {
          break;
        }

        const potentialPath = path.join(currentDir, phillMdFilename);
        try {
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllPhillMdFilenames()} paths to read: ${JSON.stringify(
        finalPaths,
      )}`,
    );
  return finalPaths;
...
async function readPhillMdFiles(
  filePaths: string[],
  debugMode: boolean,
  importFormat: 'flat' | 'tree' = 'tree',
): Promise<PhillFileContent[]> {
  ...
  const results: PhillFileContent[] = [];

  for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
    const batch = filePaths.slice(i, i + CONCURRENT_LIMIT);
    const batchPromises = batch.map(
      async (filePath): Promise<PhillFileContent> => {
        try {
          ...
          if (!isTestEnv) {
            ...
            logger.warn(
              `Warning: Could not read ${getAllPhillMdFilenames()} file at ${filePath}. Error: ${message}`,
            );
          }
...
export function concatenateInstructions(
  instructionContents: PhillFileContent[],
...
export async function loadGlobalMemory(
  ...
  const accessChecks = phillMdFilenames.map(async (filename) => {
    const globalPath = path.join(userHome, PHILL_DIR, filename);
    try {
  ...
  const contents = await readPhillMdFiles(foundPaths, debugMode, 'tree');
...
/**
 * Traverses upward from startDir to stopDir, finding all PHILL.md variants.
 *
 * Files are ordered by directory level (root to leaf), with all filename
 * variants grouped together per directory.
 */
async function findUpwardPhillFiles(
  startDir: string,
  stopDir: string,
  debugMode: boolean,
): Promise<string[]> {
  ...
  const phillMdFilenames = getAllPhillMdFilenames();
  const globalPhillDir = path.join(homedir(), PHILL_DIR);
...
export async function loadEnvironmentMemory(
  ...
  const traversalPromises = trustedRoots.map(async (root) => {
    ...
    return findUpwardPhillFiles(resolvedRoot, resolvedRoot, debugMode);
  });
  ...
  const contents = await readPhillMdFiles(sortedPaths, debugMode, 'tree');
...
/**
 * Loads hierarchical PHILL.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  const filePaths = await getPhillMdFilePathsInternal(
    currentWorkingDirectory,
    includeDirectoriesToReadPhill,
    userHomePath,
    debugMode,
  ...
  if (filePaths.length === 0) {
    if (debugMode)
      logger.debug('No PHILL.md files found in hierarchy of the workspace.');
    return { memoryContent: '', fileCount: 0, filePaths: [] };
  }
  const contentsWithPaths = await readPhillMdFiles(
    filePaths,
    debugMode,
    importFormat,
  );
 ...
 *
 * Returns the result of the call to `loadHierarchicalPhillMemory`.
 */
export async function refreshServerHierarchicalMemory(config: Config) {
  ...
  config.setPhillMdFileCount(result.fileCount);
  config.setPhillMdFilePaths(result.filePaths);
...
export async function loadJitSubdirectoryMemory(
  ...
  const potentialPaths = await findUpwardPhillFiles(
    resolvedTarget,
    bestRoot,
    debugMode,
  );
  ...
  const contents = await readPhillMdFiles(newPaths, debugMode, 'tree');
