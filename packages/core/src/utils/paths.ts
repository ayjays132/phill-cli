 ...
 * If PHILL_CLI_HOME environment variable is set, it returns its value.
 * Otherwise, it returns the user's home directory.
 */
export function homedir(): string {
  const envHome = process.env['PHILL_CLI_HOME'];
  if (envHome) {
    return envHome;
  }
  return os.homedir();
 ...
 * A project root is defined as a directory containing a `package.json` file
 * with a "workspaces" field, indicating a monorepo setup.
 * If no such file is found, the starting directory is considered the root.
 *
...
export function findProjectRoot(startDir: string): string {
  ...
  while (true) {
    ...
    if (parentDir === currentDir) {
      // Reached file system root, no monorepo package.json found
      break;
    }
    currentDir = parentDir;
  }
  return path.resolve(startDir); // Default to startDir if no monorepo root is found
}
