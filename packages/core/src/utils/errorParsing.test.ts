...
describe('parseAndFormatApiError', () => {
  ...
  const phillMessage = 'request a quota increase through AI Studio';
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "Phill 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/gemini-api/docs/rate-limits.",
    ...
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_PHILL);
    expect(result).toContain('Phill 2.5 Pro Preview');
    expect(result).toContain(phillMessage);
  });
