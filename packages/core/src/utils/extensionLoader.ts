...
export abstract class ExtensionLoader {
   ...
   */
  protected async startExtension(extension: PhillCLIExtension) {
    if (!this.config) {
    ...
    try {
      ...
      await this.maybeRefreshPhillTools(extension);
  ...
  private async maybeRefreshMemories(): Promise<void> {
    if (!this.config) {
      throw new Error(
        'Cannot refresh phill memories prior to calling `start`.',
      );
    }
    if (
   ...
   */
  private async maybeRefreshPhillTools(
    extension: PhillCLIExtension,
  ): Promise<void> {
   ...
   * initialized.
   */
  protected async stopExtension(extension: PhillCLIExtension) {
    if (!this.config) {
  ...
  async restartExtension(extension: PhillCLIExtension): Promise<void> {
    await this.stopExtension(extension);
    await this.startExtension(extension);
  }
}
...
export class SimpleExtensionLoader extends ExtensionLoader {
  ...
  async loadExtension(extension: PhillCLIExtension) {
  ...
  async unloadExtension(extension: PhillCLIExtension) {
