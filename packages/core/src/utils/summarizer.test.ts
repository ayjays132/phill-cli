...
describe('summarizers', () => {
  let mockPhillClient: PhillClient;
  let MockConfig: Mock;
  ...
  beforeEach(() => {
    ...
    mockPhillClient = new PhillClient(mockConfigInstance);
    (mockPhillClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockPhillClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockPhillClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockPhillClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockPhillClient.generateContent as Mock).mockRejectedValue(error);
...
`;
      const calledWith = (mockPhillClient.generateContent as Mock).mock
        .calls[0];
