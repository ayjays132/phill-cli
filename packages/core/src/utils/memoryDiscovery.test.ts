...
import { PHILL_DIR } from './paths.js';
import { Config, type PhillCLIExtension } from '../config/config.js';
...
describe('memoryDiscovery', () => {
  ...
  afterEach(async () => {
    ...
    setPhillMdFilename(DEFAULT_CONTEXT_FILENAME);
  ...
  describe('when untrusted', () => {
    ...
    it('loads context from outside the untrusted workspace', async () => {
      ...
      const filepath = path.join(homedir, PHILL_DIR, DEFAULT_CONTEXT_FILENAME);
  ...
  it('should load only the global context file if present and others are not (default filename)', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, PHILL_DIR, DEFAULT_CONTEXT_FILENAME),
...
--- End of Context from: ${path.relative(cwd, defaultContextFile)} ---`,
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setPhillMdFilename(customFilename);

    const customContextFile = await createTestFile(
      path.join(homedir, PHILL_DIR, customFilename),
...
--- End of Context from: ${path.join('subdir', customFilename)} ---`,
  ...
  it('should load ORIGINAL_PHILL_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootPhillFile = await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      'Project root memory',
    );
    const srcPhillFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'Src directory memory',
    );
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, projectRootPhillFile)} ---
Project root memory
--- End of Context from: ${path.relative(cwd, projectRootPhillFile)} ---

--- Context from: ${path.relative(cwd, srcPhillFile)} ---
Src directory memory
--- End of Context from: ${path.relative(cwd, srcPhillFile)} ---`,
      fileCount: 2,
      filePaths: [projectRootPhillFile, srcPhillFile],
    });
  });

  it('should load ORIGINAL_PHILL_MD_FILENAME files by downward traversal from CWD', async () => {
    const subDirPhillFile = await createTestFile(
      path.join(cwd, 'subdir', DEFAULT_CONTEXT_FILENAME),
      'Subdir memory',
    );
    const cwdPhillFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'CWD memory',
    );
...
--- End of Context from: ${path.join('subdir', DEFAULT_CONTEXT_FILENAME)} ---`,
      fileCount: 2,
      filePaths: [cwdPhillFile, subDirPhillFile],
    });
  });

  it('should load and correctly order global, upward, and downward ORIGINAL_PHILL_MD_FILENAME files', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, PHILL_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
    const rootPhillFile = await createTestFile(
...
--- Context from: ${path.relative(cwd, rootPhillFile)} ---
Project parent memory
--- End of Context from: ${path.relative(cwd, rootPhillFile)} ---

--- Context from: ${path.relative(cwd, projectRootPhillFile)} ---
Project root memory
--- End of Context from: ${path.relative(cwd, projectRootPhillFile)} ---

--- Context from: ${path.relative(cwd, cwdPhillFile)} ---
CWD memory
--- End of Context from: ${path.relative(cwd, cwdPhillFile)} ---

--- Context from: ${path.relative(cwd, subDirPhillFile)} ---
Subdir memory
--- End of Context from: ${path.relative(cwd, subDirPhillFile)} ---`,
      fileCount: 5,
      filePaths: [
        defaultContextFile,
        rootPhillFile,
        projectRootPhillFile,
        cwdPhillFile,
        subDirPhillFile,
      ],
    });
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const regularSubDirPhillFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, regularSubDirPhillFile)} ---
My code memory
--- End of Context from: ${path.relative(cwd, regularSubDirPhillFile)} ---`,
      fileCount: 1,
      filePaths: [regularSubDirPhillFile],
    });
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = await createTestFile(
      path.join(testRootDir, 'extensions/ext1/PHILL.md'),
...
--- End of Context from: ${path.relative(cwd, includedFile)} ---`,
  ...
  it('should handle multiple directories and files in parallel correctly', async () => {
    // Create multiple test directories with PHILL.md files
    const numDirs = 5;
  ...
  describe('loadEnvironmentMemory', () => {
    it('should load extension memory', async () => {
      const extFile = await createTestFile(
        path.join(testRootDir, 'ext', 'PHILL.md'),
        'Extension content',
      );
    ...
    it('should keep multiple memory files from the same directory adjacent and in order', async () => {
      ...
      setPhillMdFilename(['PRIMARY.md', 'SECONDARY.md']);
  ...
  it('refreshServerHierarchicalMemory should refresh memory and update config', async () => {
    ...
    expect(config.getPhillMdFileCount()).equals(refreshResult.fileCount);
    ...
    expect(config.getPhillMdFilePaths()).equals(refreshResult.filePaths);
  ...
  it('should include MCP instructions in user memory', async () => {
    const mockConfig = {
      ...
      setUserMemory: vi.fn(),
      setPhillMdFileCount: vi.fn(),
      setPhillMdFilePaths: vi.fn(),
