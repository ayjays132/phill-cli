...
describe('SimpleExtensionLoader', () => {
  ...
  let mockPhillClientSetTools: MockInstance<
    typeof PhillClient.prototype.setTools
  >;
  let mockHookSystemInit: MockInstance;
  let mockAgentRegistryReload: MockInstance;

  const activeExtension: PhillCLIExtension = {
    name: 'test-extension',
    isActive: true,
    version: '1.0.0',
  ...
  const inactiveExtension: PhillCLIExtension = {
    name: 'test-extension',
    isActive: false,
    version: '1.0.0',
    path: '/path/to/extension',
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getPhillClient: vi.fn(() => ({
        isInitialized: () => true,
        setTools: mockPhillClientSetTools,
      })),
      getHookSystem: () => ({
      ...
      (reloadingEnabled) => {
        ...
        it(`should ${reloadingEnabled ? '' : 'not '}reload extension features`, async () => {
          ...
          if (reloadingEnabled) {
            ...
            expect(mockPhillClientSetTools).toHaveBeenCalledOnce();
            ...
            expect(mockPhillClientSetTools).not.toHaveBeenCalledOnce();
          ...
          mockPhillClientSetTools.mockClear();
...
class TestingSimpleExtensionLoader extends SimpleExtensionLoader {
  override async startExtension(extension: PhillCLIExtension): Promise<void> {
    await super.startExtension(extension);
  }

  override async stopExtension(extension: PhillCLIExtension): Promise<void> {
