...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile(PHILL_DIR, 'config.yaml');
      await createTestFile(PHILL_DIR, 'logs.json');
  ...
  describe('with phillignore', () => {
    it('should ignore phillignore files by default', async () => {
      await fsPromises.writeFile(
    ...
    it('should not ignore files if respectGeminiIgnore is false', async () => {
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectGeminiIgnore: false,
          respectGitIgnore: true, // Explicitly disable phill ignore only
          customIgnoreFilePaths: [],
        },
      });
      expect(structure).toContain('ignored.txt');
