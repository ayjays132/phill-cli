...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Phill CLI documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Phill CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Phill CLI's internal documentation.
...
> {
  ...
  constructor(messageBus: MessageBus) {
    super(
      ...
      'GetInternalDocs',
      'Returns the content of Phill CLI internal documentation files. If no path is provided, returns a list of all available documentation paths.',
      Kind.Think,
      {
        type: 'object',
        properties: {
