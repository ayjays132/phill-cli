...
const MEMORY_SECTION_HEADER = '## Phill Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset PHILL_MD_FILENAME to its original value after each test
    setPhillMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setPhillMdFilename', () => {
    it('should update currentPhillMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setPhillMdFilename(newName);
      expect(getCurrentPhillMdFilename()).toBe(newName);
    });

    it('should not update currentPhillMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentPhillMdFilename(); // Get current before trying to change
      setPhillMdFilename('  ');
      expect(getCurrentPhillMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setPhillMdFilename(newNames);
      expect(getCurrentPhillMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllPhillMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    const mockConfig = {
      getContentGeneratorConfig: vi.fn().mockResolvedValue({}),
    } as unknown as Config;

    beforeEach(() => {
      memoryTool = new MemoryTool(createMockMessageBus(), mockConfig);
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentPhillMdFilename for the default expectation before any setPhillMdFilename calls in a test
      const expectedFilePath = path.join(
        os.homedir(),
        PHILL_DIR,
        getCurrentPhillMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', PHILL_DIR, 'PHILL.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(
          path.join('mock', 'home', PHILL_DIR),
        );
        expect(result.fileName).toContain('PHILL.md');
        expect(result.fileDiff).toContain('Index: PHILL.md');
        expect(result.fileDiff).toContain('+## Phill Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Phill Added Memories');
