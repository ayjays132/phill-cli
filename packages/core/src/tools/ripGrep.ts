...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    if (!no_ignore) {
      ...
      // Add .phillignore and custom ignore files support (if provided/mandated)
      // (ripgrep natively handles .gitignore)
      const phillIgnorePaths = this.fileDiscoveryService.getIgnoreFilePaths();
      for (const ignorePath of phillIgnorePaths) {
