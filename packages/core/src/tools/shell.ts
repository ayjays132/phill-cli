...
> {
  ...
  async execute(
    ...
    if (strippedCommand.includes('npm run build') || strippedCommand.includes('npm run build:dev')) {
      const phase = strippedCommand.includes('build:dev') ? "Development Build" : "Production Build";
      debugLogger.log(`[ContinuityVault] ${phase} detected. Snapshotting state...`);
      void ContinuityVault.snapshot(
        this.config, 
        `Building: ${strippedCommand}`, 
        "Verify build results and resume primary task.",
        phase,
        'in_progress'
      );
    }
    ...
    try {
      ...
      if (result.exitCode === 0 && (strippedCommand.includes('npm run build') || strippedCommand.includes('npm run build:dev'))) {
        debugLogger.log(`[ContinuityVault] Build successful. Flagging for reload...`);
        // We can't exit here because we need to return the tool result first.
        // We'll add a flag to the result that the executor can use.
        (result as any).shouldReload = true;
      }
