...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .phillignore files by default', async () => {
      await fs.writeFile(
    ...
    it('should not respect .phillignore when respect_phill_ignore is false', async () => {
      await fs.writeFile(
        path.join(tempRootDir, GEMINI_IGNORE_FILE_NAME),
        'gemini-ignored_test.txt',
      );
