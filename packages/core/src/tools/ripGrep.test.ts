...
// Mock child_process for ripgrep calls. Keep other exports intact to avoid
// breaking unrelated imports that rely on exec/spawnSync.
...
describe('RipGrepTool', () => {
  ...
  let mockConfig = {
    ...
    getDebugMode: () => false,
    getFileFilteringRespectPhillIgnore: () => true,
  ...
  describe('execute', () => {
    ...
    it('should filter out files based on FileDiscoveryService even if ripgrep returns them', async () => {
      // Create .phillignore to ignore 'ignored.txt'
      await fs.writeFile(
        path.join(tempRootDir, GEMINI_IGNORE_FILE_NAME),
        'ignored.txt',
      );

      // Re-initialize tool so FileDiscoveryService loads the new .phillignore
  ...
  describe('advanced search options', () => {
    ...
    it('should add .phillignore when enabled and patterns exist', async () => {
      const phillIgnorePath = path.join(tempRootDir, GEMINI_IGNORE_FILE_NAME);
      await fs.writeFile(phillIgnorePath, 'ignored.log');
      const configWithPhillIgnore = {
        getTargetDir: () => tempRootDir,
        getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir),
        getDebugMode: () => false,
        getFileFilteringRespectPhillIgnore: () => true,
      ...
      const phillIgnoreTool = new RipGrepTool(
        configWithPhillIgnore,
        createMockMessageBus(),
      );
      ...
      const invocation = phillIgnoreTool.build(params);
      await invocation.execute(abortSignal);

      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.arrayContaining(['--ignore-file', phillIgnorePath]),
        expect.anything(),
      );
    });

    it('should skip .phillignore when disabled', async () => {
      const phillIgnorePath = path.join(tempRootDir, GEMINI_IGNORE_FILE_NAME);
      await fs.writeFile(phillIgnorePath, 'ignored.log');
      const configWithoutPhillIgnore = {
        getTargetDir: () => tempRootDir,
        getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir),
        getDebugMode: () => false,
        getFileFilteringRespectPhillIgnore: () => false,
        getFileFilteringOptions: () => ({
          respectGitIgnore: true,
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.not.arrayContaining(['--ignore-file', phillIgnorePath]),
        expect.anything(),
      );
    });
