...
describe('GrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should support fixed_strings for literal search', async () => {
      const params: GrepToolParams = { pattern: 'foo.*bar', fixed_strings: true };
      const invocation = grepTool.build(params);
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain('No matches found');
    });

    it('should support case_sensitive search', async () => {
      const params: GrepToolParams = { pattern: 'HELLO', case_sensitive: true };
      const invocation = grepTool.build(params);
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain('No matches found');
    });
    ...
    it('should support files_with_matches mode', async () => {
      const params: GrepToolParams = { pattern: 'world', files_with_matches: true };
      const invocation = grepTool.build(params);
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain('Found matches for pattern "world" in 2 files');
      expect(result.llmContent).toContain('fileA.txt');
      expect(result.llmContent).toContain(path.join('sub', 'fileC.txt'));
      expect(result.llmContent).not.toContain('L1:'); // Should not show lines
    });

    it('should support context lines (JS fallback)', async () => {
      const params: GrepToolParams = { pattern: 'second line', context: 1 };
      const invocation = grepTool.build(params);
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain('File: fileA.txt');
      expect(result.llmContent).toContain(' 1: hello world'); // Context line (space prefix)
      expect(result.llmContent).toContain('L2: second line with world'); // Match line (L prefix)
    });

    it('should parse context lines from system grep output', async () => {
      vi.mocked(execStreaming).mockImplementationOnce(() =>
        createLineGenerator(['fileA.txt:2:match line', 'fileA.txt-1:context line']),
      );

      const params: GrepToolParams = { pattern: 'match', context: 1 };
      const invocation = grepTool.build(params) as unknown as {
        isCommandAvailable: (command: string) => Promise<boolean>;
        execute: (signal: AbortSignal) => Promise<ToolResult>;
      };
      ...
      const result = await invocation.execute(abortSignal);
      expect(result.llmContent).toContain('File: fileA.txt');
      expect(result.llmContent).toContain('L2: match line');
      expect(result.llmContent).toContain(' 1: context line');
    });
