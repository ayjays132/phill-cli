...
describe('ReadFileTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should support pattern-based reading', async () => {
      const filePath = path.join(tempRootDir, 'pattern.txt');
      const lines = Array.from({ length: 50 }, (_, i) => 
        i === 25 ? 'FOUND_ME' : `Line ${i + 1}`
      );
      ...
      expect(result.llmContent).toContain('FOUND_ME');
      // With limit 10, it should show lines around index 25 (line 26)
      // Math.max(0, 25 - 5) = 20 (Line 21)
      expect(result.llmContent).toContain('Line 21');
      expect(result.llmContent).toContain('Line 30');
      expect(result.returnDisplay).toBe('Read lines 21-30 of 50 from pattern.txt');
    });

    describe('with .phillignore', () => {
      beforeEach(async () => {
        await fsp.writeFile(
      ...
      it('should throw error if path is ignored by a .phillignore pattern', async () => {
