...
export class PolicyEngine {
  ...
  private shouldDowngradeForRedirection(
    ...
    // If we're in YOLO mode, we never downgrade for redirection.
  ...
  private async checkShellCommand(
    ...
    if (!command) {
      return {
        decision: this.applyContextualDecision(ruleDecision),
        rule,
      };
    }
    ...
    if (subCommands.length === 0) {
      ...
      return {
        decision: this.applyContextualDecision(PolicyDecision.ASK_USER),
        rule: undefined,
      };
    }
    ...
    if (subCommands.length > 0) {
      ...
      return {
        decision: this.applyContextualDecision(aggregateDecision),
  ...
  async check(
    ...
    for (const rule of this.rules) {
      ...
      if (match) {
        ...
        if (isShellCommand && toolName) {
          ...
          decision = this.applyContextualDecision(rule.decision);
          matchedRule = rule;
          break;
        }
      }
    ...
    if (decision === undefined) {
      ...
      if (toolName && SHELL_TOOL_NAMES.includes(toolName)) {
        ...
        decision = this.applyContextualDecision(this.defaultDecision);
      }
    }
    ...
    return {
      decision: this.applyContextualDecision(decision),
      rule: matchedRule,
    };
  }
  ...
  private applyContextualDecision(decision: PolicyDecision): PolicyDecision {
    let finalDecision = decision;

    // YOLO mode overrides ASK_USER to ALLOW
    if (
    ...
    if (this.nonInteractive && finalDecision === PolicyDecision.ASK_USER) {
