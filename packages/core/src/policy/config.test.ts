    ...
    const mockReaddir = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.phill/policies'))
        ) {
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.phill/policies/write.toml'))
        ) {
          return `
[[rule]]
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.phill/policies/safety.toml'))
        ) {
          return `
[[rule]]
toolName = "write_file"
      ...
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(
              nodePath.normalize('.phill/policies/invalid_safety.toml'),
            )
        ) {
          return `
[[rule]]
