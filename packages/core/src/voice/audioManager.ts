...
class NativeRecorder extends EventEmitter implements VoiceRecorder {
  ...
  constructor() {
    ...
    try {
      ...
      debugLogger.log(
        'AudioManager: native-recorder-nodejs loaded successfully',
      );
    } catch (_e) {
      debugLogger.debug(
        'AudioManager: native-recorder-nodejs not found. Optional native recording unavailable.',
      );
      this.recorder = null;
    }
  }
    ...
    try {
      ...
      if (!this.isLoopback && deviceId !== 'default') {
        if (!isNaN(Number(deviceId))) {
          ...
          debugLogger.warn(
            `NativeRecorder: Cannot map string device ID '${deviceId}' to native index. Failing to trigger fallback.`,
          );
      ...
      debugLogger.error('NativeRecorder failed to start', _e);
...
class FfmpegRecorder extends EventEmitter implements VoiceRecorder {
  ...
  async start(
    ...
    if (platform === 'win32') {
      ...
      inputOption =
        inputDeviceName === 'default' ? ':0' : `:${inputDeviceName}`;
    }
    ...
    if (platform === 'win32') {
      ...
      this.command.input(inputOption).inputFormat('avfoundation');
    } else {
      this.command.input(inputDeviceName).inputFormat('alsa');
    }
    ...
    this.command.on('start', (commandLine) => {
      debugLogger.log(
        `[ffmpeg-record] Spawned Ffmpeg with command: ${commandLine}`,
      );
      this.recording = true;
    });

    // Use any cast to bypass fragile fluent-ffmpeg type overloads
    (this.command as any).on(
      'error',
      (err: any, _stdout: string, stderr: string) => {
        if (this.recording) {
          // Only log if we expect to be recording
    ...
    audioStream.on('data', (chunk) => {
      if (firstChunk) {
        debugLogger.log(
          `[ffmpeg-record] First audio chunk received: ${chunk.length} bytes`,
        );
        firstChunk = false;
      }
      this.emit('data', chunk);
...
export class AudioManager extends EventEmitter {
  ...
  private noiseSuppressionLevel: 'light' | 'standard' | 'aggressive' =
    'standard';
  ...
  static forceStopAllPlayback(): void {
    debugLogger.log(
      'Force stopping all audio playback and resetting global count.',
    );
  ...
  setVadThreshold(threshold: number) {
    debugLogger.debug(
      `setVadThreshold(${threshold}) called - handled by volume emission`,
    );
  }
  ...
  setVoiceProcessing(options: {
    ...
    let changed = false;
    if (
      typeof options.autoGainControl === 'boolean' &&
    ...
    if (
      typeof options.highpassFilter === 'boolean' &&
  ...
  getRecordingStatus(): boolean {
    return this.activeRecorder.isRecording();
  }
  ...
  private scheduleRecordingRestart() {
    if (
      !this.shouldRecord ||
      this.activeRecorder.isRecording() ||
      this.recordingRestartTimer
    )
      return;
  ...
  writeAudio(chunk: Buffer | string | Uint8Array) {
    ...
    if (this.playbackProcess?.stdin) {
      try {
        ...
        this.currentOutputVolume =
          (Math.sqrt(sum / (buffer.length / 2)) / 32768) * 100;
        this.emit('outputVolume', this.currentOutputVolume);
        this.playbackProcess.stdin.write(buffer);
      } catch (_e) {
        debugLogger.error('Error writing to playback', _e);
      }
    }
  }
  ...
  startPlayback() {
    ...
    this.playbackProcess.on('close', () => {
      ...
      if (wasPlaying) {
        AudioManager.activePlaybackCount = Math.max(
          0,
          AudioManager.activePlaybackCount - 1,
        );
      }
  ...
  private getHighpassFilterString(): string | undefined {
    ...
    const freq =
      this.voiceIsolationMode === 'aggressive'
        ? 140
        : this.voiceIsolationMode === 'standard'
          ? 120
          : 90;
