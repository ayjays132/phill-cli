    ...
    const summaryResponse = await config.getBaseLlmClient().generateContent({
      ...
      contents: [
        ...historyForSummarizer,
        {
          ...
          parts: [
            {
              text: `${anchorInstruction}\n\nReason briefly in your scratchpad, then generate an EXTREMELY concise <state_snapshot>. Avoid repeating information clearly present in the recent history slice.`,
            },
          ],
        },
      ],
