...
export const NEVER_ALLOWED_VALUE_PATTERNS = [
  ...
  // Moltbook-specific autonomous agent risk patterns
  /credential_request|steal_keys|exfiltrate_config/i,
  /base64\s+-d\s+\|?\s*sh/i, // Obfuscated shell pipe
  /curl\s+-X\s+POST\s+-d\s+@/i, // Exfiltration attempt
] as const;
