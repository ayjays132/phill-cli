...
export class FileDiscoveryService {
  ...
  private phillIgnoreFilter: IgnoreFileFilter | null = null;
  ...
  constructor(projectRoot: string, options?: FilterFilesOptions) {
    ...
    this.phillIgnoreFilter = new IgnoreFileParser(
      this.projectRoot,
    ...
    if (this.gitIgnoreFilter) {
      const phillPatterns = this.phillIgnoreFilter.getPatterns();
      const customPatterns = this.customIgnoreFilter
        ? this.customIgnoreFilter.getPatterns()
        : [];
      // Create combined parser: .gitignore + .phillignore + custom ignore
      this.combinedIgnoreFilter = new GitIgnoreParser(
        this.projectRoot,
        // customPatterns should go the last to ensure overwriting of phillPatterns
        [...phillPatterns, ...customPatterns],
      );
    } else {
  ...
  filterFiles(filePaths: string[], options: FilterFilesOptions = {}): string[] {
    ...
    return filePaths.filter((filePath) => {
      ...
      if (respectGeminiIgnore && this.phillIgnoreFilter?.isIgnored(filePath)) {
        return false;
      }
      return true;
    });
  ...
  /**
   * Returns the list of ignore files being used (e.g. .phillignore) excluding .gitignore.
   */
  getIgnoreFilePaths(): string[] {
    const paths: string[] = [];
    if (
      this.phillIgnoreFilter &&
      this.defaultFilterFileOptions.respectGeminiIgnore
    ) {
      paths.push(...this.phillIgnoreFilter.getIgnoreFilePaths());
    }
