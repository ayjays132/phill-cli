...
describe('ChatCompressionService', () => {
    ...
    const mockGenerateContent = vi
      ...
      .mockResolvedValueOnce({
        candidates: [
          {
            content: {
              parts: [{ text: 'U:Dev|T:Mock_S|E:A9R1V0|G:Test' }],
            },
          },
        ],
      } as unknown as GenerateContentResponse)
  ...
  it('should compress if over token threshold', async () => {
    ...
    // The first model response is used as the summary.
    expect(result.newHistory![0].parts![0].text).toContain('Initial Summary');
  ...
  it('should fall back to initial summary if verification response is empty', async () => {
    ...
    const mockLlmClient = {
      generateContent: vi
        ...
        .mockResolvedValueOnce({
          candidates: [{ content: { parts: [{ text: 'U:Latent' }] } }],
        } as unknown as GenerateContentResponse)
        .mockResolvedValueOnce({
  ...
  it('should still compress if summary is empty', async () => {
    const history: Content[] = [
      { role: 'user', parts: [{ text: 'msg1' }] },
      { role: 'model', parts: [{ text: 'msg2' }] },
    ];
