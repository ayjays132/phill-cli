 ...
 *
 * Sessions are stored as JSON files in ~/.phill/tmp/<project_hash>/chats/
 */
export class ChatRecordingService {
  ...
  recordMessage(message: {
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        if (msg.type === 'phill') {
          // If it's a new Phill message then incorporate any queued thoughts.
  ...
  /**
   * Updates the tokens for the last message in the conversation (which should be by Phill).
   */
  recordMessageTokens(
    ...
    try {
      ...
      this.updateConversation((conversation) => {
        ...
        if (lastMsg && lastMsg.type === 'phill' && !lastMsg.tokens) {
  ...
  /**
   * Adds tool calls to the last message in the conversation (which should be by Phill).
  ...
  recordToolCalls(model: string, toolCalls: ToolCallRecord[]): void {
    ...
    try {
      this.updateConversation((conversation) => {
        const lastMsg = this.getLastMessage(conversation);
        // If a tool call was made, but the last message isn't from Phill, it's because Phill is
        // calling tools without starting the message with text.  So the user submits a prompt, and
        // Phill immediately calls a tool (maybe with some thinking first).  In that case, create
        // a new empty Phill message.
        // Also if there are any queued thoughts, it means this tool call(s) is from a new Phill
        // message--because it's thought some more since we last, if ever, created a new Phill
        ...
        if (
          ...
          const newMsg: MessageRecord = {
            ...this.newMessage('phill' as const, ''),
          ...
          // The last message is an existing Phill message that we need to update.
  ...
  private writeConversation(
    ...
    try {
      ...
      if (this.cachedLastConvData !== JSON.stringify(conversation, null, 2)) {
        ...
        // Ensure the directory exists before writing, in case it was deleted
