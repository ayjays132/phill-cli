...
import { PhillEventType } from '../core/turn.js';
...
describe('LoopDetectionService', () => {
  ...
  const createContentEvent = (content: string): ServerPhillContentEvent => ({
  ...
  describe('Tool Call Loop Detection', () => {
    ...
    it('should not reset tool call counter for other event types', () => {
      ...
      const otherEvent = {
        type: 'thought',
      } as unknown as ServerPhillStreamEvent;
...
describe('LoopDetectionService LLM Checks', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      getPhillClient: () => mockPhillClient,
  ...
  it('should prepend user message if history starts with a function call', async () => {
    ...
    vi.mocked(mockPhillClient.getHistory).mockReturnValue(functionCallHistory);
