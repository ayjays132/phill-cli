...
describe('memory commands', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      getUserMemory: vi.fn(),
      getPhillMdFileCount: vi.fn(),
      getPhillMdFilePaths: vi.fn(),
  ...
  describe('showMemory', () => {
    it('should show memory content if it exists', () => {
      ...
      vi.mocked(mockConfig.getPhillMdFileCount).mockReturnValue(1);
    ...
    it('should show a message if memory is empty', () => {
      vi.mocked(mockConfig.getUserMemory).mockReturnValue('');
      vi.mocked(mockConfig.getPhillMdFileCount).mockReturnValue(0);
  ...
  describe('listMemoryFiles', () => {
    it('should list the memory files in use', () => {
      const filePaths = ['/path/to/PHILL.md', '/other/path/PHILL.md'];
      vi.mocked(mockConfig.getPhillMdFilePaths).mockReturnValue(filePaths);
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toContain(
          'There are 2 PHILL.md file(s) in use:',
        );
