...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['PHILL_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const rawTarget =
    argv.telemetryTarget ??
    env['PHILL_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['PHILL_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    argv.telemetryOtlpProtocol ??
    env['PHILL_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['PHILL_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const outfile =
    argv.telemetryOutfile ??
    env['PHILL_TELEMETRY_OUTFILE'] ??
    settings.outfile;

  const useCollector =
    parseBooleanEnvFlag(env['PHILL_TELEMETRY_USE_COLLECTOR']) ??
  ...
  return {
    ...
    useCollector,
    useCliAuth:
      parseBooleanEnvFlag(env['PHILL_TELEMETRY_USE_CLI_AUTH']) ??
      settings.useCliAuth,
  };
}
