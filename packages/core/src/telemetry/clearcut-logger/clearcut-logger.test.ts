...
expect.extend({
  ...
  toHaveMetadataValue(
    ...
    const data = metadata.find((m) => m.phill_cli_key === key)?.value;
  ...
  toHaveMetadataKey(received: LogEventEntry[], key: EventMetadataKey) {
    ...
    const pass = metadata.some((m) => m.phill_cli_key === key);
...
describe('ClearcutLogger', () => {
  ...
  describe('createLogEvent', () => {
    ...
    it('logs the GPU information (single GPU)', async () => {
      ...
      const gpuInfoEntry = event?.event_metadata[0].find(
        (item) => item.phill_cli_key === EventMetadataKey.PHILL_CLI_GPU_INFO,
      );
    ...
    it('logs multiple GPUs', async () => {
      ...
      const gpuInfoEntry = metadata?.find(
        (m) => m.phill_cli_key === EventMetadataKey.PHILL_CLI_GPU_INFO,
      );
  ...
  describe('GH_WORKFLOW_NAME metadata', () => {
    ...
    it('does not include workflow name when GH_WORKFLOW_NAME is not set', () => {
      ...
      const hasWorkflowName = event?.event_metadata[0].some(
        (item) =>
          item.phill_cli_key === EventMetadataKey.PHILL_CLI_GH_WORKFLOW_NAME,
      );
  ...
  describe('GITHUB_REPOSITORY metadata', () => {
    it('includes hashed repository when GITHUB_REPOSITORY is set', () => {
      vi.stubEnv('GITHUB_REPOSITORY', 'google/phill-cli');
      const { logger } = setup({});
      ...
      const repositoryMetadata = event?.event_metadata[0].find(
        (item) =>
          item.phill_cli_key ===
          EventMetadataKey.PHILL_CLI_GH_REPOSITORY_NAME_HASH,
      );
      expect(repositoryMetadata).toBeDefined();
      expect(repositoryMetadata?.value).toMatch(/^[a-f0-9]{64}$/);
      expect(repositoryMetadata?.value).not.toBe('google/phill-cli');
    });
  ...
  describe('logChatCompressionEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_COMPRESSION_TOKENS_BEFORE,
        '9001',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_COMPRESSION_TOKENS_AFTER,
        '8000',
      ]);
    });
  });
  ...
  describe('enqueueLogEvent', () => {
    ...
    it('should evict the oldest event when the queue is full', () => {
      ...
      for (let i = 0; i < TEST_ONLY.MAX_EVENTS; i++) {
        logger!.enqueueLogEvent(
          logger!.createLogEvent(EventNames.API_ERROR, [
            {
              phill_cli_key: EventMetadataKey.PHILL_CLI_AI_ADDED_LINES,
              value: `${i}`,
            },
          ]),
        );
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AI_ADDED_LINES,
        '0',
      ]);
  ...
  describe('logModelRoutingEvent', () => {
    it('logs a successful routing event', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_DECISION,
        'gemini-pro',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_DECISION_SOURCE,
        'default-strategy',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_LATENCY_MS,
        '123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_FAILURE,
        'false',
      ]);
    });
    ...
    it('logs a failed routing event with a reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_FAILURE_REASON,
        'Something went wrong',
      ]);
    });
    ...
    it('logs a successful routing event with numerical routing fields', () => {
      ...
      expect(events[0]).toHaveEventName(EventNames.MODEL_ROUTING);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_REASONING,
        '[Score: 90 / Threshold: 80] reasoning',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_ROUTING_NUMERICAL_ENABLED,
        'true',
      ]);
      expect(events[0]).toHaveMetadataValue([
  ...
  describe('logAgentStartEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AGENT_ID,
        'agent-123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AGENT_NAME,
        'TestAgent',
      ]);
    });
  });
  ...
  describe('logExperiments', () => {
    it('async path includes exp.gws_experiment field with experiment IDs', async () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_EXPERIMENT_IDS,
        '123,456,789',
      ]);
  ...
  describe('logAgentFinishEvent', () => {
    it('logs an event with proper fields (success)', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AGENT_DURATION_MS,
        '1000',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AGENT_TURN_COUNT,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AGENT_TERMINATE_REASON,
        'GOAL',
      ]);
    });
  ...
  describe('logToolCallEvent', () => {
    it('logs an event with all diff metadata', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AI_REMOVED_LINES,
        '2',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AI_ADDED_CHARS,
        '3',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_AI_REMOVED_CHARS,
        '4',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_USER_ADDED_LINES,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_USER_REMOVED_LINES,
        '6',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_USER_ADDED_CHARS,
        '7',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_USER_REMOVED_CHARS,
        '8',
      ]);
    });
  ...
  describe('logWebFetchFallbackAttemptEvent', () => {
    it('logs an event with the proper name and reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_WEB_FETCH_FALLBACK_REASON,
        'private_ip',
      ]);
    });
  });
  ...
  describe('logHookCallEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_HOOK_EVENT_NAME,
        'before-tool',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_HOOK_DURATION_MS,
        '150',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_HOOK_SUCCESS,
        'true',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.PHILL_CLI_HOOK_EXIT_CODE,
        '0',
      ]);
    });
  });
