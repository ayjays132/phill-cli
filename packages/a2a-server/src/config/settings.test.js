...
vi.mock('node:os', async (importOriginal) => {
    ...
    return {
        ...actual,
        homedir: () => path.join(actual.tmpdir(), `phill-home-${mocks.suffix}`),
    };
});
vi.mock('phill-cli-core', async (importOriginal) => {
    ...
    return {
        ...
        homedir: () => path.join(os.tmpdir(), `phill-home-${mocks.suffix}`),
    };
});
describe('loadSettings', () => {
    const mockHomeDir = path.join(os.tmpdir(), `phill-home-${mocks.suffix}`);
    const mockWorkspaceDir = path.join(os.tmpdir(), `phill-workspace-${mocks.suffix}`);
    const mockPhillHomeDir = path.join(mockHomeDir, '.phill');
    const mockPhillWorkspaceDir = path.join(mockWorkspaceDir, '.phill');
    beforeEach(() => {
        vi.clearAllMocks();
        // Create the directories using the real fs
        if (!fs.existsSync(mockPhillHomeDir)) {
            fs.mkdirSync(mockPhillHomeDir, { recursive: true });
        }
        if (!fs.existsSync(mockPhillWorkspaceDir)) {
            fs.mkdirSync(mockPhillWorkspaceDir, { recursive: true });
        }
        // Clean up settings files before each test
        if (fs.existsSync(USER_SETTINGS_PATH)) {
            fs.rmSync(USER_SETTINGS_PATH);
        }
        const workspaceSettingsPath = path.join(mockPhillWorkspaceDir, 'settings.json');
