...
import { debugLogger, SimpleExtensionLoader } from 'phill-cli-core';
import type { Command, CommandArgument } from '../commands/types.js';
import { GitService, BrowserService } from 'phill-cli-core';
...
export async function createApp() {
  try {
    ...
    // --- VLA STREAMING ENDPOINT ---
    expressApp.get('/vla/stream', (req, res) => {
        res.setHeader('Content-Type', 'text/event-stream');
        ...
        browserService.startVLAStream();

        const handler = (latent: string) => {
            res.write(`data: ${JSON.stringify({ type: 'vla-update', latent })}\n\n`);
        };

        browserService.on('vla-update', handler);

        req.on('close', () => {
            browserService.off('vla-update', handler);
            // Optionally stop stream if no listeners? keeping it running for now for resilience
        });
    });
