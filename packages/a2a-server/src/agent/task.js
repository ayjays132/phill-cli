 ...
 */
import { CoreToolScheduler, PhillEventType, ToolConfirmationOutcome, ApprovalMode, getAllMCPServerStatuses, MCPServerStatus, isNodeError, parseAndFormatApiError, safeLiteralReplace, DEFAULT_GUI_EDITOR, EDIT_TOOL_NAMES, processRestorableToolCalls, } from 'phill-cli-core';
...
export class Task {
    ...
    constructor(id, contextId, config, eventBus, autoExecute = false) {
        ...
        this.phillClient = this.config.getPhillClient();
    ...
    // state managed within the @phill-cli/core module.
    async getMetadata() {
    ...
    async scheduleToolCalls(requests, abortSignal) {
        ...
        if (restorableToolCalls.length > 0 &&
            ...
            if (gitService) {
                const { checkpointsToWrite, toolCallToCheckpointMap, errors } = await processRestorableToolCalls(restorableToolCalls, gitService, this.phillClient);
                if (errors.length > 0) {
    ...
    async acceptAgentMessage(event) {
        ...
        switch (event.type) {
            ...
            case PhillEventType.ToolCallRequest:
                ...
                break;
            case PhillEventType.ToolCallResponse:
                // This event type from ServerPhillStreamEvent might be for when LLM *generates* a tool response part.
                ...
                break;
            case PhillEventType.ToolCallConfirmation:
                ...
                break;
            case PhillEventType.UserCancelled:
    ...
    async *sendCompletedToolsToLlm(completedToolCalls, aborted) {
        ...
        this.setTaskStateAndPublishUpdate('working', stateChange);
        yield* this.phillClient.sendMessageStream(llmParts, aborted, completedToolCalls[0]?.request.prompt_id ?? '');
    }
    async *acceptUserMessage(requestContext, aborted) {
        ...
        if (hasContentForLlm) {
            ...
            // Set task state to working as we are about to call LLM
            this.setTaskStateAndPublishUpdate('working', stateChange);
            yield* this.phillClient.sendMessageStream(llmParts, aborted, this.currentPromptId);
        }
