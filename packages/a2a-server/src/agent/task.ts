...
import {
  ...
  type ServerPhillErrorEvent,
  type ServerPhillStreamEvent,
...
export class Task {
  ...
  async acceptAgentMessage(event: ServerPhillStreamEvent): Promise<void> {
    const stateChange: StateChange = {
      kind: CoderAgentEvent.StateChangeEvent,
    };
    const traceId =
    ...
    switch (event.type) {
      ...
      default: {
        ...
        const errorEvent = event as ServerPhillErrorEvent; // Type assertion
        const errorMessage =
  ...
  async *sendCompletedToolsToLlm(
    ...
    this.setTaskStateAndPublishUpdate('working', stateChange);
    yield* this.phillClient.sendMessageStream(
      llmParts,
      aborted,
