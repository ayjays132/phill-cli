...
async function checkForUpdates(context, log, isManagedExtensionSurface) {
    try {
        ...
        if (!isManagedExtensionSurface &&
            ...
            const selection = await vscode.window.showInformationMessage(`A new version (${latestVersion}) of the Phill CLI Companion extension is available.`, 'Update to latest version');
...
export async function activate(context) {
    logger = vscode.window.createOutputChannel('Phill CLI IDE Companion');
    ...
    if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) &&
        ...
        void vscode.window.showInformationMessage('Phill CLI Companion extension successfully installed.');
    ...
    context.subscriptions.push((vscode.workspace.onDidChangeWorkspaceFolders(() => {
        ...
        vscode.workspace.onDidGrantWorkspaceTrust(() => {
            // eslint-disable-next-line @typescript-eslint/no-floating-promises
            ideServer.syncEnvVars();
        })), vscode.commands.registerCommand('phill-cli.runPhillCLI', async () => {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders || workspaceFolders.length === 0) {
            vscode.window.showInformationMessage('No folder open. Please open a folder to run Phill CLI.');
            return;
        }
        let selectedFolder;
        if (workspaceFolders.length === 1) {
            ...
            selectedFolder = await vscode.window.showWorkspaceFolderPick({
                placeHolder: 'Select a folder to run Phill CLI in',
            });
        }
        if (selectedFolder) {
            const phillCmd = 'phill';
            const terminal = vscode.window.createTerminal({
                name: `Phill CLI (${selectedFolder.name})`,
                cwd: selectedFolder.uri.fsPath,
            });
            terminal.show();
            terminal.sendText(phillCmd);
        }
    }), vscode.commands.registerCommand('phill-cli.showNotices', async () => {
