  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Phill CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Phill CLI Companion extension successfully installed.',
    );
    ...
    vscode.commands.registerCommand('phill-cli.runPhillCLI', async () => {
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Phill CLI.',
        );
        return;
      }
