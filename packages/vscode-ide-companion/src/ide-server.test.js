...
const getPortFromMock = (replaceMock) => {
    const port = vi
        .mocked(replaceMock)
        .mock.calls.find((call) => call[0] === 'PHILL_CLI_IDE_SERVER_PORT')?.[1];
    if (port === undefined) {
...
describe('IDEServer', () => {
    ...
    it('should set environment variables and workspace path on start with multiple folders', async () => {
        ...
        expect(replaceMock).toHaveBeenNthCalledWith(1, 'PHILL_CLI_IDE_SERVER_PORT', expect.any(String));
        const expectedWorkspacePaths = [
            '/test/workspace1',
            '/test/workspace2',
        ].join(path.delimiter);
        expect(replaceMock).toHaveBeenNthCalledWith(2, 'PHILL_CLI_IDE_WORKSPACE_PATH', expectedWorkspacePaths);
        expect(replaceMock).toHaveBeenNthCalledWith(3, 'PHILL_CLI_IDE_AUTH_TOKEN', 'test-auth-token');
        const port = getPortFromMock(replaceMock);
        const expectedPortFile = path.join('/tmp', 'phill', 'ide', `phill-ide-server-${process.ppid}-${port}.json`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
            workspacePath: expectedWorkspacePaths,
            authToken: 'test-auth-token',
        });
        expect(fs.mkdir).toHaveBeenCalledWith(path.join('/tmp', 'phill', 'ide'), {
            recursive: true,
        });
    ...
    it('should set a single folder path', async () => {
        ...
        await ideServer.start(mockContext);
        const replaceMock = mockContext.environmentVariableCollection.replace;
        expect(replaceMock).toHaveBeenCalledWith('PHILL_CLI_IDE_WORKSPACE_PATH', '/foo/bar');
        const port = getPortFromMock(replaceMock);
        const expectedPortFile = path.join('/tmp', 'phill', 'ide', `phill-ide-server-${process.ppid}-${port}.json`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
    ...
    it('should set an empty string if no folders are open', async () => {
        ...
        await ideServer.start(mockContext);
        const replaceMock = mockContext.environmentVariableCollection.replace;
        expect(replaceMock).toHaveBeenCalledWith('PHILL_CLI_IDE_WORKSPACE_PATH', '');
        const port = getPortFromMock(replaceMock);
        const expectedPortFile = path.join('/tmp', 'phill', 'ide', `phill-ide-server-${process.ppid}-${port}.json`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
    ...
    it('should update the path when workspace folders change', async () => {
        ...
        expect(replaceMock).toHaveBeenCalledWith('PHILL_CLI_IDE_WORKSPACE_PATH', expectedWorkspacePaths);
        expect(replaceMock).toHaveBeenCalledWith('PHILL_CLI_IDE_AUTH_TOKEN', 'test-auth-token');
        const port = getPortFromMock(replaceMock);
        const expectedPortFile = path.join('/tmp', 'phill', 'ide', `phill-ide-server-${process.ppid}-${port}.json`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
        ...
        await ideServer.syncEnvVars();
        expect(replaceMock).toHaveBeenCalledWith('PHILL_CLI_IDE_WORKSPACE_PATH', '/baz/qux');
    ...
    it('should clear env vars and delete port file on stop', async () => {
        ...
        const replaceMock = mockContext.environmentVariableCollection.replace;
        const port = getPortFromMock(replaceMock);
        const portFile = path.join('/tmp', 'phill', 'ide', `phill-ide-server-${process.ppid}-${port}.json`);
