...
export class ExtensionManager extends ExtensionLoader {
  ...
  private loadedExtensions: PhillCLIExtension[] | undefined;
  ...
  async installOrUpdateExtension(
    ...
    let extension: PhillCLIExtension | null;
    try {
...
Would you like to attempt to install via "git clone" instead?`,
      ...
      try {
        ...
        if (missingSettings.length > 0) {
          const message = `Extension "${newExtensionConfig.name}" has missing settings: ${missingSettings
            ...
            .join(
              ', ',
            )}. Please run "phill extensions config ${newExtensionConfig.name} [setting-name]" to configure them.`;
   ...
   */
  async loadExtensions(): Promise<PhillCLIExtension[]> {
    if (this.loadedExtensions) {
  ...
  private async loadExtension(
    extensionDir: string,
  ): Promise<PhillCLIExtension | null> {
    this.loadedExtensions ??= [];
    ...
    try {
      ...
      const extension: PhillCLIExtension = {
        name: config.name,
  ...
  toOutputString(extension: PhillCLIExtension): string {
