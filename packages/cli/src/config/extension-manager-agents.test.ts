...
vi.mock('phill-cli-core', async (importOriginal) => {
  const core = await importOriginal<typeof import('phill-cli-core')>();
  return {
    ...core,
    homedir: mockHomedir,
...
describe('ExtensionManager agents loading', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'phill-test-agents-'));
    mockHomedir.mockReturnValue(tempDir);

    // Create the extensions directory that ExtensionManager expects
    extensionsDir = path.join(tempDir, '.phill', EXTENSIONS_DIRECTORY_NAME);
