...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'phill-cli-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'phill-cli-test-workspace-'),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when PHILL.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'PHILL.md'),
      ]);
    ...
    describe('id generation', () => {
      ...
      it('should warn about hooks during installation', async () => {
        ...
        fs.writeFileSync(
          path.join(sourceExtDir, 'phill-extension.json'),
          JSON.stringify({
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error and cleanup if phill-extension.json is missing', async () => {
    ...
    it('should throw an error for invalid JSON in phill-extension.json', async () => {
    ...
    it('should throw an error for missing name in phill-extension.json', async () => {
      const sourceExtDir = createExtension({
        extensionsDir: tempHomeDir,
...
${INSTALL_WARNING_MESSAGE}`,
    ...
    describe('installing from github', () => {
      ...
      const extensionName = 'phill-test-extension';
      ...
      it('should fallback to git clone without consent if no release data is found on first install', async () => {
        ...
        expect(mockRequestConsent).toHaveBeenCalledExactlyOnceWith(
          expect.stringContaining(
            'Installing extension "phill-test-extension"',
          ),
        );
        expect(mockGit.clone).toHaveBeenCalled();
        const metadataPath = path.join(
  ...
  describe('enableExtension', () => {
    ...
    const getActiveExtensions = (): PhillCLIExtension[] => {
      const extensions = extensionManager.getExtensions();
