...
import { getPackageJson } from 'phill-cli-core';
...
describe('loadSandboxConfig', () => {
  ...
  describe('with PHILL_SANDBOX environment variable', () => {
    it('should use docker if PHILL_SANDBOX=docker and it exists', async () => {
      process.env['PHILL_SANDBOX'] = 'docker';
    ...
    it('should throw if PHILL_SANDBOX is an invalid command', async () => {
      process.env['PHILL_SANDBOX'] = 'invalid-command';
    ...
    it('should throw if PHILL_SANDBOX command does not exist', async () => {
      process.env['PHILL_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'docker' (from PHILL_SANDBOX)",
      );
    });
  });
  ...
  describe('with sandbox: true', () => {
    ...
    it('should throw if sandbox: true but no command is found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(
        'PHILL_SANDBOX is true but failed to determine command for sandbox; ' +
          'install docker or podman or specify command in PHILL_SANDBOX',
      );
    });
  });
  ...
  describe("with sandbox: 'command'", () => {
    ...
    it('should throw if the specified command does not exist', async () => {
      ...
      await expect(
        ...
        "Missing sandbox command 'podman' (from PHILL_SANDBOX)",
      );
    });
  ...
  describe('image configuration', () => {
    it('should use image from PHILL_SANDBOX_IMAGE env var if set', async () => {
      process.env['PHILL_SANDBOX_IMAGE'] = 'env/image';
      process.env['PHILL_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
