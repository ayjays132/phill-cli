...
function createTestDir() {
  const dirPath = `/virtual-tmp/phill-test-${Math.random().toString(36).substring(2, 15)}`;
...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(ExtensionStorage.getUserExtensionsDir).mockReturnValue(
      path.join(testDir.path, PHILL_DIR),
    );
  ...
  it('should correctly prioritize more specific enable rules', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/phill-cli');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/phill-cli')).toBe(
      true,
    );
  });
