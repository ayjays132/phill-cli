...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .command('$0 [query..]', 'Launch Phill CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('resume', {
          ...
          coerce: (value: string): string => {
            // When --resume passed with a value (`phill --resume 123`): value = "123" (string)
            // When --resume passed without a value (`phill --resume`): value = "" (string)
...
export async function loadCliConfig(
  ...
  if (argv.sandbox) {
    process.env['PHILL_SANDBOX'] = 'true';
  }
  ...
  // directly to the Config constructor in core, and have core handle setPhillMdFilename.
  // However, loadHierarchicalPhillMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerPhillMdFilename(settings.context.fileName);
  } else {
    // Reset to default if not provided in settings.
    setServerPhillMdFilename(getCurrentPhillMdFilename());
  }
  ...
  if (!experimentalJitContext) {
    // Call the (now wrapper) loadHierarchicalPhillMemory which calls the server's version
  ...
  const yoloRequestedByAutoApproveSetting =
    settings.security?.autoApproveAllActionsForever === true;
  const rawApprovalMode =
    argv.approvalMode ||
    (argv.yolo ? 'yolo' : undefined) ||
    (yoloRequestedByAutoApproveSetting ? 'yolo' : undefined) ||
  ...
  if (settings.security?.disableYoloMode || settings.admin?.secureModeEnabled) {
    if (approvalMode === ApprovalMode.YOLO) {
      if (yoloRequestedByAutoApproveSetting) {
        debugLogger.warn(
          'Global auto-approve is enabled in settings, but YOLO mode is disabled by policy. Falling back to default approval mode.',
        );
        approvalMode = ApprovalMode.DEFAULT;
      } else {
  ...
  return new Config({
    ...
    phillMdFileCount: fileCount,
    phillMdFilePaths: filePaths,
    approvalMode,
    disableYoloMode:
