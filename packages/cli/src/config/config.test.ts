...
import * as ServerConfig from 'phill-cli-core';
...
const originalPhillModel = process.env['PHILL_MODEL'];

beforeEach(() => {
  delete process.env['PHILL_MODEL'];
...
afterEach(() => {
  ...
  if (originalPhillModel !== undefined) {
    process.env['PHILL_MODEL'] = originalPhillModel;
  } else {
    delete process.env['PHILL_MODEL'];
  }
});
    ...
    it.each([
      {
        ...
        argv: ['node', 'script.js', 'Hi Phill'],
        expectedQuery: 'Hi Phill',
...
describe('loadCliConfig', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('PHILL_API_KEY', 'test-api-key');
  ...
  it('should use default fileFilter options when unconfigured', async () => {
    ...
    expect(config.getFileFilteringRespectPhillIgnore()).toBe(
    ...
    vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
      {
        ...
        id: 'ext1-id',
        version: '1.0.0',
        contextFiles: ['/path/to/ext1/PHILL.md'],
        isActive: true,
      },
      {
        path: '/path/to/ext2',
...
describe('loadCliConfig model selection', () => {
  ...
  it('uses the default phill model if nothing is set', async () => {
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    },
    {
      ...
      getter: (c) => c.getFileFilteringRespectPhillIgnore(),
      value: true,
    },
    {
      property: 'respectGeminiIgnore',
...
describe('Telemetry configuration via environment variables', () => {
  ...
  it('should prioritize PHILL_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize PHILL_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should throw when PHILL_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('PHILL_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize PHILL_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('PHILL_TELEMETRY_OTLP_ENDPOINT', 'http://phill.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOtlpEndpoint()).toBe('http://phill.env.com');
  });

  it('should prioritize PHILL_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize PHILL_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize PHILL_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_OUTFILE', '/phill/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOutfile()).toBe('/phill/env/telemetry.log');
  });

  it('should prioritize PHILL_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('PHILL_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should use settings value when PHILL_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('PHILL_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
  ...
  it('should use settings value when PHILL_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('PHILL_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
      telemetry: { target: ServerConfig.TelemetryTarget.LOCAL },
  ...
  it("should treat PHILL_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('PHILL_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat PHILL_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('PHILL_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
  ...
  it("should treat PHILL_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('PHILL_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat PHILL_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('PHILL_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
