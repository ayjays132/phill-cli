...
import { FatalConfigError, PHILL_DIR } from 'phill-cli-core';
...
// Use the (mocked) PHILL_DIR for consistency
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    describe('when PHILL_CLI_SYSTEM_SETTINGS_PATH is set', () => {
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      const phillEnvPath = path.resolve(path.join(PHILL_DIR, '.env'));
      ...
      (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
        [USER_SETTINGS_PATH, phillEnvPath].includes(p.toString()),
      );
      ...
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === phillEnvPath) return 'TESTTEST=1234';
          return '{}';
        },
      );
    }
