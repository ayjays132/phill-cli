...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/PhillCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\phill-cli\\settings.json';
  } else {
    return '/etc/phill-cli/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['PHILL_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['PHILL_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homePhillEnvPath = path.join(homedir(), PHILL_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(PHILL_DIR);
