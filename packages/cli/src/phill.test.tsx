...
vi.mock('./config/config.js', () => ({
  loadCliConfig: vi.fn().mockResolvedValue({
    ...
    storage: {
      getProjectTempDir: vi.fn().mockReturnValue('/tmp/phill-test'),
    },
...
describe('phill.tsx main function', () => {
  let originalEnvPhillSandbox: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalEnvPhillSandbox = process.env['PHILL_SANDBOX'];
    originalEnvSandbox = process.env['SANDBOX'];
    delete process.env['PHILL_SANDBOX'];
  ...
  afterEach(() => {
    ...
    if (originalEnvPhillSandbox !== undefined) {
      process.env['PHILL_SANDBOX'] = originalEnvPhillSandbox;
    } else {
      delete process.env['PHILL_SANDBOX'];
    }
...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('phill-cli-core');
    const { initializeOutputListenersAndFlush } = await import('./phill.js');
...
describe('getNodeMemoryArgs', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['PHILL_CLI_NO_RELAUNCH'];
  });
  ...
  it('should return empty array if PHILL_CLI_NO_RELAUNCH is set', () => {
    process.env['PHILL_CLI_NO_RELAUNCH'] = 'true';
...
describe('phill.tsx main function kitty protocol', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvNoRelaunch = process.env['PHILL_CLI_NO_RELAUNCH'];
    process.env['PHILL_CLI_NO_RELAUNCH'] = 'true';
  ...
  afterEach(() => {
    ...
    if (originalEnvNoRelaunch !== undefined) {
      process.env['PHILL_CLI_NO_RELAUNCH'] = originalEnvNoRelaunch;
    } else {
      delete process.env['PHILL_CLI_NO_RELAUNCH'];
    }
    vi.restoreAllMocks();
  ...
  it.each([
    ...
    process.env['PHILL_API_KEY'] = 'test-key';
    try {
      await main();
    } catch (e) {
...
describe('phill.tsx main function exit codes', () => {
  let originalEnvNoRelaunch: string | undefined;
  ...
  it('should validate and refresh auth in non-interactive mode when no auth type is selected but env var is present', async () => {
    ...
    const { loadSettings } = await import('./config/settings.js');
    const { AuthType } = await import('phill-cli-core');
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('phill-cli-core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    const { getVersion } = await import('phill-cli-core');
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('phill-cli-core');
