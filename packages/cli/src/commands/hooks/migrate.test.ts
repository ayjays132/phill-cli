...
describe('migrate command', () => {
  ...
  beforeEach(() => {
    ...
    mockedLoadSettings.mockReturnValue({
      ...
      workspace: { path: '/test/project/.phill' },
    });
  });
  ...
  it('should migrate hooks from settings.json when it exists', async () => {
    ...
    expect(debugLoggerLogSpy).toHaveBeenCalledWith(
      '✓ Hooks successfully migrated to .phill/settings.json',
    );
  });
  ...
  it('should replace $CLAUDE_PROJECT_DIR with $PHILL_PROJECT_DIR', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
  ...
  it('should merge with existing Phill hooks', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
