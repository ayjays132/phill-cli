...
import { debugLogger, getErrorMessage } from 'phill-cli-core';
...
/**
 * Mapping from Claude Code event names to Phill event names
 */
const EVENT_MAPPING: Record<string, string> = {
  ...
  Stop: 'AfterAgent',
  SubAgentStop: 'AfterAgent', // Phill doesn't have sub-agents, map to AfterAgent
...
/**
 * Mapping from Claude Code tool names to Phill tool names
 */
...
/**
 * Transform a matcher regex to update tool names from Claude to Phill
 */
function transformMatcher(matcher: string | undefined): string | undefined {
  if (!matcher) return matcher;

  let transformed = matcher;
  for (const [claudeName, phillName] of Object.entries(TOOL_NAME_MAPPING)) {
...
/**
 * Migrate a Claude Code hook configuration to Phill format
 */
function migrateClaudeHook(claudeHook: unknown): unknown {
  ...
  if ('command' in hook) {
    ...
    // Replace CLAUDE_PROJECT_DIR with PHILL_PROJECT_DIR in command
  ...
  // Map timeout field (Claude uses seconds, Phill uses seconds)
...
/**
 * Migrate Claude Code hooks configuration to Phill format
 */
function migrateClaudeHooks(claudeConfig: unknown): Record<string, unknown> {
  ...
  const phillHooks: Record<string, unknown> = {};
  ...
  for (const [eventName, eventConfig] of Object.entries(hooksSection)) {
    // Map event name
    const phillEventName = EVENT_MAPPING[eventName] || eventName;
    ...
    phillHooks[phillEventName] = migratedDefinitions;
  }
...
export async function handleMigrateFromClaude() {
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .phill/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .phill/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Phill CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: phill hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Phill CLI format.',
      );
    }
    await exitCli();
  },
