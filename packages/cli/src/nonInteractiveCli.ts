  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    try {
      ...
      while (true) {
        ...
        const responseStream = phillClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (event.type === PhillEventType.Content) {
            const isRaw =
          ...
          } else if (event.type === PhillEventType.ToolCallRequest) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === PhillEventType.LoopDetected) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === PhillEventType.MaxSessionTurns) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
                type: JsonStreamEventType.ERROR,
                timestamp: new Date().toISOString(),
          ...
          } else if (event.type === PhillEventType.Error) {
            throw event.value.error;
          } else if (event.type === PhillEventType.AgentExecutionStopped) {
            ...
            return;
          } else if (event.type === PhillEventType.AgentExecutionBlocked) {
        ...
        if (toolCallRequests.length > 0) {
          ...
          // Record tool calls with full metadata before sending responses to Phill
          try {
            const currentModel =
              phillClient.getCurrentSequenceModel() ?? config.getModel();
            phillClient
              .getChat()
          ...
          // Check if any tool requested a reload (e.g. after a successful build)
          const shouldReload = completedToolCalls.some((tc) => (tc as any).shouldReload);
          if (shouldReload) {
            await relaunchApp();
          }
        } else {
