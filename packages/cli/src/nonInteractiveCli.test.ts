...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getPhillClient: vi.fn().mockReturnValue(mockPhillClient),
      getToolRegistry: vi.fn().mockReturnValue(mockToolRegistry),
      getMaxSessionTurns: vi.fn().mockReturnValue(10),
      getSessionId: vi.fn().mockReturnValue('test-session-id'),
      getProjectRoot: vi.fn().mockReturnValue('/test/project'),
      storage: {
        getProjectTempDir: vi.fn().mockReturnValue('/test/project/.phill/tmp'),
      },
  ...
  async function* createStreamFromEvents(
    events: ServerPhillStreamEvent[],
  ): AsyncGenerator<ServerPhillStreamEvent> {
  ...
  it('should process input and write text output', async () => {
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Hello' },
      { type: PhillEventType.Content, value: ' World' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockPhillClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockPhillClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerPhillStreamEvent = {
    ...
    const firstCallEvents: ServerPhillStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Final answer' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockPhillClient.sendMessageStream
    ...
    expect(mockPhillClient.sendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockSchedulerSchedule).toHaveBeenCalledWith(
      [expect.objectContaining({ name: 'testTool' })],
      expect.any(AbortSignal),
    );
    expect(mockPhillClient.sendMessageStream).toHaveBeenNthCalledWith(
      2,
      [{ text: 'Tool response' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should write a single newline between sequential text outputs from the model', async () => {
    ...
    const modelTurn1: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Use mock tool' },
      toolCallEvent,
    ];
    // Turn 2: Model outputs more text, then requests another tool call.
    const modelTurn2: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Use mock tool again' },
      toolCallEvent,
    ];
    // Turn 3: Model outputs a final answer.
    const modelTurn3: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Finished.' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerPhillStreamEvent[] = [
      {
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockPhillClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the Phill client
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Summary complete.' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Hello World' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerPhillStreamEvent[] = [
      toolCallEvent,
      {
        type: PhillEventType.Finished,
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Response from command' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should handle slash commands', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Response to slash command' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle cancellation (Ctrl+C)', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Thinking...' },
    ];
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Response to unknown' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Acknowledged' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should allow a normally-excluded tool when --allowed-tools is set', async () => {
    ...
    const firstCallEvents: ServerPhillStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'file.txt' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should resume chat when resumedSessionData is provided', async () => {
    const events: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Resumed' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    expect(mockPhillClient.resumeChat).toHaveBeenCalledWith(
      expect.any(Array),
      resumedSessionData,
    );
  ...
  it.each([
    ...
    async ({ events, input, promptId }) => {
      ...
      const streamEvents: ServerPhillStreamEvent[] = [
        ...events,
        {
          type: PhillEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should log error when tool recording fails', async () => {
    ...
    const events: ServerPhillStreamEvent[] = [
      toolCallEvent,
      { type: PhillEventType.Content, value: 'Done' },
      {
        type: PhillEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    mockPhillClient.getChat = vi.fn().mockReturnValue(mockChat);
    // @ts-expect-error - Mocking internal structure
    mockPhillClient.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('model-1');

    // Mock debugLogger.error
    const { debugLogger } = await import('phill-cli-core');
  ...
  it('should stop agent execution immediately when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Executing tool...' },
      toolCallEvent,
    ];
    ...
    expect(mockPhillClient.sendMessageStream).toHaveBeenCalledTimes(1);
  ...
  it('should write JSON output when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerPhillStreamEvent[] = [
      { type: PhillEventType.Content, value: 'Partial content' },
      toolCallEvent,
    ];
  ...
  describe('Agent Execution Events', () => {
    ...
    it('should handle AgentExecutionBlocked event', async () => {
      const allEvents: ServerPhillStreamEvent[] = [
        {
  ...
  describe('Output Sanitization', () => {
    ...
    it('should sanitize ANSI output by default', async () => {
      const events: ServerPhillStreamEvent[] = [
        { type: PhillEventType.Content, value: ANSI_SEQUENCE },
        { type: PhillEventType.Content, value: ' ' },
        { type: PhillEventType.Content, value: OSC_HYPERLINK },
        {
          type: PhillEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
        },
