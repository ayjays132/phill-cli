...
import { debugLogger, PHILL_DIR } from 'phill-cli-core';

export const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'phill-cli-sandbox';
export const SANDBOX_NETWORK_NAME = 'phill-cli-sandbox';
export const SANDBOX_PROXY_NAME = 'phill-cli-sandbox-proxy';
...
export function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const projectSandboxBashrc = `${PHILL_DIR}/sandbox.bashrc`;
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : isDebugMode
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which phill)`
        : 'phill';
