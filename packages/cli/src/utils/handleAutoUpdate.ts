...
const GITHUB_RELEASES_URL = 'https://github.com/ayjays132/phill-cli/releases';
const NPM_PACKAGE_URL = 'https://www.npmjs.com/package/phill-cli';
const PYPI_PACKAGE_URL = 'https://pypi.org/project/phill-cli/';

function withUpdateResources(message: string): string {
  return [
    message,
    `Release notes: ${GITHUB_RELEASES_URL}`,
    `npm package: ${NPM_PACKAGE_URL}`,
    `PyPI launcher: ${PYPI_PACKAGE_URL}`,
    'You can continue using Phill now. Updated version is used on next run.',
  ].join('\n');
}
...
export function handleAutoUpdate(
  ...
  if (settings.merged.tools.sandbox || process.env['PHILL_SANDBOX']) {
    updateEventEmitter.emit('update-info', {
      message: withUpdateResources(
  ...
  combinedMessage = withUpdateResources(combinedMessage);
