...
describe('handleAutoUpdate', () => {
  ...
  it('should emit "update-received" but not update if auto-updates are disabled', () => {
    ...
    mockGetInstallationInfo.mockReturnValue({
      updateCommand: 'npm i -g phill-cli@latest',
    ...
    expect(updateEventEmitter.emit).toHaveBeenCalledWith(
      'update-received',
  ...
  it('should combine update messages correctly', () => {
    ...
    expect(updateEventEmitter.emit).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        message: expect.stringContaining(
          'https://github.com/ayjays132/phill-cli/releases',
        ),
      }),
    );
  });
  ...
  it('should emit "update-failed" when the update process fails', async () => {
    ...
    expect(updateEventEmitter.emit).toHaveBeenCalledWith('update-failed', {
      message:
        'Automatic update failed. Please try updating manually. (command: npm i -g phill-cli@2.0.0)',
    });
  });
  ...
  it('should use the "@nightly" tag for nightly updates', async () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith('npm i -g phill-cli@nightly', {
      shell: true,
      stdio: 'ignore',
      detached: true,
    });
