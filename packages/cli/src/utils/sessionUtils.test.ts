...
import type { Config, MessageRecord } from 'phill-cli-core';
import { SESSION_FILE_PREFIX } from 'phill-cli-core';
...
describe('SessionSelector', () => {
  ...
  it('should list session with phill message even without user message', async () => {
    const sessionIdPhillOnly = randomUUID();

    // Create test session files
    const chatsDir = path.join(tmpDir, 'chats');
    await fs.mkdir(chatsDir, { recursive: true });

    // Session with only phill message - should be listed
    const sessionPhillOnly = {
      sessionId: sessionIdPhillOnly,
      projectHash: 'test-hash',
      startTime: '2024-01-01T10:00:00.000Z',
      lastUpdated: '2024-01-01T10:30:00.000Z',
      messages: [
    ...
    await fs.writeFile(
      path.join(
        chatsDir,
        `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdPhillOnly.slice(0, 8)}.json`,
      ),
      JSON.stringify(sessionPhillOnly, null, 2),
    );
    ...
    expect(sessions.length).toBe(1);
    expect(sessions[0].id).toBe(sessionIdPhillOnly);
  });
});
...
describe('hasUserOrAssistantMessage', () => {
  ...
  it('should return true when session has phill message', () => {
    const messages = [
      {
        type: 'phill',
        content: 'Hello, how can I help?',
  ...
  it('should return true when session has both user and phill messages', () => {
    const messages = [
      {
        type: 'user',
        content: 'Hello',
