...
import { debugLogger, isGitRepository } from 'phill-cli-core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        const brewPrefix = childProcess
          .execSync('brew --prefix phill-cli', {
            encoding: 'utf8',
        ...
        // Brew is not installed or phill-cli is not installed via brew.
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g phill-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add phill-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g phill-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g phill-cli@latest';
    return {
