...
import type { Config, SandboxConfig } from 'phill-cli-core';
import {
  coreEvents,
  debugLogger,
  FatalSandboxError,
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        profileFile = path.join(PHILL_DIR, `sandbox-macos-${profile}.sb`);
      }
      ...
      // start and set up proxy if PHILL_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['PHILL_SANDBOX_PROXY_COMMAND'];
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under phill-cli repo
    //
    // note this can only be done with binary linked from phill-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('phill-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed phill binary; ' +
            'run `npm link ./packages/cli` under phill-cli repo to switch to linked binary.',
        );
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              PHILL_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (
      ...
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the phill-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify phill-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    const userSettingsDirOnHost = path.join(userHomeDirOnHost, PHILL_DIR);
    ...
    // PHILL_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['PHILL_SANDBOX_PROXY_COMMAND'];
    ...
    const isIntegrationTest =
      process.env['PHILL_CLI_INTEGRATION_TEST'] === 'true';
    let containerName;
    if (isIntegrationTest) {
      containerName = `phill-cli-integration-test-${randomBytes(4).toString(
        'hex',
      )}`;
    ...
    // copy PHILL_CLI_TEST_VAR for integration tests
    if (process.env['PHILL_CLI_TEST_VAR']) {
      args.push(
        '--env',
        `PHILL_CLI_TEST_VAR=${process.env['PHILL_CLI_TEST_VAR']}`,
      );
    }
    ...
    // copy PHILL_MODEL
    if (process.env['PHILL_MODEL']) {
      args.push('--env', `PHILL_MODEL=${process.env['PHILL_MODEL']}`);
    }
    ...
    for (const envVar of [
      'PHILL_CLI_IDE_SERVER_PORT',
      'PHILL_CLI_IDE_WORKSPACE_PATH',
      'TERM_PROGRAM',
    ]) {
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(PHILL_DIR, 'sandbox.venv');
    ...
    if (process.env['PHILL_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
      ...
      // finally switch to that user to run the phill process. This is
      // necessary on Linux to ensure the user exists within the
      // container's /etc/passwd file, which is required by os.userInfo().
      const username = 'phill';
