...
import { EngineMessageType } from 'phill-cli-core';
import {
  cleanupCheckpoints,
  registerCleanup,
  registerSyncCleanup,
...
import { VoiceProvider } from './ui/contexts/VoiceContext.js';
...
export function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['PHILL_CLI_NO_RELAUNCH']) {
    return [];
  }
...
export async function startInteractiveUI(
  ...
  cognitiveEngineProcess: ChildProcess | null,
) {
                ...
                <VimModeProvider settings={settings}>
                  <VoiceProvider config={config}>
                    <AppContainer
                      ...
                      initializationResult={initializationResult}
                      cognitiveEngineProcess={cognitiveEngineProcess} // <--- New prop
                    />
                  </VoiceProvider>
                </VimModeProvider>
              </SessionStatsProvider>
...
export async function main() {
  ...
  // --- Meta-Cognition Engine (Cognitive Line) Setup ---
  let cognitiveEngineProcess: ChildProcess | null = null;
  const isCognitiveEngineEnabled = settings.merged.ui.cognitiveEngine?.enabled ?? true;
  if (!process.env['PHILL_CLI_NO_COGNITIVE_ENGINE'] && isCognitiveEngineEnabled) {
    try {
      // Resolve path using robust discovery logic
      const searchPaths = [
        path.join(__dirname, 'engine-process.js'), // Prod bundle
        path.resolve(__dirname, '../engine-process.js'), // Prod bundle sibling
        path.resolve(__dirname, '../../core/dist/cognitive-engine/engine-process.js'), // Built dev
        path.resolve(__dirname, '../../core/src/cognitive-engine/engine-process.ts'), // Raw dev
      ];
      ...
      if (!enginePath) {
        debugLogger.error(`Cognitive Engine not found. tried: ${searchPaths.join(', ')}`);
      } else {
        cognitiveEngineProcess = fork(enginePath, [], { stdio: ['inherit', 'inherit', 'inherit', 'ipc'] });
        debugLogger.debug(`Cognitive Engine spawned with PID: ${cognitiveEngineProcess.pid} from ${enginePath}`);
      }

      if (cognitiveEngineProcess) {
        // Configure the engine with current settings
        cognitiveEngineProcess.send({
          type: EngineMessageType.SET_CONFIG,
          idleThresholdSeconds: settings.merged.ui.cognitiveEngine?.idleThresholdSeconds ?? 15,
        });

        registerCleanup(() => {
          if (cognitiveEngineProcess && !cognitiveEngineProcess.killed) {
            debugLogger.debug('Terminating Cognitive Engine process.');
            cognitiveEngineProcess.kill('SIGTERM');
          }
        });

        cognitiveEngineProcess.on('exit', (code, signal) => {
          if (code !== 0 && signal !== 'SIGTERM') {
            debugLogger.warn(`Cognitive Engine exited unexpectedly with code ${code}, signal ${signal}`);
          } else {
            debugLogger.debug('Cognitive Engine process exited.');
          }
          cognitiveEngineProcess = null;
        });
      ...
      debugLogger.error('Failed to spawn Cognitive Engine:', err);
    }
  }
  ...
  // to run Phill CLI. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    const vaultState = await ContinuityVault.unlock(config);
    if (vaultState) {
      const phaseStr = vaultState.phase ? ` [Phase: ${vaultState.phase}]` : "";
      const statusStr = vaultState.status === 'completed' ? " ✅ COMPLETE" : " ✦ RESUMING";

      debugLogger.log(`[ContinuityVault] Handshake established. Resuming session: ${vaultState.sessionId}${phaseStr}`);
      coreEvents.emitFeedback('info', `Neural Bridge Active.${phaseStr}${statusStr}`);
      coreEvents.emitFeedback('info', `Current Task: ${vaultState.taskSummary}`);

      if (vaultState.nextStep) {
        coreEvents.emitFeedback('info', `Planned Next Step: ${vaultState.nextStep}`);
      }
    }
    ...
    if (!input) {
      debugLogger.error(
        `No input provided via stdin. Input can be provided by piping data into phill or using the --prompt option.`,
      );
      await runExitCleanup();
      process.exit(ExitCodes.FATAL_INPUT_ERROR);
    }
