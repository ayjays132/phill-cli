...
export async function runZedIntegration(
  ...
  const connection = new acp.AgentSideConnection(
    (connection) => new PhillAgent(config, settings, argv, connection),
    stream,
  );
...
export class PhillAgent {
  ...
  async initialize(
    ...
    const authMethods = [
      ...
      {
        id: AuthType.OPENAI_BROWSER,
        name: 'OpenAI / Codex (Browser Sign-In)',
        description: 'Uses native OpenAI browser/device OAuth flow',
      },
      {
        id: AuthType.ANTHROPIC,
      ...
      },
      {
        ...
        name: 'Custom API',
        description:
          'Requires CUSTOM_API_ENDPOINT and CUSTOM_API_MODEL (OpenAI-compatible)',
      },
    ];
  ...
  async newSession({
    ...
    const chat = await phillClient.startChat();
