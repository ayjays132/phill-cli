...
import { PhillAgent, Session } from './zedIntegration.js';
...
vi.mock(
  'phill-cli-core',
  async (
    importOriginal: () => Promise<typeof import('phill-cli-core')>,
  ) => {
...
describe('PhillAgent', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      ...
      getPhillClient: vi.fn().mockReturnValue({
    ...
    agent = new PhillAgent(mockConfig, mockSettings, mockArgv, mockConnection);
  });
  ...
  it('should create a new session', async () => {
    ...
    expect(mockConfig.getPhillClient).toHaveBeenCalled();
  });
...
describe('Session', () => {
  ...
  beforeEach(() => {
    mockChat = {
      ...
      addHistory: vi.fn(),
    } as unknown as Mocked<PhillChat>;
    mockTool = {
      kind: 'native',
      build: vi.fn().mockReturnValue({
