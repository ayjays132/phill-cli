...
import { AuthType, type Config } from 'phill-cli-core';
...
describe('useAuth', () => {
  beforeEach(() => {
    ...
    delete process.env['PHILL_DEFAULT_AUTH_TYPE'];
  });
  ...
  describe('validateAuthMethodWithSettings', () => {
    ...
    it('should return null if authType is USE_PHILL', () => {
      const settings = {
        merged: {
          security: {
            auth: {},
  ...
  describe('useAuthCommand', () => {
    ...
    it('should set error if no auth type is selected but env key exists', async () => {
      ...
      await waitFor(() => {
        expect(result.current.authError).toContain(
          'Existing API key detected (PHILL_API_KEY)',
        );
        expect(result.current.authState).toBe(AuthState.Updating);
      });
    });
    ...
    it('should authenticate if USE_PHILL and env key is found', async () => {
      mockLoadApiKey.mockResolvedValue(null);
      process.env['PHILL_API_KEY'] = 'env-key';
      const { result } = renderHook(() =>
        useAuthCommand(createSettings(AuthType.USE_GEMINI), mockConfig),
    ...
    it('should set error if PHILL_DEFAULT_AUTH_TYPE is invalid', async () => {
      process.env['PHILL_DEFAULT_AUTH_TYPE'] = 'INVALID_TYPE';
      const { result } = renderHook(() =>
        useAuthCommand(createSettings(AuthType.LOGIN_WITH_GOOGLE), mockConfig),
      );
      ...
      await waitFor(() => {
        expect(result.current.authError).toContain(
          'Invalid value for PHILL_DEFAULT_AUTH_TYPE',
        );
        expect(result.current.authState).toBe(AuthState.Updating);
      });
    });
