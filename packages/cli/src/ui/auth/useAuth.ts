...
import { getErrorMessage } from 'phill-cli-core';
    ...
    (async () => {
      ...
      if (!authType) {
        if (
          ...
          onAuthError(
            'Existing API key detected (PHILL_API_KEY, GEMINI_API_KEY, or GOOGLE_API_KEY). Select "Use Gemini API Key" to use it.',
          );
        } else {
      ...
      const defaultAuthType = process.env['PHILL_DEFAULT_AUTH_TYPE'];
      if (
        defaultAuthType &&
        !Object.values(AuthType).includes(defaultAuthType as AuthType)
      ) {
        onAuthError(
          `Invalid value for PHILL_DEFAULT_AUTH_TYPE: "${defaultAuthType}". ` +
