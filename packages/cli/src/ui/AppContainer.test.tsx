...
vi.mock('./hooks/usePhillStream.js');
...
vi.mock('./components/VoiceManager.js', () => ({
  VoiceManager: () => null,
}));
vi.mock('./contexts/VoiceContext.js', () => ({
  useVoice: () => ({
    toggleVoice: vi.fn(),
    toggleTts: vi.fn(),
    setSelectedInputDevice: vi.fn(),
...
describe('AppContainer State Management', () => {
  ...
  const mockedUsePhillStream = usePhillStream as Mock;
  ...
  beforeEach(() => {
    ...
    mockedUsePhillStream.mockReturnValue({
      streamingState: 'idle',
      submitQuery: vi.fn(),
      initError: null,
      pendingHistoryItems: [],
  ...
  describe('Session Resumption', () => {
    ...
    it('initializes chat recording service when config has it', () => {
      ...
      const configWithRecording = {
        ...mockConfig,
        getPhillClient: vi.fn(() => mockPhillClient),
  ...
  describe('Session Recording Integration', () => {
    it('provides chat recording service configuration', () => {
      ...
      expect(configWithRecording.getPhillClient).toBeDefined();
      expect(mockPhillClient.getChatRecordingService).toBeDefined();
  ...
  describe('Session Resume Flow', () => {
    it('accepts resumed session data', () => {
      ...
      expect(mockPhillClient.resumeChat).toBeDefined();
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should use legacy terminal title when dynamicWindowTitle is false', () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'Phill CLI (workspace)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    describe('Shell Focus Action Required', () => {
      ...
      it('should NOT show Action Required in title if shell is streaming output', async () => {
        ...
        mockedUsePhillStream.mockImplementation(() => ({
          streamingState: 'responding',
          submitQuery: vi.fn(),
          initError: null,
          pendingHistoryItems: [],
    ...
    it('should use CLI_TITLE environment variable when set', () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom Phill Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'✦  Working… (Custom Phill Title)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
  ...
  describe('onCancelSubmit Behavior', () => {
    ...
    // Helper to extract arguments from the usePhillStream hook call
    // This isolates the positional argument dependency to a single location
    const extractUsePhillStreamArgs = (args: unknown[]) => ({
    ...
    it('clears the prompt when onCancelSubmit is called with shouldRestorePrompt=false', async () => {
      ...
      const { onCancelSubmit } = extractUsePhillStreamArgs(
        mockedUsePhillStream.mock.lastCall!,
      );
