...
import { CoreToolScheduler } from 'phill-cli-core';
...
export type TrackedScheduledToolCall = ScheduledToolCall & {
  responseSubmittedToPhill?: boolean;
};
 ...
 *
 * This is currently the default implementation used by usePhillStream.
    ...
    (allCoreToolCalls: ToolCall[]) => {
      setToolCallsForDisplay((prevTrackedCalls) => {
        ...
        return allCoreToolCalls.map((coreTc): TrackedToolCall => {
          ...
          const responseSubmittedToPhill =
            existingTrackedCall?.responseSubmittedToPhill ?? false;
      ...
      setToolCallsForDisplay((prevCalls) =>
        prevCalls.map((tc) =>
          callIdsToMark.includes(tc.request.callId)
            ? { ...tc, responseSubmittedToPhill: true }
            : tc,
        ),
      );
    },
