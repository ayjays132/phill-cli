...
import type { Config, DiscoveredMCPResource } from 'phill-cli-core';
import {
...
import * as core from 'phill-cli-core';
...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectTempDir: () => path.join(os.tmpdir(), 'phill-cli-temp'),
      },
  ...
  describe('phill-ignore filtering', () => {
    it('should skip gemini-ignored files in @ commands', async () => {
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${phillIgnoredFile} is gemini-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nPhill-ignored: ${phillIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .phillignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, GEMINI_IGNORE_FILE_NAME),
      'build/output.js',
    );
  ...
  it('should handle mixed gemini-ignored and valid files', async () => {
    ...
    const query = `@${validFile} @${phillIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${getRelativePath(validFile)} @${phillIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${getRelativePath(validFile)}:\n` },
