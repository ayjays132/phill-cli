...
import { VoiceCommandLoader } from '../../services/VoiceCommandLoader.js';
...
interface SlashCommandProcessorActions {
  ...
  setForgeOpen: (value: boolean) => void;
}
  ...
  const commandContext = useMemo(
    (): CommandContext => ({
      ...
      ui: {
        ...
        setForgeOpen: actions.setForgeOpen,
      },
      session: {
        stats: session.stats,
    ...
    (async () => {
      const commandService = await CommandService.create(
        [
          ...
          new VoiceCommandLoader(actions.toggleVoice, actions.toggleTts),
        ],
        controller.signal,
      );
          ...
          if (commandToExecute.action) {
            ...
            if (result) {
              switch (result.type) {
                ...
                case 'load_history': {
                  config?.getPhillClient()?.setHistory(result.clientHistory);
