...
interface UseSessionResumeParams {
  ...
  refreshStatic: () => void;
  isPhillClientInitialized: boolean;
  ...
  const loadHistoryForResume = useCallback(
    async (
      ...
      try {
        ...
        // Give the history to the Phill client.
        await config.getPhillClient()?.resumeChat(clientHistory, resumedData);
      } catch (error) {
    ...
    [config, isPhillClientInitialized, setQuittingMessages],
  );
