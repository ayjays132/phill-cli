...
import { createMockMessageBus } from 'phill-cli-core/src/test-utils/mock-message-bus.js';
...
describe('useToolExecutionScheduler', () => {
  ...
  it('preserves responseSubmittedToPhill flag across updates', () => {
    const { result } = renderHook(() =>
      useToolExecutionScheduler(
        vi.fn().mockResolvedValue(undefined),
        mockConfig,
    ...
    expect(result.current[0][0].responseSubmittedToPhill).toBe(true);
  ...
  it('resolves the schedule promise when scheduler resolves', async () => {
    ...
    const { Scheduler } = await import('phill-cli-core');
  ...
  it('setToolCallsForDisplay re-groups tools by schedulerId (Multi-Scheduler support)', () => {
    ...
    act(() => {
      ...
      setToolCalls((prev) =>
        prev.map((t) => ({ ...t, responseSubmittedToPhill: true })),
      );
    });
    ...
    expect(toolCalls).toHaveLength(2);
    expect(toolCalls.every((t) => t.responseSubmittedToPhill)).toBe(true);
