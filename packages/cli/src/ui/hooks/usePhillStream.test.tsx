...
import { usePhillStream } from './usePhillStream.js';
...
const MockedPhillClientClass = vi.hoisted(() =>
...
vi.mock('phill-cli-core', async (importOriginal) => {
  ...
  return {
    ...actualCoreModule,
    GitService: vi.fn(),
    PhillClient: MockedPhillClientClass,
...
// --- Tests for usePhillStream Hook ---
describe('usePhillStream', () => {
  ...
  const mockGetPhillClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedPhillClientClass(mockConfig);
  ...
  const mockConfig: Config = {
    ...
    getPhillClient: mockGetPhillClient,
  ...
  beforeEach(() => {
    ...
    // Reset mocks for PhillClient instance methods (startChat and sendMessageStream)
    // The PhillClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // PhillChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.phill/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = (
    ...
    const client = phillClient || mockConfig.getPhillClient();
  ...
  const createMockToolCall = (
    ...
    responseSubmittedToPhill: false,
    confirmationDetails:
  ...
  const renderHookWithDefaults = (
    ...
    return renderHookWithProviders(() =>
      usePhillStream(
        new MockedPhillClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedPhillClientClass(mockConfig);
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerPhillEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Phill when a command is handled without a tool call', async () => {
    ...
    it('should call Phill with prompt content when slash command returns a `submit_prompt` action', async () => {
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = renderHookWithProviders(() =>
        usePhillStream(
          new MockedPhillClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPhillEventType.Content,
            value: 'This is a truncated response...',
          };
          yield {
            type: ServerPhillEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerPhillEventType.ContextWindowWillOverflow,
                value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerPhillEventType.ToolCallRequest,
    ...
    // Expectation: addItem:phill (rationale) MUST happen before scheduleToolCalls_START
    const rationaleIndex = addItemOrder.indexOf('addItem:phill');
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the Phill stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerPhillEventType.Content,
  ...
  describe('Thought Reset', () => {
    it('should reset thought to null when starting a new prompt', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPhillEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerPhillEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerPhillEventType.Error,
    ...
    it('should update lastOutputTime on Phill thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getPhillClient = vi.fn().mockReturnValue({
        ...new MockedPhillClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerPhillEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getPhillClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPhillEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerPhillEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
