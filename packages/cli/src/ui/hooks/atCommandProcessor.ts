...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    if (gitIgnored || phillIgnored) {
      const reason =
        gitIgnored && phillIgnored ? 'both' : gitIgnored ? 'git' : 'phill';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and phill'
          : reason === 'git'
            ? 'git-ignored'
            : 'gemini-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['phill'].length) {
      messages.push(`Phill-ignored: ${ignoredByReason['phill'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_phill_ignore: respectFileIgnore.respectGeminiIgnore,
    },
