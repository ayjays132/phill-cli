...
import { Config, ApprovalMode } from 'phill-cli-core';
import type { Config as ActualConfigType } from 'phill-cli-core';
...
vi.mock('phill-cli-core', async () => {
  const actualServerModule = await vi.importActual('phill-cli-core');
  return {
    ...actualServerModule,
    Config: vi.fn(),
  };
...
interface MockConfigInstanceShape {
  ...
  getPhillMdFileCount: Mock<() => number>;
  ...
  beforeEach(() => {
    ...
    (
      ...
      const instance: MockConfigInstanceShape = {
        ...
        getPhillMdFileCount: vi.fn().mockReturnValue(0) as Mock<() => number>,
        getToolRegistry: vi
          .fn()
