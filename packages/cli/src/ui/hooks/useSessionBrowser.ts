...
import { partListUnionToString, coreEvents } from 'phill-cli-core';
...
/**
 * Converts session/conversation data into UI history and Phill client history formats.
 */
export function convertSessionToHistoryFormats(
  ...
  // Convert to Phill client history format
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'user') {
    ...
    } else if (msg.type === 'phill') {
      // Handle Phill messages with potential tool calls
      ...
      if (hasToolCalls) {
        ...
        // Regular Phill message without tool calls
        const contentString = partListUnionToString(msg.content);
        if (msg.content && contentString.trim()) {
          clientHistory.push({
            role: 'model',
