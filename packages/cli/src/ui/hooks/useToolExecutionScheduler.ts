...
/**
 * The shape expected by usePhillStream.
...
export function useToolExecutionScheduler(
  ...
  const schedule: ScheduleFn = useCallback(
    async (request, signal) => {
      ...
      // 2. Trigger legacy reinjection logic (usePhillStream loop)
...
function adaptToolCalls(
  ...
  return coreCalls.map((coreCall): TrackedToolCall => {
    ...
    const responseSubmittedToPhill = prev?.responseSubmittedToPhill ?? false;
