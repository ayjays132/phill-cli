...
import { debugLogger, type Config } from 'phill-cli-core';
...
export function createShowMemoryAction(
  ...
  return async () => {
    ...
    const fileCount = config.getPhillMdFileCount();
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (PHILL.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (PHILL.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
