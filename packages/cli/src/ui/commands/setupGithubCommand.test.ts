...
import type { ToolActionReturn } from 'phill-cli-core';
...
describe('setupGithubCommand', async () => {
  ...
  it('downloads workflows, updates gitignore, and includes pipefail on non-windows', async () => {
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.phill/');
...
describe('updateGitignore', () => {
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .phill/ entry
    expect((content.match(/\.phill\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .phill/ folder',
      'my-app.phill/config',
    ...
    expect(content).toContain('.phill/');
    expect(content).toContain('gha-creds-*.json');
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.phill/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.phill/config');
