...
describe('restoreCommand', () => {
  ...
  beforeEach(async () => {
    ...
    phillTempDir = path.join(testRootDir, PHILL_DIR);
    checkpointsDir = path.join(phillTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempDir: vi.fn().mockReturnValue(phillTempDir),
      },
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .phill directory path.',
      });
    });
