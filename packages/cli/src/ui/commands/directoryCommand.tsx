...
import { refreshServerHierarchicalMemory } from 'phill-cli-core';
import {
  expandHomeDir,
...
async function finishAddingDirectories(
  ...
  if (added.length > 0) {
    try {
      ...
      addItem({
        ...
        text: `Successfully added PHILL.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
      });
    } catch (error) {
  ...
  if (added.length > 0) {
    const phill = config.getPhillClient();
    if (phill) {
      await phill.addDirectoryContext();
    }
    addItem({
      type: MessageType.INFO,
