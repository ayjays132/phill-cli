...
import { convertToRestPayload } from 'phill-cli-core';
...
const getSavedChatTags = async (
  ...
  const phillDir = cfg?.storage?.getProjectTempDir();
  if (!phillDir) {
    return [];
  }
  try {
    ...
    const files = await fsPromises.readdir(phillDir);
    const chatDetails: ChatDetail[] = [];

    for (const file of files) {
      if (file.startsWith(file_head) && file.endsWith(file_tail)) {
        const filePath = path.join(phillDir, file);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = config?.getPhillClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
...
const resumeCommand: SlashCommand = {
  ...
  action: async (context, args) => {
    ...
    for (const item of conversation.slice(INITIAL_HISTORY_LENGTH)) {
      ...
      uiHistory.push({
        type: (item.role && rolemap[item.role]) || MessageType.PHILL,
        text,
...
const shareCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    const chat = context.services.config?.getPhillClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
