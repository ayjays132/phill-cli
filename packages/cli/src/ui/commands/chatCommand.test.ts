...
import { AuthType, type PhillClient } from 'phill-cli-core';
    ...
    mockContext = createMockCommandContext({
      services: {
        config: {
          ...
          storage: {
            getProjectTempDir: () => '/project/root/.phill/tmp/mockhash',
          },
  ...
  describe('resume subcommand', () => {
    ...
    it('should resume a conversation with matching authType', async () => {
      const conversation: Content[] = [
        ...
        { role: 'user', parts: [{ text: 'hello phill' }] },
      ...
      expect(result).toEqual({
        type: 'load_history',
        history: [
          { type: 'user', text: 'hello phill' },
          { type: 'phill', text: 'hello world' },
  ...
  describe('share subcommand', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(process, 'cwd').mockReturnValue(
        path.resolve('/usr/local/google/home/myuser/phill-cli'),
      );
