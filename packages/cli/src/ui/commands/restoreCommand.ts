...
async function restoreAction(
  ...
  try {
    ...
    // 2. We trust that files valid according to this schema (written by usePhillStream) contain the full HistoryItem structure.
    ...
    for await (const action of actionStream) {
      if (action.type === 'message') {
        ...
        if (action.clientHistory) {
          config?.getPhillClient()?.setHistory(action.clientHistory);
        }
      }
    }
