...
import { coreEvents } from 'phill-cli-core';
...
describe('rewindCommand', () => {
    ...
    await waitFor(() => {
      ...
      expect(mockLoadHistory).toHaveBeenCalledWith(
        [
          ...
          expect.objectContaining({ text: 'old phill', id: 2 }),
        ],
        'New Prompt',
      );
  ...
  it('should fail if client is not initialized', () => {
    const context = createMockCommandContext({
      services: {
        config: { getPhillClient: () => undefined },
      },
    }) as unknown as CommandContext;
  ...
  it('should fail if recording service is unavailable', () => {
    const context = createMockCommandContext({
      services: {
        config: {
          getPhillClient: () => ({ getChatRecordingService: () => undefined }),
        },
      },
    }) as unknown as CommandContext;
  ...
  it('should return info if no user interactions found', () => {
    mockGetConversation.mockReturnValue({
      messages: [{ id: 'msg-1', type: 'phill', content: 'hello' }],
      sessionId: 'test-session',
    });
