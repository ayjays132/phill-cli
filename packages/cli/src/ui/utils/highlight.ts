...
// Matches slash commands, @ references, large paste placeholders, numbers, flags, and quoted strings.
const HIGHLIGHT_REGEX = new RegExp(
  `(^/[a-zA-Z0-9_-]+|@(?:\\\\ |[^,\\s;!?()\\[\\]{}])+|${PASTED_TEXT_PLACEHOLDER_REGEX.source}|\\b\\d+(?:\\.\\d+)?\\b|--[a-zA-Z0-9_-]+|"(?:\\\\.|[^"\\\\])*"|'(?:\\\\.|[^'\\\\])*')`,
  'g',
);
  ...
  const parseUntransformedInput = (text: string): HighlightToken[] => {
    ...
    while ((match = HIGHLIGHT_REGEX.exec(text)) !== null) {
      ...
      if (fullMatch.startsWith('/')) {
        ...
        type = 'number';
      } else if (fullMatch.startsWith('[Pasted Text:')) {
        type = 'paste';
      }
