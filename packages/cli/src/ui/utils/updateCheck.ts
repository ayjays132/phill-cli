...
import { getPackageJson, debugLogger } from 'phill-cli-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Phill CLI is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Phill CLI update available! ${currentVersion} → ${latestUpdate}`;
    ...
    if (errMsg.includes('PackageNotFoundError')) {
      return null;
    }
