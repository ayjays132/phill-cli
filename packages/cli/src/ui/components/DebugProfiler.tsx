...
import { coreEvents, CoreEvent, debugLogger } from 'phill-cli-core';
...
export const profiler = {
  ...
  hasLoggedRepeatedFlicker: false,
  ...
  registerFlickerHandler(constrainHeight: boolean) {
    const flickerHandler = () => {
      ...
      if (!this.hasLoggedFirstFlicker) {
        ...
        debugLogger.warn(
          'A flicker frame was detected. Type `/profile` for more info.',
        );
      } else if (!this.hasLoggedRepeatedFlicker && this.totalFlickerFrames >= 10) {
        this.hasLoggedRepeatedFlicker = true;
        debugLogger.error(
          'Repeated flicker frames were detected. This will cause UI instability. Type `/profile` for more info.',
        );
      }
    };
