...
describe('<ModelDialog />', () => {
  ...
  describe('Preview Logic', () => {
    ...
    it('should show "Phill 3 is now available" header if user has access but preview features disabled', () => {
      ...
      mockGetPreviewFeatures.mockReturnValue(false);
      const { lastFrame } = renderComponent();
      expect(lastFrame()).toContain('Phill 3 is now available.');
      expect(lastFrame()).toContain('Enable "Preview features" in /settings');
    });

    it('should show "Phill 3 is coming soon" header if user has no access', () => {
      mockGetHasAccessToPreviewModel.mockReturnValue(false);
      mockGetPreviewFeatures.mockReturnValue(false);
      const { lastFrame } = renderComponent();
      expect(lastFrame()).toContain('Phill 3 is coming soon.');
    });
    ...
    it('should NOT show header/subheader if preview options are shown', () => {
      ...
      mockGetPreviewFeatures.mockReturnValue(true);
      const { lastFrame } = renderComponent();
      expect(lastFrame()).not.toContain('Phill 3 is now available.');
      expect(lastFrame()).not.toContain('Phill 3 is coming soon.');
    });
  });
});
