...
vi.mock('../PhillRespondingSpinner.js', () => ({
  PhillRespondingSpinner: () => <Text>Spinner</Text>,
}));
...
describe('AlternateBufferQuittingDisplay', () => {
  ...
  it('renders with user and phill messages', () => {
    persistentStateMock.setData({ tipsShown: 0 });
    const history: HistoryItem[] = [
      { id: 1, type: 'user', text: 'Hello Phill' },
      { id: 2, type: 'phill', text: 'Hello User!' },
    ];
    const { lastFrame } = renderWithProviders(
      <AlternateBufferQuittingDisplay />,
      {
    ...
    expect(lastFrame()).toMatchSnapshot('with_user_phill_messages');
  });
});
