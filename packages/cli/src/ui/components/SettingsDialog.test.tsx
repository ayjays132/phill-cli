...
vi.mock('phill-cli-core', async (importOriginal) => {
  const actual = await importOriginal<typeof import('phill-cli-core')>();
  return {
    ...actual,
    DeviceManager: {
      getInstance: () => ({
        // Keep pending in tests to avoid async setState outside act() warnings.
        getInputDevices: () => new Promise(() => {}),
        getOutputDevices: () => new Promise(() => {}),
      }),
    },
  };
});
    ...
    it('should show restart prompt for restart-required settings', async () => {
      ...
      await waitFor(() => {
        expect(lastFrame()).not.toContain(
          'To see changes, Phill CLI must be restarted',
        );
      });
