...
import { coreEvents, debugLogger } from 'phill-cli-core';
...
import { DeviceManager, type AudioDevice } from 'phill-cli-core';
...
export function SettingsDialog({
  ...
  const viewportWidth = Math.max(mainAreaWidth - 8, 10);
  ...
  useEffect(() => {
    ...
    deviceManager
      .getInputDevices()
      .then((devices) => {
        if (mounted) setInputDevices(devices);
      })
      .catch((err) => {
        if (mounted) {
          debugLogger.error('Failed to list input devices:', err);
          setInputDevices([{ name: 'Default Device', id: 'default' }]);
        }
      });
    ...
    deviceManager
      .getOutputDevices()
      .then((devices) => {
        if (mounted) setOutputDevices(devices);
      })
      .catch((err) => {
        if (mounted) {
          debugLogger.error('Failed to list output devices:', err);
          setOutputDevices([{ name: 'Default Device', id: 'default' }]);
        }
      });
  ...
  const items: SettingsDialogItem[] = useMemo(() => {
    ...
    return settingKeys.map((key) => {
      ...
      // Special handling for voice.inputDevice
      if (key === 'voice.inputDevice' || key === 'voice.outputDevice') {
        type = 'enum';
        const currentId = getEffectiveValue(key, pendingSettings, {}) as string;
        const devices =
          key === 'voice.inputDevice' ? inputDevices : outputDevices;
        const device = devices.find((d) => d.id === currentId);
        const displayName = device ? device.name : currentId || 'default';
        // Re-apply modification indicator if needed
  ...
  const handleItemToggle = useCallback(
    (key: string, _item: SettingsDialogItem) => {
      const definition = getSettingDefinition(key);
      const isVoiceInput = key === 'voice.inputDevice';
      const isVoiceOutput = key === 'voice.outputDevice';

      if (!TOGGLE_TYPES.has(definition?.type) && !isVoiceInput && !isVoiceOutput) {
        return;
      }
      ...
      if (isVoiceInput) {
        const currentId = currentValue as string;
        const currentIndex = inputDevices.findIndex((d) => d.id === currentId);
        let nextIndex = currentIndex + 1;
        if (nextIndex >= inputDevices.length || nextIndex < 0) {
          nextIndex = 0;
        }
        newValue = inputDevices[nextIndex]?.id || 'default';

        setPendingSettings((prev) =>
          setPendingSettingValueAny(key, newValue, prev),
        );
      } else if (isVoiceOutput) {
        const currentId = currentValue as string;
        const currentIndex = outputDevices.findIndex((d) => d.id === currentId);
        let nextIndex = currentIndex + 1;
        if (nextIndex >= outputDevices.length || nextIndex < 0) {
          nextIndex = 0;
        }
        newValue = outputDevices[nextIndex]?.id || 'default';
      ...
      if (!requiresRestart(key)) {
        ...
        if (key === 'voice.enabled' && newValue !== voiceState.isEnabled) {
          setVoiceEnabled(Boolean(newValue));
        }
        if (key === 'voice.ttsEnabled' && newValue !== voiceState.ttsEnabled) {
          setTtsEnabled(Boolean(newValue));
        }
        if (key === 'voice.inputDevice') {
          setSelectedInputDevice(String(newValue || 'default'));
        }
        if (key === 'voice.outputDevice') {
          setSelectedOutputDevice(String(newValue || 'default'));
        }
      } else {
    ...
    (key: string, _item: SettingsDialogItem) => {
      ...
      if (!requiresRestart(key)) {
        ...
        if (
          ...
          defaultValue !== voiceState.isEnabled
        ) {
          setVoiceEnabled(defaultValue);
        }
        if (
          key === 'voice.ttsEnabled' &&
          typeof defaultValue === 'boolean' &&
          defaultValue !== voiceState.ttsEnabled
        ) {
          setTtsEnabled(defaultValue);
        }
        if (key === 'voice.inputDevice') {
          setSelectedInputDevice(String(defaultValue || 'default'));
        }
        if (key === 'voice.outputDevice') {
          setSelectedOutputDevice(String(defaultValue || 'default'));
        }
      }
  ...
  const footerContent = showRestartPrompt ? (
    <Text color={theme.status.warning}>
      To see changes, Phill CLI must be restarted. Press r to exit and apply
      changes now.
    </Text>
  ) : null;
