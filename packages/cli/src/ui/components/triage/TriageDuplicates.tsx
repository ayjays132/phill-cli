...
import { debugLogger, spawnAsync } from 'phill-cli-core';
...
interface AnalysisResult {
  ...
  recommendation: PhillRecommendation;
}
...
`;
      ...
      const rec = response as unknown as PhillRecommendation;
    ...
    try {
      if (action === 'duplicate' && state.canonicalIssue) {
        ...
        await spawnAsync('gh', [
          ...
          '-X',
          'PATCH',
          `repos/google-phill/phill-cli/issues/${String(state.currentIssue.number).replace(/[^a-zA-Z0-9-]/g, '')}`, // Sanitize issue number
          '-f',
          'state=closed',
          '-f',
