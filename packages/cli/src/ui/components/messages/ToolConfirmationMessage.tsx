...
type GlobalApprovalOption =
  | 'allow_all_actions_session'
  | 'allow_all_actions_forever';
type ConfirmationOption = ToolConfirmationOutcome | GlobalApprovalOption;
...
> = ({
  ...
  const handleSelect = useCallback(
    (item: ConfirmationOption) => {
      if (item === 'allow_all_actions_session') {
        try {
          config.setApprovalMode(ApprovalMode.YOLO);
        } catch (error) {
          debugLogger.warn(
            'Unable to switch to YOLO mode for session-wide approval:',
            error,
          );
        }
      ...
      if (item === 'allow_all_actions_forever') {
        try {
          config.setApprovalMode(ApprovalMode.YOLO);
        } catch (error) {
          debugLogger.warn(
            'Unable to switch to YOLO mode for persistent approval:',
            error,
          );
        }
        try {
          settings.setValue(
            SettingScope.User,
            'security.autoApproveAllActionsForever',
            true,
          );
        } catch (error) {
          debugLogger.warn(
            'Failed to persist security.autoApproveAllActionsForever:',
            error,
          );
        }
        handleConfirm(ToolConfirmationOutcome.ProceedOnce);
    ...
    [config, handleConfirm, settings],
  );

  const getOptions = useCallback(() => {
    const options: Array<RadioSelectItem<ConfirmationOption>> = [];
    ...
    if (confirmationDetails.type === 'edit') {
      if (!confirmationDetails.isModifying) {
        ...
        if (isTrustedFolder) {
          ...
          options.push({
            label: 'Allow ALL actions for this session (dangerous)',
            value: 'allow_all_actions_session',
            key: 'Allow ALL actions for this session (dangerous)',
          });
          options.push({
            label: 'Allow ALL actions forever (dangerous)',
            value: 'allow_all_actions_forever',
            key: 'Allow ALL actions forever (dangerous)',
          });
      ...
      {isTrustedFolder && (
        <Box marginTop={1} flexShrink={0}>
          <Text color={theme.text.secondary}>
            Configure defaults in /settings {'>'} Security {'>'} Approvals.
          </Text>
        </Box>
      )}
    </Box>
