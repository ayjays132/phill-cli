...
export function getDefaultTimeout() {
  ...
  if (env['PHILL_SANDBOX']) return 30000; // 30s in containers
...
export class TestRig {
  ...
  setup(
    ...
    const testFileDir =
      env['INTEGRATION_TEST_FILE_DIR'] || join(os.tmpdir(), 'phill-cli-tests');
  ...
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const isNpmReleaseTest =
      env['INTEGRATION_TEST_USE_INSTALLED_PHILL'] === 'true';
