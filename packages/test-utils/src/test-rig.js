...
import { DEFAULT_GEMINI_MODEL, PHILL_DIR } from '../../core/src/index.js';
import fs from 'node:fs';
import * as pty from '@lydell/node-pty';
import stripAnsi from 'strip-ansi';
import * as os from 'node:os';
const __dirname = dirname(fileURLToPath(import.meta.url));
const BUNDLE_PATH = join(__dirname, '..', '..', '..', 'bundle/phill.js');
// Get timeout based on environment
export function getDefaultTimeout() {
    if (env['CI'])
        return 60000; // 1 minute in CI
    if (env['PHILL_SANDBOX'])
...
export class TestRig {
    ...
    setup(testName, options = {}) {
        ...
        const testFileDir = env['INTEGRATION_TEST_FILE_DIR'] || join(os.tmpdir(), 'phill-cli-tests');
    ...
    _createSettingsFile(overrideSettings) {
        const projectPhillDir = join(this.testDir, PHILL_DIR);
        mkdirSync(projectPhillDir, { recursive: true });
        ...
        const settings = {
            ...
            sandbox: env['PHILL_SANDBOX'] !== 'false' ? env['PHILL_SANDBOX'] : false,
            // Don't show the IDE connection dialog when running from VsCode
            ide: { enabled: false, hasSeenNudge: true },
            ...overrideSettings, // Allow tests to override/add settings
        };
        writeFileSync(join(projectPhillDir, 'settings.json'), JSON.stringify(settings, null, 2));
    }
    ...
    _getCommandAndArgs(extraInitialArgs = []) {
        const isNpmReleaseTest = env['INTEGRATION_TEST_USE_INSTALLED_PHILL'] === 'true';
        const command = isNpmReleaseTest ? 'phill' : 'node';
    ...
    async waitForTelemetryEvent(eventName, timeout) {
        ...
        return poll(() => {
            ...
            return logs.some((logData) => logData.attributes &&
                logData.attributes['event.name'] === `phill_cli.${eventName}`);
        }, timeout, 100);
    }
                    ...
                    if (braceDepth === 0) {
                        ...
                        try {
                            ...
                            if (obj.body &&
                                ...
                                obj.attributes['event.name'] === 'phill_cli.tool_call') {
                                logs.push({
    ...
    readToolLogs() {
        ...
        for (const logData of parsedLogs) {
            ...
            if (logData.attributes &&
                logData.attributes['event.name'] === 'phill_cli.tool_call') {
    ...
    readAllApiRequest() {
        ...
        const apiRequests = logs.filter((logData) => logData.attributes &&
            logData.attributes['event.name'] === `phill_cli.api_request`);
        return apiRequests;
    }
    readLastApiRequest() {
        const logs = this._readAndParseTelemetryLog();
    ...
    async waitForMetric(metricName, timeout) {
        await this.waitForTelemetryReady();
        const fullName = metricName.startsWith('phill_cli.')
            ? metricName
            : `phill_cli.${metricName}`;
        return poll(() => {
            const logs = this._readAndParseTelemetryLog();
    ...
    readMetric(metricName) {
        ...
        for (const logData of logs) {
            if (logData.scopeMetrics) {
                for (const scopeMetric of logData.scopeMetrics) {
                    for (const metric of scopeMetric.metrics) {
                        if (metric.descriptor.name === `phill_cli.${metricName}`) {
                            return metric;
                        }
                    }
                }
    ...
    readHookLogs() {
        ...
        for (const logData of parsedLogs) {
            // Look for tool call logs
            if (logData.attributes &&
                logData.attributes['event.name'] === 'phill_cli.hook_call') {
                logs.push({
                    hookCall: {
